...
describe("package scripts", () => {
  ...
  it("exposes harness engineering commands", () => {
    ...
    const round8PackageTarget = JSON.parse(readFileSync(path.join(
      process.cwd(),
    ...
    expect(round9PackageTarget).toEqual({
      ...round8PackageTarget,
      scripts: {
        ...round8PackageTarget.scripts,
        test: "node scripts/run-conversation-disclosure-tests-v9.mjs",
        "program:check":
          "node scripts/check-runtime-convergence-program.mjs && node scripts/check-kernel-migration-program.mjs && node scripts/check-storage-convergence-program.mjs && node scripts/check-release-program.mjs && node scripts/check-conversation-disclosure-program-v9.mjs && node scripts/check-harness-state.mjs --diagnostic-only",
      },
    });
    ...
    expect(packageJson.scripts?.["program:check"]).toContain(
      "check-conversation-disclosure-program-v9.mjs",
    );
    ...
    expect(harnessSource).toContain(
      "const checkerReceipt = await runConversationDisclosureContinuationCheckerV9(",
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v9-receipt"',
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v9-local-diagnostic"',
    );
    expect(harnessSource).toContain("schemaVersion: 9");
    expect(harnessSource).toContain("CD03A-round9-successor-evolution-policy.json");
    expect(harnessSource).toContain("CD03A-round9-review-snapshot.json");
    expect(harnessSource).toContain("CD03A-round8-review-rejection.json");
