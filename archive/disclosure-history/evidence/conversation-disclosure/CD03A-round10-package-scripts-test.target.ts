...
describe("package scripts", () => {
  ...
  it("exposes harness engineering commands", () => {
    ...
    const round9PackageTarget = JSON.parse(readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round9-package.target.json",
    ), "utf8")) as PackageJson;
    const round10PackageTarget = JSON.parse(readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round10-package.target.json",
    ), "utf8")) as PackageJson;
    expect(round10PackageTarget).toEqual({
      ...round9PackageTarget,
      scripts: {
        ...round9PackageTarget.scripts,
        test: "node scripts/run-conversation-disclosure-tests-v10.mjs",
        "program:check":
          "node scripts/check-runtime-convergence-program.mjs && node scripts/check-kernel-migration-program.mjs && node scripts/check-storage-convergence-program.mjs && node scripts/check-release-program.mjs && node scripts/check-conversation-disclosure-program-v10.mjs && node scripts/check-harness-state.mjs --diagnostic-only",
      },
    });
    ...
    expect(packageJson.scripts?.["program:check"]).toContain(
      "check-conversation-disclosure-program-v10.mjs",
    );
    ...
    expect(harnessSource).toContain(
      "const checkerReceipt = await runConversationDisclosureContinuationCheckerV10(",
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v10-receipt"',
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v10-local-diagnostic"',
    );
    expect(harnessSource).toContain("schemaVersion: 10");
    expect(harnessSource).toContain("CD03A-round10-successor-evolution-policy.json");
    expect(harnessSource).toContain("CD03A-round10-review-snapshot.json");
    expect(harnessSource).toContain("CD03A-round9-review-rejection.json");
