...
describe("package scripts", () => {
  ...
  it("exposes harness engineering commands", () => {
    ...
    const round11PackageTarget = JSON.parse(readFileSync(path.join(
      process.cwd(),
    ...
    expect(round12PackageTarget).toEqual({
      ...round11PackageTarget,
      scripts: {
        ...round11PackageTarget.scripts,
        test: "node scripts/run-conversation-disclosure-tests-v12.mjs",
        "program:check":
          "node scripts/check-runtime-convergence-program.mjs && node scripts/check-kernel-migration-program.mjs && node scripts/check-storage-convergence-program.mjs && node scripts/check-release-program.mjs && node scripts/check-conversation-disclosure-program-v12.mjs && node scripts/check-harness-state.mjs --diagnostic-only",
      },
    });
    ...
    expect(
      packageJson.scripts?.["program:check"]?.match(
        /check-harness-state\.mjs --diagnostic-only/g,
      ),
    ).toHaveLength(1);
    expect(packageJson.scripts?.["program:check"]).toContain(
      "check-conversation-disclosure-program-v12.mjs",
    );
    const completionCheckerSource = readFileSync(
      path.join(
        process.cwd(),
        "scripts",
        "check-conversation-disclosure-program-v12.mjs",
    ...
    expect(completionCheckerSource).toContain(
      "P107A_V12_COMPLETION_ARTIFACTS",
    );
    expect(completionCheckerSource).toContain(
      "completed P107A artifact is missing or invalid",
    );
    expect(completionCheckerSource).toContain(
      "P108 completion requires a separately reviewed delta head",
    );
    expect(completionCheckerSource).toContain(
      "ZEROX_CD03A_CONTINUATION_ANCHOR",
    );
    expect(completionCheckerSource).toContain(
      "ZEROX_CD03A_BASE_ANCHOR",
    );
    expect(completionCheckerSource).toContain(
      "CONTINUATION_V12_GOVERNANCE_TRANSITION_TRUST_ROOTS",
    );
    expect(completionCheckerSource).toContain(
      "runConversationDisclosureContinuationCheckerV12",
    );
    expect(completionCheckerSource).toContain(
      "completed P107A requires caller-pinned base and continuation anchor environment",
    );
    const harnessSource = readFileSync(
      path.join(process.cwd(), "scripts", "check-harness-state.mjs"),
      "utf8",
    );
    ...
    expect(harnessSource).toContain(
      "const checkerReceipt = await runConversationDisclosureContinuationCheckerV12(",
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v12-receipt"',
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v12-local-diagnostic"',
    );
    expect(harnessSource).toContain("schemaVersion: 12");
    expect(harnessSource).toContain("CD03A-round12-successor-evolution-policy.json");
    expect(harnessSource).toContain("CD03A-round12-review-snapshot.json");
    expect(harnessSource).toContain("CD03A-round11-review-rejection.json");
    ...
    expect(harnessSource).not.toContain("cryptographically-proven-independent");
    ...
    const externalClosureRunnerSource = readFileSync(
      path.join(
        process.cwd(),
        "scripts",
    ...
    expect(externalClosureRunnerSource).toContain(
      "executeCandidateInFreshControlTree",
    );
    expect(externalClosureRunnerSource).toContain(
      "validateStagedTrustSet",
    );
    expect(externalClosureRunnerSource).toContain("captureCompletedFreezeMarker");
    expect(externalClosureRunnerSource).toContain("stageCompletedMarker");
    expect(externalClosureRunnerSource).toContain(
      "postflight completed marker inode changed",
    );
    expect(externalClosureRunnerSource).toContain("`${kind} pre-execution`");
    expect(externalClosureRunnerSource).toContain("`${kind} post-execution`");
    expect(externalClosureRunnerSource).toContain(
      "publishExternalAttestation",
    );
    expect(externalClosureRunnerSource).toContain(
      '"--external-anchor-output"',
    );
    expect(externalClosureRunnerSource).toContain(
      "validateExactExternalAttestation(attestation",
    );
    expect(externalClosureRunnerSource).toContain(
      "PUBLICATION_TRANSACTION_KIND",
    );
    expect(externalClosureRunnerSource).toContain("atomicPublishRepositoryFile");
    expect(externalClosureRunnerSource).toContain(
    ...
    expect(externalClosureRunnerSource).toContain("is_recoverable_prefix");
    expect(externalClosureRunnerSource).toContain("complete_recoverable_prefix");
    expect(externalClosureRunnerSource).toContain("capture_private_temp_fd");
    expect(externalClosureRunnerSource).toContain(
      'return 0 <= state["size"] < len(payload)',
    );
    expect(externalClosureRunnerSource).toContain(
      'maybe_exit("after-recovered-exact-fsync", 105)',
    );
    expect(externalClosureRunnerSource).toContain("require_private(temp_state, temp)");
    expect(externalClosureRunnerSource).toContain('target + ".remove.tombstone"');
    expect(externalClosureRunnerSource).toContain("completed_marker(tombstone, expected_state)");
    expect(externalClosureRunnerSource).toContain('operation: "retire"');
    expect(externalClosureRunnerSource).not.toContain("os.unlink(");
    expect(externalClosureRunnerSource).not.toContain("os.replace(temp, target");
    expect(externalClosureRunnerSource).not.toContain(
      "await rename(tempPath, absolutePath)",
    );
    expect(externalClosureRunnerSource).not.toContain("handle.truncate(0)");
    const reviewFreezerSource = readFileSync(
      path.join(
        process.cwd(),
        "scripts",
    ...
    expect(reviewFreezerSource).toContain(
      "featureFileSetDigest: hashCanonical(featureFiles)",
    );
    expect(reviewFreezerSource).toContain(
      ".filter((relativePath) => !contract.postReviewMutablePaths.includes(relativePath))",
    );
    expect(reviewFreezerSource).toContain(".slice()\n    .sort();");
    expect(reviewFreezerSource).toContain("validateReviewSnapshot(snapshot)");
    expect(reviewFreezerSource).toContain(
      '"conversation-disclosure-review-freeze-transaction"',
    );
    expect(reviewFreezerSource).toContain("atomicPublishRepositoryFile");
    expect(reviewFreezerSource).toContain(
    ...
    expect(reviewFreezerSource).toContain("is_recoverable_prefix");
    expect(reviewFreezerSource).toContain("complete_recoverable_prefix");
    expect(reviewFreezerSource).toContain("capture_private_temp_fd");
    expect(reviewFreezerSource).toContain(
      'return 0 <= state["size"] < len(payload)',
    );
    expect(reviewFreezerSource).toContain(
    ...
    expect(reviewFreezerSource).toContain("require_private(temp_state, temp)");
    expect(reviewFreezerSource).toContain('target + ".remove.tombstone"');
    expect(reviewFreezerSource).toContain("completed_marker(tombstone, expected_state)");
    expect(reviewFreezerSource).toContain('operation: "retire"');
    expect(reviewFreezerSource).not.toContain("os.unlink(");
    expect(reviewFreezerSource).not.toContain("os.replace(temp, target");
    expect(reviewFreezerSource).not.toContain("handle.truncate(0)");
    const runnerBridge = externalClosureRunnerSource.match(
      /const ANCHORED_FS_BRIDGE = String\.raw`([\s\S]*?)`;\n/,
    )?.[1];
    const freezerBridge = reviewFreezerSource.match(
      /const ANCHORED_FS_BRIDGE = String\.raw`([\s\S]*?)`;\n/,
    )?.[1];
    expect(runnerBridge).toBeTruthy();
    expect(freezerBridge).toBe(runnerBridge);
    const disclosureCheckerSource = readFileSync(
      path.join(
        process.cwd(),
        "scripts",
    ...
    expect(disclosureCheckerSource).toContain("validateExternalAttestation");
    expect(disclosureCheckerSource).toContain("validateExternalAnchor");
    expect(disclosureCheckerSource).toContain(
      '"--expected-external-anchor-digest"',
    );
    expect(disclosureCheckerSource).toContain("CLOSURE_STATUS_ATTESTED");
    expect(disclosureCheckerSource).toContain("CLOSURE_STATUS_PENDING");
    expect(disclosureCheckerSource).toContain(
    ...
    const featureList = JSON.parse(
      readFileSync(path.join(process.cwd(), ".zerox", "feature_list.json"), "utf8"),
    ) as { features: Array<{ id: string; files?: string[] }> };
    const p107Files = featureList.features.find(
      (feature) => feature.id === "P107-conversation-disclosure-domain-adapters",
    )?.files;
    expect(p107Files).toEqual(expect.arrayContaining([
      "package.json",
      ...
      "src/shared/conversationDisclosureFreeze.test.ts",
    ]));
    ...
    ) as { workstreams: Array<{ id: string; completionContract?: { requiredExecutableClosurePaths?: string[] } }> };
    expect(program.workstreams.find((workstream) => workstream.id === "CD03")
      ?.completionContract?.requiredExecutableClosurePaths).toEqual([
        "package.json",
        "scripts/check-harness-state.mjs",
        "scripts/check-conversation-disclosure-program.mjs",
      ]);
