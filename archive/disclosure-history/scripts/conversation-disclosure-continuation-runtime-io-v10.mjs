...
export const PRIVATE_MODE_V10 = 0o600;

const LEDGER_KIND = "conversation-disclosure-runtime-capture-ledger-v10";

export class RuntimeIoV10Error extends Error {
  constructor(code, message) {
    super(message);
    this.name = "RuntimeIoV10Error";
    this.code = code;
  }
}

export function createCaptureLedgerV10() {
  return {
    kind: LEDGER_KIND,
    entries: [],
...
export function sha256BytesV10(bytes) {
...
export async function captureStableFileV10(
  absolutePath,
  label = absolutePath,
  {
    expectedRoot,
  ...
  await assertExpectedRootV10(absolutePath, expectedRoot, label, true);
  const parents = await captureParentIdentitiesV10(absolutePath, label);
  let before;
  try {
  ...
  const requiredMode = requirePrivate ? PRIVATE_MODE_V10 : expectedMode;
  ...
  try {
    ...
    await assertParentIdentitiesV10(parents, label);
    await assertExpectedRootV10(absolutePath, expectedRoot, label, true);
    const capture = {
      kind: "present",
      absolutePath,
      ...
      bytes,
      digest: sha256BytesV10(bytes),
      mode: Number(opened.mode & 0o777n),
      uid: Number(opened.uid),
    };
    recordCaptureV10(ledger, capture);
    return capture;
  } finally {
    await handle.close();
  }
...
export function capturePrivateEvidenceV10(
  absolutePath,
  label = absolutePath,
  options = {},
) {
  return captureStableFileV10(absolutePath, label, {
    ...options,
    requirePrivate: true,
  });
}

export async function captureRequiredAbsentV10(
  absolutePath,
  label = absolutePath,
  { expectedRoot, ledger } = {},
) {
  requireAbsolutePath(absolutePath, label);
  await assertExpectedRootV10(absolutePath, expectedRoot, label, false);
  const parents = await captureParentIdentitiesV10(absolutePath, label);
  try {
...
export async function postflightCaptureLedgerV10(ledger) {
  requireLedgerV10(ledger);
  for (const capture of ledger.entries) {
    await assertParentIdentitiesV10(capture.parents, capture.label);
    await assertExpectedRootV10(
    ...
    const current = await captureStableFileV10(
      capture.absolutePath,
      capture.label,
      {
...
export async function publishPrivateExactV10(
  absolutePath,
  bytes,
  ...
  try {
    ...
    try {
      ...
      if (existingEntry.nlink === 1n) {
        const existing = await capturePrivateEvidenceV10(
          absolutePath,
          label,
          { expectedRoot },
        );
    ...
    const status = await runPrivatePublicationBridgeV10({
      directoryHandle,
      expectedParent,
      basename,
      bytes,
    ...
    await assertOpenedDirectoryV10(directoryHandle, expectedParent, label);
    await assertParentIdentitiesV10(parents, label);
    const published = await capturePrivateEvidenceV10(
      absolutePath,
      label,
      { expectedRoot },
    );
...
async function runPrivatePublicationBridgeV10({
  directoryHandle,
  expectedParent,
  basename,
  bytes,
...
function recordCaptureV10(ledger, capture) {
...
function requireLedgerV10(ledger) {
...
async function captureParentIdentitiesV10(absolutePath, label) {
...
async function assertParentIdentitiesV10(identities, label) {
...
async function assertOpenedDirectoryV10(handle, expected, label) {
...
async function assertExpectedRootV10(absolutePath, expectedRoot, label, leafExists) {
...
function failure(code, message) {
  return new RuntimeIoV10Error(`RUNTIME_IO_V10_${code}`, message);
}
