...
export const GOVERNANCE_V12_INPUT_KIND =
  "conversation-disclosure-program-governance-v12-input";
export const GOVERNANCE_V12_RESULT_KIND =
  "conversation-disclosure-program-governance-v12-result";
...
const P107A_WORKSTREAM_ID = CONTINUATION_V12_WORKSTREAM_ID;
const P107A_FEATURE_ID = CONTINUATION_V12_FEATURE_ID;
const P108_WORKSTREAM_ID = CONTINUATION_V12_SUCCESSOR_WORKSTREAM_ID;
const P108_FEATURE_ID = CONTINUATION_V12_SUCCESSOR_FEATURE_ID;
export const P107A_V12_COMPLETION_ARTIFACTS = Object.freeze([
  ...P107A_V11_COMPLETION_ARTIFACTS.filter((relativePath) =>
    !CONTINUATION_V12_ROUND11_REPOSITORY_FORBIDDEN_OUTPUT_PATHS.includes(
      relativePath,
    )),
  ...
  ".zerox/verification/conversation-disclosure/CD03A-round12-contract-review.json",
  ".zerox/verification/conversation-disclosure/CD03A-round12-runtime-review.json",
  ".zerox/verification/conversation-disclosure/CD03A-round12-governance-review.json",
...
export const PROGRAM_GOVERNANCE_V12_RULE_LEDGER = Object.freeze([
...
export const PROGRAM_GOVERNANCE_V12_RULE_IDS = Object.freeze(
  PROGRAM_GOVERNANCE_V12_RULE_LEDGER.map((rule) => rule.id),
);

export function validateConversationDisclosureProgramGovernanceV12(input) {
  const issues = new Map(PROGRAM_GOVERNANCE_V12_RULE_IDS.map((id) => [id, []]));
  ...
  run("CDG4-001-input-contract", () => {
    ...
    if (!sameOrderedStrings(Object.keys(value).sort(), expectedKeys)) {
      add("CDG4-001-input-contract", "input keys must exactly match the v12 contract");
    }
    if (!sameOrderedStrings(Object.keys(roster).sort(), [
      ...
      "schemaVersion",
      "workstreamIds",
    ]) || roster.schemaVersion !== 12) {
      add("CDG4-001-input-contract", "unknown closed-world governance schema");
    }
    if (!sameOrderedStrings(roster.ruleIds, PROGRAM_GOVERNANCE_V12_RULE_IDS)) {
      add("CDG4-001-input-contract", "closedWorld.ruleIds must exactly match the known v12 ledger");
    }
  ...
  run("CDG4-011-program-root-contract", () => {
    if (!plainObject(roster.programRootDefinition)
      ...
      || roster.programRootDefinitionDigest
        !== hashCanonicalV12(roster.programRootDefinition)) {
      add("CDG4-011-program-root-contract", "frozen Program root binding is invalid");
      return;
    }
    const liveDefinition = stableProgramRootDefinitionV12(program);
    if (canonicalJsonV12(liveDefinition)
        !== canonicalJsonV12(roster.programRootDefinition)
      || hashCanonicalV12(liveDefinition) !== roster.programRootDefinitionDigest) {
      add(
  ...
  run("CDG4-054-completion-artifacts", () => {
    ...
    if (!sameOrderedStrings(
      ...
      add(
        ...
        "P107A completionArtifacts must match the exact satisfiable Round12 closure set",
      );
    }
    const forbidden = new Set(CONTINUATION_V12_REJECTED_OUTPUT_ABSENT_PATHS);
  ...
  const ruleResults = PROGRAM_GOVERNANCE_V12_RULE_LEDGER.map((rule) => {
  ...
  return Object.freeze({
    schemaVersion: 12,
    kind: GOVERNANCE_V12_RESULT_KIND,
...
export function createProgramRootBindingV12(program) {
  const programRootDefinition = stableProgramRootDefinitionV12(program);
  return {
    programRootDefinition,
    programRootDefinitionDigest: hashCanonicalV12(programRootDefinition),
  };
}
