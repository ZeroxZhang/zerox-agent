...
export const GOVERNANCE_V10_INPUT_KIND =
  "conversation-disclosure-program-governance-v10-input";
export const GOVERNANCE_V10_RESULT_KIND =
  "conversation-disclosure-program-governance-v10-result";
...
const P107A_WORKSTREAM_ID = CONTINUATION_V10_WORKSTREAM_ID;
const P107A_FEATURE_ID = CONTINUATION_V10_FEATURE_ID;
const P108_WORKSTREAM_ID = CONTINUATION_V10_SUCCESSOR_WORKSTREAM_ID;
const P108_FEATURE_ID = CONTINUATION_V10_SUCCESSOR_FEATURE_ID;
export const P107A_V10_COMPLETION_ARTIFACTS = Object.freeze([
  ...P107A_V9_COMPLETION_ARTIFACTS.filter((relativePath) =>
    !CONTINUATION_V10_ROUND9_REPOSITORY_FORBIDDEN_OUTPUT_PATHS.includes(
      relativePath,
    )),
...
export const PROGRAM_GOVERNANCE_V10_RULE_LEDGER = Object.freeze([
...
export const PROGRAM_GOVERNANCE_V10_RULE_IDS = Object.freeze(
  PROGRAM_GOVERNANCE_V10_RULE_LEDGER.map((rule) => rule.id),
);

export function validateConversationDisclosureProgramGovernanceV10(input) {
  const issues = new Map(PROGRAM_GOVERNANCE_V10_RULE_IDS.map((id) => [id, []]));
  ...
  run("CDG4-001-input-contract", () => {
    ...
    if (!sameOrderedStrings(Object.keys(value).sort(), expectedKeys)) {
      add("CDG4-001-input-contract", "input keys must exactly match the v10 contract");
    }
    if (!sameOrderedStrings(Object.keys(roster).sort(), [
      ...
      "schemaVersion",
      "workstreamIds",
    ]) || roster.schemaVersion !== 10) {
      add("CDG4-001-input-contract", "unknown closed-world governance schema");
    }
    if (!sameOrderedStrings(roster.ruleIds, PROGRAM_GOVERNANCE_V10_RULE_IDS)) {
      add("CDG4-001-input-contract", "closedWorld.ruleIds must exactly match the known v10 ledger");
    }
  ...
  run("CDG4-011-program-root-contract", () => {
    if (!plainObject(roster.programRootDefinition)
      ...
      || roster.programRootDefinitionDigest
        !== hashCanonicalV10(roster.programRootDefinition)) {
      add("CDG4-011-program-root-contract", "frozen Program root binding is invalid");
      return;
    }
    const liveDefinition = stableProgramRootDefinitionV10(program);
    if (canonicalJsonV10(liveDefinition)
        !== canonicalJsonV10(roster.programRootDefinition)
      || hashCanonicalV10(liveDefinition) !== roster.programRootDefinitionDigest) {
      add(
  ...
  run("CDG4-054-completion-artifacts", () => {
    ...
    if (!sameOrderedStrings(
      ...
      add(
        ...
        "P107A completionArtifacts must match the exact satisfiable Round10 closure set",
      );
    }
    const forbidden = new Set(CONTINUATION_V10_REJECTED_OUTPUT_ABSENT_PATHS);
  ...
  const ruleResults = PROGRAM_GOVERNANCE_V10_RULE_LEDGER.map((rule) => {
  ...
  return Object.freeze({
    schemaVersion: 10,
    kind: GOVERNANCE_V10_RESULT_KIND,
...
export function createProgramRootBindingV10(program) {
  const programRootDefinition = stableProgramRootDefinitionV10(program);
  return {
    programRootDefinition,
    programRootDefinitionDigest: hashCanonicalV10(programRootDefinition),
  };
}
