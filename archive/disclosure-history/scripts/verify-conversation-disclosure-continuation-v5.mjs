...
// CD03A external closure runner v5. This file is intentionally self-contained:
...
const ROUND4_REVIEW_REJECTION_PATH =
...
const ROUND4_POLICY_ROOT = Object.freeze({
  path:
...
const ROUND4_SNAPSHOT_ROOT = Object.freeze({
  path:
...
const ROUND4_RECEIPT_ROOTS = Object.freeze([
  Object.freeze({
    lane: "governance",
    path:
...
const ROUND4_FINDING_IDS = Object.freeze([
  "R4-GOV-001",
]);
const ROUND4_FINDING_SET_DIGEST =
  "sha256:e0176105147f2c0c33071986a167044d9d394459be7b26a8606f126b3fcb61ac";
const ROUND4_FINDING_COUNTS = Object.freeze({
  critical: 0,
  major: 1,
  minor: 0,
});
const ROUND4_FORBIDDEN_OUTPUTS = Object.freeze([
...
const JOURNAL_KIND =
  "conversation-disclosure-continuation-publication-transaction-v5";
...
const POLICY_KEYS = Object.freeze([
  ...
  "round1Rejection", "round2PrefreezeRejection", "round4ReviewRejection",
...
const SNAPSHOT_KEYS = Object.freeze([
  ...
  "round4ReviewRejectionDigest", "schemaVersion",
...
const MANIFEST_KEYS = Object.freeze([
  ...
  "policy", "programId", "reviewReceipts", "round", "round4ReviewRejection",
...
const CONTROL_SET_KIND = "conversation-disclosure-continuation-control-set-v5";
...
const RECEIPT_KEYS = Object.freeze([
  ...
  "round4ReviewRejectionDigest", "schemaVersion", "snapshotDigest",
...
const ATTESTATION_KEYS = Object.freeze([
  ...
  "repositoryRealpath", "reviewAssurance", "round4ReviewRejectionDigest",
...
if (options.journalOutput !== `${options.externalAnchorOutput}.closure-v5.journal.json`) {
...
async function buildFreshTransaction() {
  ...
  await captureAndValidateRound4ReviewRejection(policy);
  ...
  if (!exactKeys(snapshot, SNAPSHOT_KEYS)
    ...
    || snapshot.round4ReviewRejectionDigest
      !== policy.round4ReviewRejection?.digest
  ...
  if (transitions.length !== 4) {
    fail("runner v5 requires exactly four governance transitions");
  }
...
async function captureAndValidateRound4ReviewRejection(policy) {
  const embedded = policy.round4ReviewRejection;
  const capture = await captureRepositoryFile(
    ROUND4_REVIEW_REJECTION_PATH,
    "Round4 review-rejection witness",
    { requirePrivate: true },
  );
  const rejection = parseJson(capture.bytes, "Round4 review-rejection witness");
  validateCanonicalObject(rejection, "Round4 review-rejection witness");
  if (canonicalJson(rejection) !== canonicalJson(embedded)) {
    fail("continuation policy Round4 review-rejection binding is stale");
  }
  await validateRound4ReviewRejection(rejection, policy);
}

async function validateRound4ReviewRejection(rejection, policy) {
  const expectedKeys = [
  ...
  if (!exactKeys(rejection, expectedKeys)
    ...
    || rejection.recoveryRound !== 5
    || rejection.rejectedRound !== 4
    || rejection.status !== "rejected_after_review"
    || canonicalJson(rejection.sourcePolicy) !== canonicalJson(ROUND4_POLICY_ROOT)
    || canonicalJson(rejection.sourceSnapshot)
      !== canonicalJson({
        ...ROUND4_SNAPSHOT_ROOT,
        frozenFileCount: 90,
        transitionPayloadFileCount: 4,
        baselineFileCount: 12,
      })
    || canonicalJson(rejection.findingIds) !== canonicalJson(ROUND4_FINDING_IDS)
    || rejection.findingSetDigest !== ROUND4_FINDING_SET_DIGEST
    || canonicalJson(rejection.aggregateFindingCounts)
      !== canonicalJson(ROUND4_FINDING_COUNTS)
    || canonicalJson(rejection.repositoryForbiddenOutputs)
      !== canonicalJson(ROUND4_FORBIDDEN_OUTPUTS)
    || !Array.isArray(rejection.failedReceipts)
    || rejection.failedReceipts.length !== 1) {
    fail("Round4 review-rejection schema or lifecycle binding is stale");
  }
  for (const expected of [
    ROUND4_POLICY_ROOT,
    ROUND4_SNAPSHOT_ROOT,
    ...ROUND4_RECEIPT_ROOTS,
  ]) {
    const current = await captureRepositoryFile(
      expected.path,
      `Round4 rejection source ${expected.path}`,
      { requirePrivate: expected === ROUND4_POLICY_ROOT
        || expected === ROUND4_SNAPSHOT_ROOT
        || ROUND4_RECEIPT_ROOTS.includes(expected) },
    );
    if (current.digest !== expected.byteSha256) {
      fail(`Round4 rejection source bytes drifted: ${expected.path}`);
    }
    const value = parseJson(current.bytes, `Round4 rejection source ${expected.path}`);
    const canonicalDigest = value.digest ?? hashCanonical(value);
    if (canonicalDigest !== expected.canonicalDigest) {
      fail(`Round4 rejection source canonical digest drifted: ${expected.path}`);
    }
  }
  for (let index = 0; index < ROUND4_RECEIPT_ROOTS.length; index += 1) {
    const actual = rejection.failedReceipts[index];
    const expected = ROUND4_RECEIPT_ROOTS[index];
    if (actual?.lane !== expected.lane
      || actual?.path !== expected.path
      || actual?.byteSha256 !== expected.byteSha256
      || actual?.canonicalDigest !== expected.canonicalDigest) {
      fail(`Round4 rejection receipt root is stale: ${expected.lane}`);
    }
  }
  for (const relativePath of rejection.repositoryForbiddenOutputs ?? []) {
    if (await repositoryEntryExists(repositoryRealpath, relativePath)) {
      fail(`Round4 forbidden repository output exists: ${relativePath}`);
    }
  }
}
...
async function convergePreparedTransaction(journal, { skipFaults }) {
  ...
  for (const [index, transition] of journal.governanceTransitions.entries()) {
    const requestedFault = skipFaults
      ? ""
      : process.env.ZEROX_CD03A_RUNNER_V5_TEST_FAULT ?? "";
    ...
    if (fault !== "none") fail(`injected runner v5 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
  ...
  for (const publication of journal.publications) {
    ...
    if (bridgeFault !== "none") fail(`injected runner v5 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
...
async function executeCandidatesInStage({ stagedTree, policy, baseAnchor, snapshot }) {
  const stageRoot = await realpath(
    await mkdtemp(path.join(os.tmpdir(), "zerox-cd03a-v5-stage-")),
  );
  ...
  try {
    ...
    for (const [kind, relativePath] of [["checker", checkerPath], ["harness", harnessPath]]) {
      ...
      const receiptKind = kind === "checker"
        ? "cd03a-continuation-checker-v5-receipt"
        : "cd03a-continuation-harness-v5-receipt";
...
async function removeValidatedStageRoot(stageRoot, controlRoot) {
  ...
  if (path.dirname(stageRealpath) !== temporaryRoot
    || !path.basename(stageRealpath).startsWith("zerox-cd03a-v5-stage-")
...
function validateJournal(journal, caller, repoRealpath, runnerDigest) {
  ...
  if (!exactKeys(journal, JOURNAL_KEYS)
    || journal.schemaVersion !== 5 || journal.kind !== JOURNAL_KIND
  ...
  if (!exactKeys(embeddedSnapshot, SNAPSHOT_KEYS)
    || embeddedSnapshot.schemaVersion !== 5
  ...
  if (!exactKeys(attestation, ATTESTATION_KEYS)
    ...
    || attestation?.round4ReviewRejectionDigest
      !== embeddedPendingManifest.round4ReviewRejection?.canonicalDigest
  ...
  if (!exactKeys(anchor, ANCHOR_KEYS)
    ...
    || anchor?.round4ReviewRejectionDigest
      !== embeddedPendingManifest.round4ReviewRejection?.canonicalDigest
...
function validateCandidateResultSet(
  results,
  checkerPath = "scripts/check-conversation-disclosure-continuation-v5.mjs",
) {
...
function parseOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    fail(`unknown runner v5 option: ${argument}`);
  }
  for (const key of [
    ...
    if (!values[key]) fail(`missing required runner v5 option: ${key}`);
  }
...
function injectFault(point) {
  if (process.env.ZEROX_CD03A_RUNNER_V5_TEST_FAULT === point) {
    fail(`injected runner v5 fault: ${point}`);
  }
}
...
function printPublicationReceipt(journal, recovered) {
  ...
  const receiptWithoutDigest = {
    kind: "cd03a-external-publication-v5-receipt",
    status: "passed",
    recovered,
...
function fail(message) {
  console.error("CD03A external continuation runner v5 failed:");
