...
// CD03A external closure runner v11. This file is intentionally self-contained:
...
const ROUND10_REVIEW_REJECTION_PATH =
...
const ROUND10_POLICY_ROOT = Object.freeze({
  path:
...
const ROUND10_SNAPSHOT_ROOT = Object.freeze({
  path:
...
const ROUND10_RECEIPT_ROOTS = Object.freeze([
  Object.freeze({
    lane: "contract",
    path:
      ".zerox/verification/conversation-disclosure/CD03A-round10-contract-review.json",
    byteSha256:
    ...
    challenge:
      "sha256:f6f14d9a5eb522bc0eb4936295d9994becf4fedc89624a98ab7eb419b7ce9660",
    verdict: "failed",
    findingCounts: Object.freeze({ critical: 0, major: 1, minor: 0 }),
  }),
]);
const ROUND10_FINDING_IDS = Object.freeze([
  "R10-CONTRACT-001",
]);
const ROUND10_FINDING_SET_DIGEST =
  "sha256:1d1568105e054964ae0fcf7f5e01c17a8bc8ba8610f1c4a5154c4ac91323bd07";
const ROUND10_FINDING_COUNTS = Object.freeze({
  critical: 0,
  major: 1,
  minor: 0,
});
const ROUND10_FORBIDDEN_OUTPUTS = Object.freeze([
...
const JOURNAL_KIND =
  "conversation-disclosure-continuation-publication-transaction-v11";
...
const POLICY_KEYS = Object.freeze([
  ...
  "round1Rejection", "round2PrefreezeRejection", "round10ReviewRejection",
...
const SNAPSHOT_KEYS = Object.freeze([
  ...
  "round10ReviewRejectionDigest", "schemaVersion",
...
const MANIFEST_KEYS = Object.freeze([
  ...
  "policy", "programId", "reviewReceipts", "round", "round10ReviewRejection",
...
const CONTROL_SET_KIND = "conversation-disclosure-continuation-control-set-v11";
...
const RECEIPT_KEYS = Object.freeze([
  ...
  "round10ReviewRejectionDigest", "schemaVersion", "snapshotDigest",
...
const ATTESTATION_KEYS = Object.freeze([
  ...
  "repositoryRealpath", "reviewAssurance", "round10ReviewRejectionDigest",
...
const ANCHOR_KEYS = Object.freeze([
  ...
  "reviewAssurance", "round10ReviewRejectionDigest", "runnerDigest",
...
if (options.journalOutput !== `${options.externalAnchorOutput}.closure-v11.journal.json`) {
...
async function buildFreshTransaction() {
  ...
  await captureAndValidateRound10ReviewRejection(policy);
  ...
  if (!exactKeys(snapshot, SNAPSHOT_KEYS)
    ...
    || snapshot.round10ReviewRejectionDigest
      !== policy.round10ReviewRejection?.digest
  ...
  if (!exactKeys(manifest, MANIFEST_KEYS)
    ...
    || manifest.round10ReviewRejection?.path !== ROUND10_REVIEW_REJECTION_PATH
    || manifest.validator?.path
      !== "scripts/check-conversation-disclosure-continuation-v11.mjs"
    || manifest.externalRunner?.path
      !== "scripts/verify-conversation-disclosure-continuation-v11.mjs"
  ...
  if (transitions.length !== 4) {
    fail("runner v11 requires exactly four governance transitions");
  }
...
async function captureAndValidateRound10ReviewRejection(policy) {
  const embedded = policy.round10ReviewRejection;
  const capture = await captureRepositoryFile(
    ROUND10_REVIEW_REJECTION_PATH,
    "Round10 review-rejection witness",
    { requirePrivate: true },
  );
  const rejection = parseJson(capture.bytes, "Round10 review-rejection witness");
  validateCanonicalObject(rejection, "Round10 review-rejection witness");
  if (canonicalJson(rejection) !== canonicalJson(embedded)) {
    fail("continuation policy Round10 review-rejection binding is stale");
  }
  await validateRound10ReviewRejection(rejection, policy);
}

async function validateRound10ReviewRejection(rejection, policy) {
  const expectedKeys = [
  ...
  if (!exactKeys(rejection, expectedKeys)
    ...
    || rejection.recoveryRound !== 11
    || rejection.rejectedRound !== 10
    || rejection.status !== "rejected_after_review"
    || canonicalJson(rejection.sourcePolicy) !== canonicalJson(ROUND10_POLICY_ROOT)
    || canonicalJson(rejection.sourceSnapshot)
      !== canonicalJson({
        ...ROUND10_SNAPSHOT_ROOT,
        frozenFileCount: 293,
        transitionPayloadFileCount: 4,
        baselineFileCount: 12,
      })
    || canonicalJson(rejection.findingIds) !== canonicalJson(ROUND10_FINDING_IDS)
    || rejection.findingSetDigest !== ROUND10_FINDING_SET_DIGEST
    || canonicalJson(rejection.aggregateFindingCounts)
      !== canonicalJson(ROUND10_FINDING_COUNTS)
    || canonicalJson(rejection.repositoryForbiddenOutputs)
      !== canonicalJson(ROUND10_FORBIDDEN_OUTPUTS)
    || !Array.isArray(rejection.completedReceipts)
    || rejection.completedReceipts.length !== ROUND10_RECEIPT_ROOTS.length) {
    fail("Round10 review-rejection schema or lifecycle binding is stale");
  }
  for (const expected of [
    ROUND10_POLICY_ROOT,
    ROUND10_SNAPSHOT_ROOT,
    ...ROUND10_RECEIPT_ROOTS,
  ]) {
    const current = await captureRepositoryFile(
      expected.path,
      `Round10 rejection source ${expected.path}`,
      { requirePrivate: expected === ROUND10_POLICY_ROOT
        || expected === ROUND10_SNAPSHOT_ROOT
        || ROUND10_RECEIPT_ROOTS.includes(expected) },
    );
    if (current.digest !== expected.byteSha256) {
      fail(`Round10 rejection source bytes drifted: ${expected.path}`);
    }
    const value = parseJson(current.bytes, `Round10 rejection source ${expected.path}`);
    const canonicalDigest = value.digest ?? hashCanonical(value);
    if (canonicalDigest !== expected.canonicalDigest) {
      fail(`Round10 rejection source canonical digest drifted: ${expected.path}`);
    }
  }
  for (let index = 0; index < ROUND10_RECEIPT_ROOTS.length; index += 1) {
    const actual = rejection.completedReceipts[index];
    const expected = ROUND10_RECEIPT_ROOTS[index];
    if (actual?.lane !== expected.lane
      ...
      fail(`Round10 rejection receipt root is stale: ${expected.lane}`);
    }
  }
  for (const relativePath of rejection.repositoryForbiddenOutputs ?? []) {
    if (await repositoryEntryExists(repositoryRealpath, relativePath)) {
      fail(`Round10 forbidden repository output exists: ${relativePath}`);
    }
  }
}
...
async function convergePreparedTransaction(journal, { skipFaults }) {
  ...
  for (const [index, transition] of journal.governanceTransitions.entries()) {
    const requestedFault = skipFaults
      ? ""
      : process.env.ZEROX_CD03A_RUNNER_V11_TEST_FAULT ?? "";
    ...
    if (fault !== "none") fail(`injected runner v11 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
  ...
  for (const publication of journal.publications) {
    ...
    if (bridgeFault !== "none") fail(`injected runner v11 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
...
async function executeCandidatesInStage({ stagedTree, policy, baseAnchor, snapshot }) {
  const stageRoot = await realpath(
    await mkdtemp(path.join(os.tmpdir(), "zerox-cd03a-v11-stage-")),
  );
  ...
  try {
    ...
    for (const [kind, relativePath] of [["checker", checkerPath], ["harness", harnessPath]]) {
      ...
      const receiptKind = kind === "checker"
        ? "cd03a-continuation-checker-v11-receipt"
        : "cd03a-continuation-harness-v11-receipt";
...
async function removeValidatedStageRoot(stageRoot, controlRoot) {
  ...
  if (path.dirname(stageRealpath) !== temporaryRoot
    || !path.basename(stageRealpath).startsWith("zerox-cd03a-v11-stage-")
...
async function retireExactFile(absolutePath, expectedDigest, label) {
  ...
  try {
    await runFilesystemBridge({
      ...
      originalDev: undefined,
      originalIno: undefined,
      targetDigest: "-",
...
function validateJournal(journal, caller, repoRealpath, runnerDigest) {
  ...
  if (!exactKeys(journal, JOURNAL_KEYS)
    || journal.schemaVersion !== 11 || journal.kind !== JOURNAL_KIND
  ...
  if (!exactKeys(embeddedSnapshot, SNAPSHOT_KEYS)
    || embeddedSnapshot.schemaVersion !== 11
  ...
  if (!exactKeys(attestation, ATTESTATION_KEYS)
    ...
    || attestation?.round10ReviewRejectionDigest
      !== embeddedPendingManifest.round10ReviewRejection?.canonicalDigest
  ...
  if (!exactKeys(anchor, ANCHOR_KEYS)
    ...
    || anchor?.round10ReviewRejectionDigest
      !== embeddedPendingManifest.round10ReviewRejection?.canonicalDigest
...
function validateCandidateResultSet(
  results,
  checkerPath = "scripts/check-conversation-disclosure-continuation-v11.mjs",
) {
...
function validateReviewSet(receipts, snapshot, policy, verifierNow) {
  ...
  const validatorDigest = policy.continuationExecutables?.find(
    (entry) => entry.kind === "checker"
      && entry.path === "scripts/check-conversation-disclosure-continuation-v11.mjs",
  )?.sha256;
...
function parseOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    fail(`unknown runner v11 option: ${argument}`);
  }
  for (const key of [
    ...
    if (!values[key]) fail(`missing required runner v11 option: ${key}`);
  }
...
function injectFault(point) {
  if (process.env.ZEROX_CD03A_RUNNER_V11_TEST_FAULT === point) {
    fail(`injected runner v11 fault: ${point}`);
  }
}
...
function printPublicationReceipt(journal, recovered) {
  ...
  const receiptWithoutDigest = {
    kind: "cd03a-external-publication-v11-receipt",
    status: "passed",
    recovered,
...
function fail(message) {
  console.error("CD03A external continuation runner v11 failed:");
