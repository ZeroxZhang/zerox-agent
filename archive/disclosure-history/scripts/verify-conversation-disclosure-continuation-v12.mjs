...
// CD03A external closure runner v12. This file is intentionally self-contained:
...
const ROUND11_REVIEW_REJECTION_PATH =
...
const ROUND11_POLICY_ROOT = Object.freeze({
  path:
...
const ROUND11_SNAPSHOT_ROOT = Object.freeze({
  path:
...
const ROUND11_RECEIPT_ROOTS = Object.freeze([
  Object.freeze({
    lane: "contract",
    path:
      ".zerox/verification/conversation-disclosure/CD03A-round11-contract-review.json",
    byteSha256:
      "sha256:22912a96d5bef4ec8a2df07c1af6947ba5a0a9d3417a5eddf75db1b2224310b1",
    canonicalDigest:
      "sha256:57f08bfcb4ba25d6a025147682fa441e9452a2ca2c164b9c56226d29262e0d88",
    challenge:
      "sha256:4100ec1f6bc737ec41e37b4ba33cfad2c61b9352b56515ae50def300cda80109",
    verdict: "passed",
  ...
  Object.freeze({
    lane: "runtime",
    path:
      ".zerox/verification/conversation-disclosure/CD03A-round11-runtime-review.json",
    byteSha256:
      "sha256:c911637686cbd884a49facba75a70cedf1abe0f328befa64c841a2bace356ad4",
    canonicalDigest:
      "sha256:e2025a7bc548c10370de8fa226b1df9eb35e735c142c511563fbe7b7e8c68c0c",
    challenge:
      "sha256:3db0eb48160eeccb6e4a08d2ad3f4fb99a71aa2e5eeae19994d8d52795b88edd",
    verdict: "failed",
  ...
  Object.freeze({
    lane: "governance",
    path:
      ".zerox/verification/conversation-disclosure/CD03A-round11-governance-review.json",
    byteSha256:
      "sha256:1d09681129535c1c5389aa4a18e41c2ef09ac0c8e23cbb3711578d38598c1a8a",
    canonicalDigest:
      "sha256:87226066a6318e7a1a7f23cfda91c681f3ebeb3bcba518ec9c383d9753b1663c",
    challenge:
      "sha256:d0cc7a5a3ac0805b466c9d4ec284f6d6ae2aee0adbb397115220f53aeb926caa",
    verdict: "passed",
    findingCounts: Object.freeze({ critical: 0, major: 0, minor: 0 }),
  }),
]);
const ROUND11_FINDING_IDS = Object.freeze([
  "R11-RUNTIME-001",
  "R11-RUNTIME-002",
  "R11-RUNTIME-003",
]);
const ROUND11_FINDING_SET_DIGEST =
  "sha256:d6e3763ba26567881445beef83ad4a8b11081af59e9961d7d6100f530e68a882";
const ROUND11_FINDING_COUNTS = Object.freeze({
  critical: 0,
  major: 3,
  minor: 0,
});
const ROUND11_FORBIDDEN_OUTPUTS = Object.freeze([
...
const JOURNAL_KIND =
  "conversation-disclosure-continuation-publication-transaction-v12";
...
const POLICY_KEYS = Object.freeze([
  ...
  "round1Rejection", "round2PrefreezeRejection", "round11ReviewRejection",
...
const SNAPSHOT_KEYS = Object.freeze([
  ...
  "round11ReviewRejectionDigest", "schemaVersion",
...
const MANIFEST_KEYS = Object.freeze([
  ...
  "policy", "programId", "reviewReceipts", "round", "round11ReviewRejection",
...
const CONTROL_SET_KIND = "conversation-disclosure-continuation-control-set-v12";
...
const RECEIPT_KEYS = Object.freeze([
  ...
  "round11ReviewRejectionDigest", "schemaVersion", "snapshotDigest",
...
const ATTESTATION_KEYS = Object.freeze([
  ...
  "repositoryRealpath", "reviewAssurance", "round11ReviewRejectionDigest",
  "runnerDigest", "schemaVersion", "snapshotDigest", "status",
  "validatorDigest",
]);
const ANCHOR_KEYS = Object.freeze([
  "attestationDigest", "callerDispatchSet", "candidateResults", "completedAt",
  "digest", "head", "identityAssurance", "kind",
  "parentEvidenceBundleDigest", "policyDigest", "repositoryRealpath",
  "reviewAssurance", "round11ReviewRejectionDigest", "runnerDigest",
  "schemaVersion", "snapshotDigest", "validatorDigest",
]);
let resolvedPython;
...
if (options.journalOutput !== `${options.externalAnchorOutput}.closure-v12.journal.json`) {
...
if (existingJournal) {
  if (existingJournal.nlink !== 1) {
    fail("prepared journal has an unpaired multi-link state");
  }
...
async function buildFreshTransaction() {
  ...
  if (!exactKeys(policy, POLICY_KEYS)
    ...
    || policy.programId !== policy.closedWorld?.programRootDefinition?.programId
  ...
  await captureAndValidateRound11ReviewRejection(policy);
  ...
  if (!exactKeys(snapshot, SNAPSHOT_KEYS)
    ...
    || snapshot.round11ReviewRejectionDigest
      !== policy.round11ReviewRejection?.digest
  ...
  if (!exactKeys(manifest, MANIFEST_KEYS)
    ...
    || manifest.round11ReviewRejection?.canonicalDigest
      !== policy.round11ReviewRejection?.digest
    || manifest.policy?.path !== POLICY_PATH
    || manifest.snapshot?.path !== SNAPSHOT_PATH
    || manifest.round11ReviewRejection?.path !== ROUND11_REVIEW_REJECTION_PATH
  ...
  validateReviewSet(receipts, snapshot, policy, Date.now());
  ...
  if (transitions.length !== 4) {
    fail("runner v12 requires exactly four governance transitions");
  }
  ...
  const transactionWithoutDigest = {
    ...
    runner: {
      ...
      nodeExecDev: nodeCapture.dev,
      nodeExecIno: nodeCapture.ino,
      nodeExecNlink: nodeCapture.nlink,
      nodeExecUid: nodeCapture.uid,
      nodeExecMode: nodeCapture.mode,
    },
    callerPins: {
...
async function captureAndValidateRound11ReviewRejection(policy) {
  const embedded = policy.round11ReviewRejection;
  const capture = await captureRepositoryFile(
    ROUND11_REVIEW_REJECTION_PATH,
    "Round11 review-rejection witness",
    { requirePrivate: true },
  );
  const rejection = parseJson(capture.bytes, "Round11 review-rejection witness");
  validateCanonicalObject(rejection, "Round11 review-rejection witness");
  if (canonicalJson(rejection) !== canonicalJson(embedded)) {
    fail("continuation policy Round11 review-rejection binding is stale");
  }
  await validateRound11ReviewRejection(rejection, policy);
}

async function validateRound11ReviewRejection(rejection, policy) {
  const expectedKeys = [
    "aggregateFindingCounts", "algorithm", "digest", "externalAnchorRule",
    "completedReceipts", "featureId", "findingIds", "findingSetDigest", "kind",
  ...
  if (!exactKeys(rejection, expectedKeys)
    ...
    || rejection.recoveryRound !== 12
    || rejection.rejectedRound !== 11
    || rejection.status !== "rejected_after_review"
    || canonicalJson(rejection.sourcePolicy) !== canonicalJson(ROUND11_POLICY_ROOT)
    || canonicalJson(rejection.sourceSnapshot)
      !== canonicalJson({
        ...ROUND11_SNAPSHOT_ROOT,
        frozenFileCount: 327,
        transitionPayloadFileCount: 4,
        baselineFileCount: 12,
      })
    || canonicalJson(rejection.findingIds) !== canonicalJson(ROUND11_FINDING_IDS)
    || rejection.findingSetDigest !== ROUND11_FINDING_SET_DIGEST
    || canonicalJson(rejection.aggregateFindingCounts)
      !== canonicalJson(ROUND11_FINDING_COUNTS)
    || canonicalJson(rejection.repositoryForbiddenOutputs)
      !== canonicalJson(ROUND11_FORBIDDEN_OUTPUTS)
    || !Array.isArray(rejection.completedReceipts)
    || rejection.completedReceipts.length !== ROUND11_RECEIPT_ROOTS.length) {
    fail("Round11 review-rejection schema or lifecycle binding is stale");
  }
  for (const expected of [
    ROUND11_POLICY_ROOT,
    ROUND11_SNAPSHOT_ROOT,
    ...ROUND11_RECEIPT_ROOTS,
  ]) {
    const current = await captureRepositoryFile(
      expected.path,
      `Round11 rejection source ${expected.path}`,
      { requirePrivate: expected === ROUND11_POLICY_ROOT
        || expected === ROUND11_SNAPSHOT_ROOT
        || ROUND11_RECEIPT_ROOTS.includes(expected) },
    );
    if (current.digest !== expected.byteSha256) {
      fail(`Round11 rejection source bytes drifted: ${expected.path}`);
    }
    const value = parseJson(current.bytes, `Round11 rejection source ${expected.path}`);
    const canonicalDigest = value.digest ?? hashCanonical(value);
    if (canonicalDigest !== expected.canonicalDigest) {
      fail(`Round11 rejection source canonical digest drifted: ${expected.path}`);
    }
  }
  for (let index = 0; index < ROUND11_RECEIPT_ROOTS.length; index += 1) {
    const actual = rejection.completedReceipts[index];
    const expected = ROUND11_RECEIPT_ROOTS[index];
    if (actual?.lane !== expected.lane
      ...
      || canonicalJson(actual?.findingCounts)
        !== canonicalJson(expected.findingCounts)) {
      fail(`Round11 rejection receipt root is stale: ${expected.lane}`);
    }
  }
  for (const relativePath of rejection.repositoryForbiddenOutputs ?? []) {
    if (await repositoryEntryExists(repositoryRealpath, relativePath)) {
      fail(`Round11 forbidden repository output exists: ${relativePath}`);
    }
  }
}
...
async function convergePreparedTransaction(journal, { skipFaults }) {
  ...
  for (const [index, transition] of journal.governanceTransitions.entries()) {
    const requestedFault = skipFaults
      ? ""
      : process.env.ZEROX_CD03A_RUNNER_V12_TEST_FAULT ?? "";
    ...
    if (fault !== "none") fail(`injected runner v12 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
  ...
  for (const publication of journal.publications) {
    ...
    if (bridgeFault !== "none") fail(`injected runner v12 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
...
function validateCompletedMarkerIdentity(marker, journal, existingJournalCapture) {
  ...
  if (String(marker.dev) !== identity[1]
    || String(marker.ino) !== identity[2]) {
    fail("completed marker inode does not match its filename binding");
  }
  ...
  if (existingJournalCapture) {
    if (marker.nlink !== 2 || existingJournalCapture.nlink !== 2
      || marker.dev !== existingJournalCapture.dev
      || marker.ino !== existingJournalCapture.ino) {
      fail("journal/marker recovery pair is not one exact two-link inode");
    }
  } else if (marker.nlink !== 1) {
    fail("completed marker has an unpaired multi-link state");
  }
}

async function finalizePreparedJournal(
  journalPath,
  markerPath,
  journalCapture,
  journalBytes,
) {
  if (path.dirname(journalPath) !== path.dirname(markerPath)) {
    fail("completed marker must share the prepared journal parent");
  }
  const parent = await openAnchoredParent(journalPath, "completed marker");
  try {
    const script = String.raw`
import hashlib, json, os, stat, sys
source, marker, digest, expected_dev, expected_ino, parent_dev, parent_ino = sys.argv[1:]
dir_fd = 3
...
def inspect(name):
    ...
    if not stat.S_ISREG(entry.st_mode) or entry.st_uid != os.geteuid() or stat.S_IMODE(entry.st_mode) != 0o600:
        die("journal completion leaf metadata is invalid")
    ...
    try:
        ...
        while True:
            ...
            if not chunk:
                break
            data += chunk
        return {"dev": str(opened.st_dev), "ino": str(opened.st_ino), "nlink": opened.st_nlink, "data": data}
    finally:
        os.close(fd)

directory = os.fstat(dir_fd)
if str(directory.st_dev) != parent_dev or str(directory.st_ino) != parent_ino:
    die("journal completion parent identity changed")
if "sha256:" + hashlib.sha256(payload).hexdigest() != digest:
    die("journal completion payload digest is stale")
source_state = inspect(source)
marker_state = inspect(marker)
if marker_state is not None:
    if marker_state["dev"] != expected_dev or marker_state["ino"] != expected_ino or marker_state["data"] != payload:
        die("completed marker identity is stale")
    if source_state is not None:
        if (source_state["dev"], source_state["ino"], source_state["nlink"]) != (marker_state["dev"], marker_state["ino"], 2) or marker_state["nlink"] != 2:
            die("journal/marker recovery pair is invalid")
        os.unlink(source, dir_fd=dir_fd)
        os.fsync(dir_fd)
    print(json.dumps({"status": "idempotent"}))
    raise SystemExit(0)
if source_state is None or source_state["dev"] != expected_dev or source_state["ino"] != expected_ino or source_state["nlink"] != 1 or source_state["data"] != payload:
    die("prepared journal identity is stale")
os.link(source, marker, src_dir_fd=dir_fd, dst_dir_fd=dir_fd, follow_symlinks=False)
os.fsync(dir_fd)
source_state = inspect(source)
marker_state = inspect(marker)
if source_state is None or marker_state is None or source_state["nlink"] != 2 or marker_state["nlink"] != 2 or (source_state["dev"], source_state["ino"]) != (marker_state["dev"], marker_state["ino"]):
    die("journal completion link commit is invalid")
os.unlink(source, dir_fd=dir_fd)
os.fsync(dir_fd)
...
`;
    ...
    const child = spawn(python, [
      ...
      journalCapture.digest,
      String(journalCapture.dev),
      String(journalCapture.ino),
    ...
    child.stdin.end(journalBytes);
    const code = await new Promise((resolve, reject) => {
      child.once("error", reject);
      child.once("close", resolve);
    });
    if (code !== 0) {
      fail(`journal completion failed: ${
        Buffer.concat(stderr).toString("utf8").trim() || `exit ${code}`
      }`);
    }
    await assertOpenedParent(parent, "completed marker");
    await assertParentIdentity(parent.capture, "completed marker");
  } finally {
...
async function executeCandidatesInStage({ stagedTree, policy, baseAnchor, snapshot }) {
  const stageRoot = await realpath(
    await mkdtemp(path.join(os.tmpdir(), "zerox-cd03a-v12-stage-")),
  );
  ...
  try {
    ...
    for (const [kind, relativePath] of [["checker", checkerPath], ["harness", harnessPath]]) {
      ...
      const receiptKind = kind === "checker"
        ? "cd03a-continuation-checker-v12-receipt"
        : "cd03a-continuation-harness-v12-receipt";
...
async function runNodeCandidate(scriptPath, args, cwd, childHome, childTmp) {
  ...
  try {
    const result = await execFile(nodeExecRealpath, [scriptPath, ...args], {
      cwd,
      encoding: "utf8",
      env: {
...
async function assertNodeExecutableIdentity(label) {
  const currentRealpath = await realpath(process.execPath);
  if (currentRealpath !== nodeExecRealpath) {
    fail(`${label} realpath changed`);
  }
  const current = await captureAbsoluteFile(nodeExecRealpath, label, {
    requirePrivate: false,
  });
  if (current.digest !== nodeCapture.digest
    || current.dev !== nodeCapture.dev
    || current.ino !== nodeCapture.ino
    || current.nlink !== nodeCapture.nlink
    || current.uid !== nodeCapture.uid
    || current.mode !== nodeCapture.mode) {
    fail(`${label} identity differs from the caller-pinned startup capture`);
  }
}
...
async function removeValidatedStageRoot(stageRoot, controlRoot) {
  ...
  if (path.dirname(stageRealpath) !== temporaryRoot
    || !path.basename(stageRealpath).startsWith("zerox-cd03a-v12-stage-")
...
async function captureAbsoluteFile(
  absolutePath,
  label,
  { requirePrivate = false, expectedRoot, allowedLinkCounts = [1] } = {},
) {
  ...
  if (!before.isFile() || before.isSymbolicLink()
    ...
    fail(`${label} must be a regular non-symlink file with an allowed link count`);
  }
  ...
  try {
    ...
    if (!opened.isFile() || !allowedLinkCounts.includes(opened.nlink)
    ...
    if (!finalLeaf.isFile() || finalLeaf.isSymbolicLink()
      || !allowedLinkCounts.includes(finalLeaf.nlink)
      || finalLeaf.nlink !== opened.nlink
...
function assertParentIdentityDigest(capture, expectedDigest, label) {
  if (expectedDigest !== undefined
    && hashCanonical(capture.entries) !== expectedDigest) {
    fail(`${label} parent identity differs from the prepared journal`);
  }
}
...
async function convergeAbsoluteFile({
  ...
  label,
  bridgeFault = "none",
  expectedParentIdentityDigest,
}) {
  const parent = await openAnchoredParent(absolutePath, label);
  try {
    assertParentIdentityDigest(parent.capture, expectedParentIdentityDigest, label);
    const current = await captureOptionalAbsoluteFile(absolutePath, label, {
      requirePrivate: false,
    });
    if (current) {
      assertParentIdentityDigest(
        { entries: current.parentIdentity },
        expectedParentIdentityDigest,
        label,
      );
    }
    ...
    assertParentIdentityDigest(
      { entries: finalCapture.parentIdentity },
      expectedParentIdentityDigest,
      label,
    );
...
op, target, original, replacement, mode_text, expected_dev, expected_ino, original_dev, original_ino, fault = sys.argv[1:]
dir_fd = 3
payload = sys.stdin.buffer.read()
...
def call_renameatx(function_name, source, destination, flag):
    ...
    if function is None:
        die("required atomic leaf primitive is unavailable: " + function_name)
    function.argtypes = [
    ...
    result = function(
        dir_fd, os.fsencode(source), dir_fd, os.fsencode(destination), flag,
    )
    if result != 0:
...
def atomic_noreplace(source, destination):
    if sys.platform == "darwin":
        ...
        die("atomic no-replace rename is unsupported on this platform")
...
def atomic_exchange(source, destination):
    if sys.platform == "darwin":
        ...
        die("atomic leaf exchange is unsupported on this platform")
...
if original != "absent" and (current["dev"] != original_dev or current["ino"] != original_ino):
    die("target original identity is stale")
...
if original == "absent":
    ...
    committed = inspect(target)
    old = inspect(temp)
    if committed is None or committed["digest"] != replacement or old is None or old["digest"] != original or old["dev"] != original_dev or old["ino"] != original_ino:
        atomic_exchange(temp, target)
        die("atomic exchange identity verification failed")
    if fault == "after-target-rename":
        raise SystemExit(98)
    atomic_noreplace(temp, discard)
    retired = inspect(discard)
    if retired is None or retired["digest"] != original or retired["dev"] != original_dev or retired["ino"] != original_ino:
        die("retired original identity mismatch")
    os.unlink(discard, dir_fd=dir_fd)
os.fsync(dir_fd)
...
async function runFilesystemBridge({
  ...
  const child = spawn(python, [
    ...
    String(parent.stat.dev),
    String(parent.stat.ino),
    originalDev === undefined ? "-" : String(originalDev),
    originalIno === undefined ? "-" : String(originalIno),
    fault,
  ], {
    env: { PATH: "/usr/bin:/bin" },
    stdio: ["pipe", "pipe", "pipe", parent.handle.fd],
...
function validateJournal(journal, caller, repoRealpath, runnerDigest) {
  ...
  if (!exactKeys(journal, JOURNAL_KEYS)
    || journal.schemaVersion !== 12 || journal.kind !== JOURNAL_KIND
  ...
  if (!exactKeys(journal.runner, [
    "nodeExecDev", "nodeExecIno", "nodeExecMode", "nodeExecNlink",
    "nodeExecPath", "nodeExecRealpath", "nodeExecSha256", "nodeExecUid",
    "sha256",
  ])
    || journal.runner.nodeExecPath !== process.execPath
    || journal.runner.nodeExecRealpath !== nodeExecRealpath
    || journal.runner.nodeExecDev !== nodeCapture.dev
    || journal.runner.nodeExecIno !== nodeCapture.ino
    || journal.runner.nodeExecNlink !== nodeCapture.nlink
    || journal.runner.nodeExecUid !== nodeCapture.uid
    || journal.runner.nodeExecMode !== nodeCapture.mode
  ...
  if (!exactKeys(embeddedSnapshot, SNAPSHOT_KEYS)
    || embeddedSnapshot.schemaVersion !== 12
  ...
  if (!exactKeys(attestation, ATTESTATION_KEYS)
    ...
    || attestation?.round11ReviewRejectionDigest
      !== embeddedPendingManifest.round11ReviewRejection?.canonicalDigest
  ...
  if (!exactKeys(anchor, ANCHOR_KEYS)
    ...
    || anchor?.round11ReviewRejectionDigest
      !== embeddedPendingManifest.round11ReviewRejection?.canonicalDigest
  ...
  validateCandidateResultSet(
    journal.candidateResults,
    embeddedPendingManifest.validator?.path,
  );
...
function validateCandidateResultSet(
  results,
  checkerPath = "scripts/check-conversation-disclosure-continuation-v12.mjs",
) {
  if (!Array.isArray(results) || results.length !== 2) {
    fail("prepared transaction journal candidate result set is invalid");
  }
  const expectedKinds = ["checker", "harness"];
  const expectedPaths = [checkerPath, "scripts/check-harness-state.mjs"];
  for (const [index, result] of results.entries()) {
    if (!exactKeys(result, CANDIDATE_RESULT_KEYS)
      || result.kind !== expectedKinds[index]
      || result.path !== expectedPaths[index]
...
function validateReviewSet(receipts, snapshot, policy, verifierNow) {
  const snapshotFileCount =
    (snapshot.frozenFiles?.length ?? 0)
    + (snapshot.transitionPayloadFiles?.length ?? 0)
    + (snapshot.baselineFiles?.length ?? 0);
  const validatorDigest = policy.continuationExecutables?.find(
    (entry) => entry.kind === "checker"
      && entry.path === "scripts/check-conversation-disclosure-continuation-v12.mjs",
  )?.sha256;
  ...
  if (receipts.length !== 3
    ...
    || receipts.some((receipt) =>
      ...
      || receipt.programId !== policy.programId
      || receipt.snapshotDigest !== snapshot.digest
      || receipt.snapshotFileCount !== snapshotFileCount
      || receipt.policyDigest !== policy.digest
      || receipt.validatorDigest !== validatorDigest
      || !Number.isFinite(Date.parse(receipt.completedAt))
      || new Date(receipt.completedAt).toISOString() !== receipt.completedAt
      || Date.parse(receipt.completedAt) < frozenAt
      || Date.parse(receipt.completedAt) > verifierNow
...
function validateGeneratedEvidence({
  ...
  validateCandidateResultSet(attestation.candidateResults, manifest.validator?.path);
  const expectedFinalManifest = withDigest({
    ...withoutDigest(manifest),
    status: "externally_attested",
    externalAttestation: {
  ...
  const attestationTime = Date.parse(attestation.completedAt);
  const anchorTime = Date.parse(anchor.completedAt);
  const lowerBounds = [
    Date.parse(snapshot.frozenAt),
  ...
  if (!exactKeys(attestation, ATTESTATION_KEYS)
    ...
    || canonicalJson(finalManifest) !== canonicalJson(expectedFinalManifest)
    || !Number.isFinite(attestationTime)
    || new Date(attestationTime).toISOString() !== attestation.completedAt
    || attestationTime > Date.now()
    || lowerBounds.some((value) => attestationTime < value)
    || !Number.isFinite(anchorTime)
    || new Date(anchorTime).toISOString() !== anchor.completedAt
    || anchorTime > Date.now()
    || anchorTime < attestationTime
    || attestation.pendingManifestDigest !== manifest.pendingManifestDigest
    || attestation.parentEvidenceBundleDigest
      !== policy.parentEvidence.bundleDigest
    || attestation.policyDigest !== policy.digest
    || attestation.snapshotDigest !== snapshot.digest
    || attestation.round11ReviewRejectionDigest
      !== policy.round11ReviewRejection.digest
    || attestation.validatorDigest !== manifest.validator.sha256
    || attestation.runnerDigest !== manifest.externalRunner.sha256
    || canonicalJson(attestation.callerDispatchSet)
    ...
    || canonicalJson(anchor.candidateResults)
      !== canonicalJson(attestation.candidateResults)
    || canonicalJson(anchor.callerDispatchSet)
      !== canonicalJson(manifest.callerDispatchSet)
    || canonicalJson(anchor.head) !== canonicalJson(expectedHead)) {
...
function parseOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    fail(`unknown runner v12 option: ${argument}`);
  }
  for (const key of [
    ...
    if (!values[key]) fail(`missing required runner v12 option: ${key}`);
  }
...
function injectFault(point) {
  if (process.env.ZEROX_CD03A_RUNNER_V12_TEST_FAULT === point) {
    fail(`injected runner v12 fault: ${point}`);
  }
}
...
function printPublicationReceipt(journal, recovered) {
  ...
  const receiptWithoutDigest = {
    kind: "cd03a-external-publication-v12-receipt",
    status: "passed",
    recovered,
...
function fail(message) {
  console.error("CD03A external continuation runner v12 failed:");
