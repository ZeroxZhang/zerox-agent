...
export const ROUND10_TRANSITION_MAPPINGS = Object.freeze([
  Object.freeze({
    source:
...
async function main() {
  ...
  try {
    const transitionState = await detectRound10TransitionState(root);
    let currentRoot = root;
    if (transitionState === "target") {
      currentRoot = await createFixture("zerox-r10-current-source-");
      await restoreCurrentRound10Sources(currentRoot);
    }
    await runVitest(currentRoot, [
      ...excludedFromCurrent.flatMap((entry) => ["--exclude", entry]),
    ]);

    const historicalRoot = await createFixture("zerox-r10-v3-history-");
...
export async function detectRound10TransitionState(repositoryRoot) {
  const states = [];
  for (const transition of ROUND10_TRANSITION_MAPPINGS) {
    const targetBytes = await readFile(path.join(repositoryRoot, transition.source));
    if (sha256(targetBytes) !== transition.toSha256) {
      throw new Error(
        `Round10 transition payload is stale: ${transition.source}`,
      );
    }
    ...
    if (liveDigest === transition.fromSha256) {
      ...
      throw new Error(
        `Round10 transition live bytes are a third state: ${transition.destination}`,
      );
    }
  }
  const uniqueStates = new Set(states);
  if (uniqueStates.size !== 1) {
    throw new Error("Round10 transition live bytes are mixed");
  }
  return states[0];
}

export async function restoreCurrentRound10Sources(fixture) {
  const archivePath =
    ".zerox/verification/conversation-disclosure/CD03A-round9-baseline-archive.json";
  ...
  for (const transition of ROUND10_TRANSITION_MAPPINGS) {
    ...
    if (!entry || entry.sha256 !== transition.fromSha256
      || entry.encoding !== "gzip-base64-v1") {
      throw new Error(
        `Round9 archive misses Round10 source: ${transition.destination}`,
      );
    }
    const bytes = gunzipSync(Buffer.from(entry.bytes, "base64"));
    if (sha256(bytes) !== transition.fromSha256) {
      throw new Error(
        `Round9 archive source bytes are stale: ${transition.destination}`,
      );
    }
    await writeFile(path.join(fixture, transition.destination), bytes);
  }
  if (await detectRound10TransitionState(fixture) !== "source") {
    throw new Error("Round10 source reconstruction did not converge");
  }
}

async function applyRound10Targets(fixture) {
  for (const { source, destination } of ROUND10_TRANSITION_MAPPINGS) {
    await writeFile(
