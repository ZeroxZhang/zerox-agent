...
try {
  ...
  const historicalRoot = await createFixture("zerox-r4-v3-history-");
  ...
  const targetRoot = await createFixture("zerox-r4-target-tests-");
  await applyRound4Targets(targetRoot);
...
async function applyRound4Targets(fixture) {
  const mappings = [
    [
