...
import {
  ...
  CONTINUATION_V8_LIFECYCLE_PROFILE_PHASES,
...
import {
  PROGRAM_GOVERNANCE_V8_RULE_IDS,
  validateConversationDisclosureProgramGovernanceV8,
...
const CHECKER_RECEIPT_KIND = "cd03a-continuation-checker-v8-receipt";
...
export async function runConversationDisclosureContinuationCheckerV8(
  argv = process.argv.slice(2),
) {
  const options = parseOptionsV8(argv);
  ...
  const [policy, archive, rejection, snapshot, program, featureList] =
    await Promise.all([
      readControlJson(CONTINUATION_V8_POLICY_PATH, "Round8 policy", {
  ...
  for (const entry of snapshot.frozenFiles) {
    const capture = await captureControl(
      entry.path,
      `Round8 frozen subject ${entry.path}`,
      {
        privateEvidence: [
          CONTINUATION_V8_POLICY_PATH,
          CONTINUATION_V8_ROUND7_REVIEW_REJECTION_PATH,
    ...
    if (capture.digest !== entry.sha256) {
      throw new Error(`Round8 frozen subject drifted: ${entry.path}`);
    }
  }
  for (const entry of snapshot.transitionPayloadFiles) {
    const capture = await captureControl(
      entry.path,
      `Round8 transition payload ${entry.path}`,
    );
    if (capture.digest !== entry.sha256) {
      throw new Error(`Round8 transition payload drifted: ${entry.path}`);
    }
  }
  ...
  for (const entry of snapshot.baselineFiles) {
    if (POST_TRANSITION_PHASES.has(options.mode)
      ...
      await captureControl(entry.path, `Round8 mutable baseline ${entry.path}`);
      continue;
    }
    const capture = await captureControl(
      entry.path,
      `Round8 baseline ${entry.path}`,
    );
    if (capture.digest !== entry.sha256) {
      throw new Error(`Round8 baseline drifted: ${entry.path}`);
    }
  }
  ...
  for (const transition of policy.governanceTransitions) {
    const [live, staged] = await Promise.all([
      captureControl(transition.path, `Round8 transition live ${transition.path}`),
      captureControl(
        transition.stagedTargetPath,
        `Round8 transition staged ${transition.stagedTargetPath}`,
      ),
    ]);
  ...
  if (options.mode === "review_pre_transition") {
    for (const relativePath of snapshot.reviewOutputAbsentPaths) {
      if (relativePath === CONTINUATION_V8_REVIEW_SNAPSHOT_PATH) continue;
      await captureRequiredAbsentV8(
        path.join(controlRoot, relativePath),
        `Round8 pre-review output ${relativePath}`,
        { expectedRoot: controlRoot, ledger },
      );
    }
  } else {
  ...
  const lifecycleProfile = selectLifecycleProfileV8(policy, options.mode);
  if (!lifecycleProfile) {
    throw new Error(`Round8 lifecycle profile is missing: ${options.mode}`);
  }
  assertNoErrors(validateLifecycleStateV8({
    phase: options.mode,
    activeFeatureId: program.activeFeatureId,
    nextFeatureId: program.nextFeatureId,
    workstreams: program.workstreams,
    features: featureList.features,
  }, policy), "Round8 lifecycle state");
  const parentEvidence = await materializeParentEvidence(
    policy,
    baseAnchor,
    readControlJson,
  );
  const governance = validateConversationDisclosureProgramGovernanceV8({
    program,
    featureList,
    closedWorld: {
      schemaVersion: 8,
      ...
      ruleIds: [...PROGRAM_GOVERNANCE_V8_RULE_IDS],
  ...
  assertNoErrors(governance.errors, "Round8 program governance");
  ...
  return {
    ...
    digest: hashCanonicalV8(receiptWithoutDigest),
  };
}
...
async function validateContinuationEvidence({
  ...
  const [policyCapture, snapshotCapture, rejectionCapture, validatorCapture,
    ...
    captureControl(CONTINUATION_V8_POLICY_PATH, "Round8 manifest policy", {
      privateEvidence: true,
    }),
    captureControl(
      CONTINUATION_V8_REVIEW_SNAPSHOT_PATH,
      "Round8 manifest snapshot",
      { privateEvidence: true },
    ),
    captureControl(
      CONTINUATION_V8_ROUND7_REVIEW_REJECTION_PATH,
      "Round7 manifest rejection",
      { privateEvidence: true },
    ),
    captureControl(manifest.validator?.path, "Round8 manifest validator"),
    captureControl(manifest.externalRunner?.path, "Round8 manifest runner"),
  ]);
  const reviewReferences = receipts.map((receipt) => ({
    lane: receipt.lane,
    path:
      `.zerox/verification/conversation-disclosure/CD03A-round8-${receipt.lane}-review.json`,
  ...
  const manifestBindings = {
    ...
    round7ReviewRejectionReference: {
  ...
  if (options.mode === "review_post_transition") {
    assertNoErrors(validateContinuationClosureManifestV8(
      manifest,
      manifestBindings,
    ), "Round8 pending closure manifest");
    ...
    await captureRequiredAbsentV8(
      path.join(controlRoot, CONTINUATION_V8_EXTERNAL_ATTESTATION_PATH),
      "Round8 pre-transaction external attestation",
      { expectedRoot: controlRoot, ledger },
    );
  ...
  const attestation = await readControlJson(
    CONTINUATION_V8_EXTERNAL_ATTESTATION_PATH,
    "Round8 external attestation",
    { privateEvidence: true },
  );
  const pendingManifest = reconstructPendingManifestV8(manifest);
  assertNoErrors(validateContinuationExternalAttestationV8(attestation, {
    policy,
    snapshot,
  ...
  if (ORDINARY_PHASES.has(options.mode)) {
    ...
    if (canonicalAnchor !== options.continuationAnchorPath
      ...
      throw new Error("Round8 continuation anchor must be canonical and external");
    }
    anchor = parseJson(
      (await capturePrivateEvidenceV8(
        canonicalAnchor,
        "Round8 continuation anchor",
        { ledger },
      )).bytes,
      "Round8 continuation anchor",
    );
    assertNoErrors(validateContinuationExternalAnchorV8(anchor, {
    ...
    if (anchor.repositoryRealpath !== subjectRepositoryRealpath
      ...
      throw new Error("Round8 continuation anchor bindings are stale");
    }
  }
  return { receipts, manifest, attestation, anchor };
}

function reconstructPendingManifestV8(finalManifest) {
  ...
  return withCanonicalDigestV8(projected);
}

function parseOptionsV8(argv) {
  const options = {
    errors: [],
  };
  const map = new Map([
  ...
  if (!CONTINUATION_V8_LIFECYCLE_PROFILE_PHASES.includes(options.mode)) {
    options.errors.push("checker V8 --mode is invalid");
  }
  return options;
}
...
if (process.argv[1]
  ...
  runConversationDisclosureContinuationCheckerV8()
    .then((receipt) => {
      process.stdout.write("Conversation disclosure continuation v8 check passed.\n");
      process.stdout.write(`${JSON.stringify(receipt)}\n`);
    })
    .catch((error) => {
      process.stderr.write("Conversation disclosure continuation v8 check failed:\n");
