...
import {
  ...
  CONTINUATION_V11_CALLER_DISPATCH_ASSURANCE,
...
const LANE_SCOPE = Object.freeze({
  contract:
    "Audit exact V11 schemas and semantic bindings, including candidate results, trusted time, final-manifest projection, complete roots, the exact successor-admission head, and rejection of P108 done.",
  runtime:
...
export function buildReviewDispatchArtifactsV11({
  policy,
  snapshot,
  challenges,
}) {
  if (!sha256DigestV11(policy?.digest)
    || !sha256DigestV11(snapshot?.digest)
    || snapshot.policyDigest !== policy.digest
    || !sha256DigestV11(policy?.round10ReviewRejection?.digest)) {
    throw new Error("dispatch builder requires one bound V11 policy and snapshot");
  }
  const challengeMap = new Map(challenges ?? []);
  if (challengeMap.size !== CONTINUATION_V11_REVIEW_LANES.length
    || CONTINUATION_V11_REVIEW_LANES.some(
      (lane) => !sha256DigestV11(challengeMap.get(lane)),
    )
  ...
  const instructions = [];
  const entries = CONTINUATION_V11_REVIEW_LANES.map((lane) => {
    const challenge = challengeMap.get(lane);
    const reviewContextId = `round11-${lane}-${challenge.slice(7, 23)}`;
    const taskPath = `round11/${lane}`;
    const agentLabel = `round11-${lane}-reviewer`;
    const text = [
      "READ-ONLY independent Round11 review.",
      "Do not edit, publish, chmod, rename, or delete any file.",
      `Policy: ${policy.digest}`,
      `Snapshot: ${snapshot.digest}`,
      `Rejected parent witness: ${policy.round10ReviewRejection.digest}`,
    ...
    const instructionDigest = sha256BytesV11(bytes);
    instructions.push({
      lane,
    ...
    return {
      lane,
      assurance: CONTINUATION_V11_CALLER_DISPATCH_ASSURANCE,
      challenge,
      instructionDigest,
      reviewContextId,
      taskPath,
  ...
  return {
    entries,
    dispatchSetDigest: hashCanonicalV11(entries),
    instructions,
  };
}

export async function buildConversationDisclosureReviewDispatchV11({
  ...
  const readPrivateJson = async (relativePath, label) =>
    JSON.parse((await capturePrivateEvidenceV11(
      path.join(root, relativePath),
      label,
      { expectedRoot: root, ledger },
    )).bytes.toString("utf8"));
  const [policy, archive, rejection, snapshot] = await Promise.all([
    readPrivateJson(CONTINUATION_V11_POLICY_PATH, "Round11 policy"),
    readPrivateJson(CONTINUATION_V11_BASELINE_ARCHIVE_PATH, "Round11 archive"),
    readPrivateJson(
      CONTINUATION_V11_ROUND10_REVIEW_REJECTION_PATH,
      "Round10 review rejection",
    ),
    readPrivateJson(CONTINUATION_V11_REVIEW_SNAPSHOT_PATH, "Round11 snapshot"),
  ]);
  ...
  assertNoErrors(validateBaselineArchiveV11(archive, policy), "Round11 archive");
  ...
  const artifacts = buildReviewDispatchArtifactsV11({
    policy,
    snapshot,
    challenges,
  });
  ...
  for (const instruction of artifacts.instructions) {
    publications.push(await publishPrivateExactV11(
      path.join(outputRoot, instruction.filename),
      instruction.bytes,
      { label: `Round11 ${instruction.lane} review instruction` },
    ));
  }
  ...
  publications.push(await publishPrivateExactV11(
    ...
    { label: "Round11 caller dispatch set" },
  ));
  return {
    ...artifacts,
...
if (process.argv[1]
  ...
  buildConversationDisclosureReviewDispatchV11(
