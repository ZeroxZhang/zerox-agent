...
export const GOVERNANCE_V11_INPUT_KIND =
  "conversation-disclosure-program-governance-v11-input";
export const GOVERNANCE_V11_RESULT_KIND =
  "conversation-disclosure-program-governance-v11-result";
...
const P107A_WORKSTREAM_ID = CONTINUATION_V11_WORKSTREAM_ID;
const P107A_FEATURE_ID = CONTINUATION_V11_FEATURE_ID;
const P108_WORKSTREAM_ID = CONTINUATION_V11_SUCCESSOR_WORKSTREAM_ID;
const P108_FEATURE_ID = CONTINUATION_V11_SUCCESSOR_FEATURE_ID;
export const P107A_V11_COMPLETION_ARTIFACTS = Object.freeze([
  ...P107A_V10_COMPLETION_ARTIFACTS.filter((relativePath) =>
    !CONTINUATION_V11_ROUND10_REPOSITORY_FORBIDDEN_OUTPUT_PATHS.includes(
      relativePath,
    )),
...
export const PROGRAM_GOVERNANCE_V11_RULE_LEDGER = Object.freeze([
...
export const PROGRAM_GOVERNANCE_V11_RULE_IDS = Object.freeze(
  PROGRAM_GOVERNANCE_V11_RULE_LEDGER.map((rule) => rule.id),
);

export function validateConversationDisclosureProgramGovernanceV11(input) {
  const issues = new Map(PROGRAM_GOVERNANCE_V11_RULE_IDS.map((id) => [id, []]));
  ...
  run("CDG4-001-input-contract", () => {
    ...
    if (!sameOrderedStrings(Object.keys(value).sort(), expectedKeys)) {
      add("CDG4-001-input-contract", "input keys must exactly match the v11 contract");
    }
    if (!sameOrderedStrings(Object.keys(roster).sort(), [
      ...
      "schemaVersion",
      "workstreamIds",
    ]) || roster.schemaVersion !== 11) {
      add("CDG4-001-input-contract", "unknown closed-world governance schema");
    }
    if (!sameOrderedStrings(roster.ruleIds, PROGRAM_GOVERNANCE_V11_RULE_IDS)) {
      add("CDG4-001-input-contract", "closedWorld.ruleIds must exactly match the known v11 ledger");
    }
  ...
  run("CDG4-011-program-root-contract", () => {
    if (!plainObject(roster.programRootDefinition)
      ...
      || roster.programRootDefinitionDigest
        !== hashCanonicalV11(roster.programRootDefinition)) {
      add("CDG4-011-program-root-contract", "frozen Program root binding is invalid");
      return;
    }
    const liveDefinition = stableProgramRootDefinitionV11(program);
    if (canonicalJsonV11(liveDefinition)
        !== canonicalJsonV11(roster.programRootDefinition)
      || hashCanonicalV11(liveDefinition) !== roster.programRootDefinitionDigest) {
      add(
  ...
  run("CDG4-054-completion-artifacts", () => {
    ...
    if (!sameOrderedStrings(
      ...
      add(
        ...
        "P107A completionArtifacts must match the exact satisfiable Round11 closure set",
      );
    }
    const forbidden = new Set(CONTINUATION_V11_REJECTED_OUTPUT_ABSENT_PATHS);
  ...
  const ruleResults = PROGRAM_GOVERNANCE_V11_RULE_LEDGER.map((rule) => {
  ...
  return Object.freeze({
    schemaVersion: 11,
    kind: GOVERNANCE_V11_RESULT_KIND,
...
export function createProgramRootBindingV11(program) {
  const programRootDefinition = stableProgramRootDefinitionV11(program);
  return {
    programRootDefinition,
    programRootDefinitionDigest: hashCanonicalV11(programRootDefinition),
  };
}
