...
// CD03A external closure runner v2. This file is intentionally self-contained:
...
const JOURNAL_KIND =
  "conversation-disclosure-continuation-publication-transaction-v2";
...
const SNAPSHOT_KEYS = Object.freeze([
  ...
  "successorWorkstreamDefinitionDigest", "workstreamId",
]);
...
const CONTROL_SET_KIND = "conversation-disclosure-continuation-control-set-v2";
...
if (options.journalOutput !== `${options.externalAnchorOutput}.closure-v2.journal.json`) {
...
async function buildFreshTransaction() {
  ...
  if (transitions.length !== 4) {
    fail("runner v2 requires exactly four governance transitions");
  }
...
async function convergePreparedTransaction(journal, { skipFaults }) {
  ...
  for (const [index, transition] of journal.governanceTransitions.entries()) {
    const requestedFault = skipFaults
      ? ""
      : process.env.ZEROX_CD03A_RUNNER_V2_TEST_FAULT ?? "";
    ...
    if (fault !== "none") fail(`injected runner v2 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
  ...
  for (const publication of journal.publications) {
    ...
    if (bridgeFault !== "none") fail(`injected runner v2 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
...
async function executeCandidatesInStage({ stagedTree, policy, baseAnchor, snapshot }) {
  const stageRoot = await realpath(
    await mkdtemp(path.join(os.tmpdir(), "zerox-cd03a-v2-stage-")),
  );
  ...
  try {
    ...
    for (const [kind, relativePath] of [["checker", checkerPath], ["harness", harnessPath]]) {
      ...
      const receiptKind = kind === "checker"
        ? "cd03a-continuation-checker-v2-receipt"
        : "cd03a-continuation-harness-v2-receipt";
...
async function removeValidatedStageRoot(stageRoot, controlRoot) {
  ...
  if (path.dirname(stageRealpath) !== temporaryRoot
    || !path.basename(stageRealpath).startsWith("zerox-cd03a-v2-stage-")
...
async function captureRepositoryFile(relativePath, label) {
...
function validateJournal(journal, caller, repoRealpath, runnerDigest) {
  ...
  if (!exactKeys(journal, JOURNAL_KEYS)
    || journal.schemaVersion !== 2 || journal.kind !== JOURNAL_KIND
  ...
  if (!exactKeys(embeddedSnapshot, SNAPSHOT_KEYS)
    || embeddedSnapshot.schemaVersion !== 2
...
function parseOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    fail(`unknown runner v2 option: ${argument}`);
  }
  for (const key of [
    ...
    if (!values[key]) fail(`missing required runner v2 option: ${key}`);
  }
...
function injectFault(point) {
  if (process.env.ZEROX_CD03A_RUNNER_V2_TEST_FAULT === point) {
    fail(`injected runner v2 fault: ${point}`);
  }
}
...
function printPublicationReceipt(journal, recovered) {
  ...
  const receiptWithoutDigest = {
    kind: "cd03a-external-publication-v2-receipt",
    status: "passed",
    recovered,
...
function fail(message) {
  console.error("CD03A external continuation runner v2 failed:");
