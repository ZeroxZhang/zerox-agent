...
export const GOVERNANCE_V2_INPUT_KIND =
  "conversation-disclosure-program-governance-v2-input";
export const GOVERNANCE_V2_RESULT_KIND =
  "conversation-disclosure-program-governance-v2-result";
...
export const PROGRAM_GOVERNANCE_V2_RULE_LEDGER = Object.freeze([
  Object.freeze({ id: "CDG2-001-input-contract", title: "Closed input and rule schema" }),
  Object.freeze({ id: "CDG2-010-program-shape", title: "Program identity and top-level shape" }),
  Object.freeze({ id: "CDG2-011-program-root-contract", title: "Frozen stable Program root" }),
  Object.freeze({ id: "CDG2-020-root-findings", title: "Exact D1-D13 root finding set" }),
  Object.freeze({ id: "CDG2-030-deferrals", title: "Deferred boundary declarations" }),
  Object.freeze({ id: "CDG2-040-scenario-schema", title: "Scenario identity and field schema" }),
  Object.freeze({ id: "CDG2-041-scenario-categories", title: "Exact scenario category vocabulary" }),
  Object.freeze({ id: "CDG2-042-scenario-coverage", title: "Scenario references and total coverage" }),
  Object.freeze({ id: "CDG2-050-workstream-schema", title: "Workstream and Feature identity schema" }),
  Object.freeze({ id: "CDG2-051-closed-roster", title: "Externally anchored exact roster" }),
  Object.freeze({ id: "CDG2-052-dependency-graph", title: "Dependency existence, order, and acyclicity" }),
  Object.freeze({ id: "CDG2-053-state-boundary", title: "Monotonic workstream state boundary" }),
  Object.freeze({ id: "CDG2-060-finding-owners", title: "Finding coverage and mandatory owners" }),
  Object.freeze({ id: "CDG2-070-implementation-gates", title: "Implementation and post-gate suffix" }),
  Object.freeze({ id: "CDG2-080-feature-status", title: "Feature registration and unfinished limit" }),
  Object.freeze({ id: "CDG2-081-active-next", title: "Active and next Feature pointers" }),
  Object.freeze({ id: "CDG2-090-p107a-p108-lifecycle", title: "P107A to P108 continuation lifecycle" }),
  Object.freeze({ id: "CDG2-100-cd03-evidence-refs", title: "Completed CD03 evidence reference shape" }),
]);

export const PROGRAM_GOVERNANCE_V2_RULE_IDS = Object.freeze(
  PROGRAM_GOVERNANCE_V2_RULE_LEDGER.map((rule) => rule.id),
);

export function validateConversationDisclosureProgramGovernanceV2(input) {
  const issues = new Map(PROGRAM_GOVERNANCE_V2_RULE_IDS.map((id) => [id, []]));
  ...
  run("CDG2-001-input-contract", () => {
    ...
    if (!plainObject(input)) add("CDG2-001-input-contract", "input must be an object");
    if (!sameOrderedStrings(Object.keys(value).sort(), expectedKeys)) {
      add("CDG2-001-input-contract", "input keys must exactly match the v2 contract");
    }
    if (!sameOrderedStrings(Object.keys(roster).sort(), [
      ...
      add("CDG2-001-input-contract", "unknown closed-world governance schema");
    }
    if (!sameOrderedStrings(roster.ruleIds, PROGRAM_GOVERNANCE_V2_RULE_IDS)) {
      add("CDG2-001-input-contract", "closedWorld.ruleIds must exactly match the known v2 ledger");
    }
    if (!plainObject(value.program) || !plainObject(value.featureList)
      ...
      add("CDG2-001-input-contract", "program, featureList, closedWorld, lifecycleProfile, and parentEvidence are required");
    }
  });

  run("CDG2-010-program-shape", () => {
    if (program.schemaVersion !== 1) add("CDG2-010-program-shape", "program schemaVersion must be 1");
    if (!nonEmpty(program.programId)) add("CDG2-010-program-shape", "programId is required");
    if (program.status !== "active" && program.status !== "completed") {
      add("CDG2-010-program-shape", "program status is invalid");
    }
    if (program.maxActiveFeatures !== 1) add("CDG2-010-program-shape", "maxActiveFeatures must remain 1");
    if (!stringArray(program.invariants) || program.invariants.length < 5) {
      add("CDG2-010-program-shape", "at least five invariants are required");
    }
    if (!stringArray(program.nonGoals) || program.nonGoals.length < 3) {
      add("CDG2-010-program-shape", "at least three nonGoals are required");
    }
    if (!Array.isArray(program.deferrals) || program.deferrals.length === 0
      || workstreamList.length === 0 || scenarioList.length === 0) {
      add("CDG2-010-program-shape", "deferrals, workstreams, and scenarioMatrix are required");
    }
    for (const [field, candidate] of [
      ...
      if (!repositoryPath(candidate)) add("CDG2-010-program-shape", `${field} must be repository-relative`);
    }
    if (program.acceptanceManifest !== CANONICAL_ACCEPTANCE_MANIFEST) {
      add("CDG2-010-program-shape", "acceptanceManifest is not canonical");
    }
  });

  run("CDG2-011-program-root-contract", () => {
    if (!plainObject(roster.programRootDefinition)
      ...
      || roster.programRootDefinitionDigest
        !== hashCanonicalV2(roster.programRootDefinition)) {
      add("CDG2-011-program-root-contract", "frozen Program root binding is invalid");
      return;
    }
    const liveDefinition = stableProgramRootDefinitionV2(program);
    if (canonicalJsonV2(liveDefinition)
        !== canonicalJsonV2(roster.programRootDefinition)
      || hashCanonicalV2(liveDefinition) !== roster.programRootDefinitionDigest) {
      add(
        "CDG2-011-program-root-contract",
        "live stable Program root differs from the independently frozen definition",
      );
    }
  });

  run("CDG2-020-root-findings", () => {
    if (!exactStringSet(program.rootFindings, REQUIRED_FINDINGS)) {
      add("CDG2-020-root-findings", "rootFindings must be the exact unique D1-D13 set");
    }
  });

  run("CDG2-030-deferrals", () => {
    const ids = new Set();
    for (const [index, deferral] of asArray(program.deferrals).entries()) {
      if (!nonEmpty(deferral?.id) || ids.has(deferral?.id)) {
        add("CDG2-030-deferrals", `deferrals[${index}] has a missing or duplicate id`);
      }
      ids.add(deferral?.id);
      if (deferral?.status !== "kept_deferred" || !nonEmpty(deferral?.trigger)
        || !nonEmpty(deferral?.prohibitedCurrentAction)) {
        add("CDG2-030-deferrals", `deferrals[${index}] does not preserve the deferred boundary`);
      }
    }
  });

  run("CDG2-040-scenario-schema", () => {
    const ids = new Set();
    for (const [index, scenario] of scenarioList.entries()) {
      if (!nonEmpty(scenario?.id) || ids.has(scenario?.id)) {
        add("CDG2-040-scenario-schema", `scenarioMatrix[${index}] has a missing or duplicate id`);
      }
      ids.add(scenario?.id);
      if (!nonEmpty(scenario?.title) || !nonEmpty(scenario?.surface)
        ...
        add("CDG2-040-scenario-schema", `scenarioMatrix[${index}] has invalid fields`);
      }
    }
  });

  run("CDG2-041-scenario-categories", () => {
    const present = new Set();
    for (const scenario of scenarioList) {
      if (!REQUIRED_SCENARIO_CATEGORIES.includes(scenario?.category)) {
        add("CDG2-041-scenario-categories", `unknown scenario category ${String(scenario?.category)}`);
      } else {
        present.add(scenario.category);
      }
    }
    for (const category of REQUIRED_SCENARIO_CATEGORIES) {
      if (!present.has(category)) add("CDG2-041-scenario-categories", `missing scenario category ${category}`);
    }
  });

  run("CDG2-042-scenario-coverage", () => {
    const referenced = new Set();
    for (const workstream of workstreamList) {
      if (!stringArray(workstream?.acceptanceScenarioIds)) {
        add("CDG2-042-scenario-coverage", `${String(workstream?.id)} acceptanceScenarioIds are invalid`);
        continue;
      }
      for (const scenarioId of workstream.acceptanceScenarioIds) {
        referenced.add(scenarioId);
        if (!scenarios.has(scenarioId)) {
          add("CDG2-042-scenario-coverage", `${String(workstream?.id)} references unknown scenario ${scenarioId}`);
        }
      }
    }
    for (const scenario of scenarioList) {
      if (nonEmpty(scenario?.id) && !referenced.has(scenario.id)) {
        add("CDG2-042-scenario-coverage", `scenario ${scenario.id} is unreferenced`);
      }
    }
  });

  run("CDG2-050-workstream-schema", () => {
    const ids = new Set();
    const featureIds = new Set();
    for (const [index, workstream] of workstreamList.entries()) {
      if (!nonEmpty(workstream?.id) || ids.has(workstream?.id)) {
        add("CDG2-050-workstream-schema", `workstreams[${index}] has a missing or duplicate id`);
      }
      ids.add(workstream?.id);
      if (!nonEmpty(workstream?.featureId) || featureIds.has(workstream?.featureId)) {
        add("CDG2-050-workstream-schema", `workstreams[${index}] has a missing or duplicate featureId`);
      }
      ...
      if (!["planned", "in_progress", "completed"].includes(workstream?.state)
        ...
        add("CDG2-050-workstream-schema", `workstreams[${index}] has invalid governance fields`);
      }
    }
  });

  run("CDG2-051-closed-roster", () => {
    if (!plainObject(roster)
      ...
      add("CDG2-051-closed-roster", "live workstream/Feature roster differs from the anchored exact roster");
    }
    if (!uniqueStrings(roster.workstreamIds) || !uniqueStrings(lifecycle.featureIds)) {
      add("CDG2-051-closed-roster", "exact roster ids must be unique non-empty strings");
    }
  });

  run("CDG2-052-dependency-graph", () => {
    for (const workstream of workstreamList) {
      for (const dependency of asArray(workstream?.dependsOn)) {
        if (!workstreams.has(dependency)) {
          add("CDG2-052-dependency-graph", `${String(workstream?.id)} depends on unknown ${String(dependency)}`);
        }
      }
    }
    for (const cycle of dependencyCycles(workstreams)) {
      add("CDG2-052-dependency-graph", `dependency cycle: ${cycle.join(" -> ")}`);
    }
    if (workstreamList.length > 0 && asArray(workstreamList[0]?.dependsOn).length > 0) {
      add("CDG2-052-dependency-graph", "first workstream must be the dependency root");
    }
    for (let index = 1; index < workstreamList.length; index += 1) {
      ...
      if (!dependsTransitively(current?.id, previous?.id, workstreams)) {
        add("CDG2-052-dependency-graph", `${String(current?.id)} bypasses preceding ${String(previous?.id)}`);
      }
    }
  });

  run("CDG2-053-state-boundary", () => {
    let activeSeen = false;
    let plannedSeen = false;
    for (const workstream of workstreamList) {
      if (workstream?.state === "completed" && (activeSeen || plannedSeen)) {
        add("CDG2-053-state-boundary", `completed ${String(workstream?.id)} follows unfinished work`);
      } else if (workstream?.state === "in_progress") {
        if (activeSeen || plannedSeen) add("CDG2-053-state-boundary", `active ${String(workstream?.id)} is outside the state boundary`);
        activeSeen = true;
      ...
      if (workstream?.state !== "planned") {
        for (const dependency of asArray(workstream?.dependsOn)) {
          if (workstreams.get(dependency)?.state !== "completed") {
            add("CDG2-053-state-boundary", `${String(workstream?.id)} starts before ${dependency} completes`);
          }
        }
      }
    }
  ...
  run("CDG2-060-finding-owners", () => {
    const covered = new Set(workstreamList.flatMap((item) => asArray(item?.findings)));
    for (const finding of REQUIRED_FINDINGS) {
      if (!covered.has(finding)) add("CDG2-060-finding-owners", `program does not cover ${finding}`);
      const owner = workstreams.get(MANDATORY_FINDING_OWNERS[finding]);
      if (!asArray(owner?.findings).includes(finding)) {
        add("CDG2-060-finding-owners", `${MANDATORY_FINDING_OWNERS[finding]} must own ${finding}`);
      }
    }
  });

  run("CDG2-070-implementation-gates", () => {
    ...
    if (implementationIndex < 1 || implementationIndex >= workstreamList.length - 1) {
      add("CDG2-070-implementation-gates", "implementationCompletionWorkstreamId is outside the valid boundary");
      return;
    }
    ...
    if (!sameOrderedStrings(program.postImplementationGates, expectedGates)) {
      add("CDG2-070-implementation-gates", "postImplementationGates must equal the ordered suffix");
    }
    ...
    for (const finding of REQUIRED_FINDINGS) {
      if (!implementationFindings.has(finding)) {
        add("CDG2-070-implementation-gates", `implementation workstreams do not own ${finding}`);
      }
    }
  });

  run("CDG2-080-feature-status", () => {
    const seen = new Set();
    for (const feature of features) {
      if (!nonEmpty(feature?.id) || seen.has(feature?.id)) {
        add("CDG2-080-feature-status", "Feature ids must be unique non-empty strings");
      }
      seen.add(feature?.id);
      if (feature?.status !== "done" && feature?.status !== "in_progress") {
        add("CDG2-080-feature-status", `Feature ${String(feature?.id)} has an invalid status`);
      }
    }
    for (const workstream of workstreamList) {
      const feature = featureMap.get(workstream?.featureId);
      if (workstream?.state === "completed" && feature?.status !== "done") {
        add("CDG2-080-feature-status", `completed ${String(workstream?.id)} requires a done Feature`);
      }
      if (workstream?.state === "in_progress" && feature?.status !== "in_progress") {
        add("CDG2-080-feature-status", `active ${String(workstream?.id)} requires an in_progress Feature`);
      }
      if (workstream?.state === "planned" && feature) {
        add("CDG2-080-feature-status", `planned ${String(workstream?.id)} must not be registered`);
      }
    }
    if (features.filter((feature) => feature?.status !== "done").length > program.maxActiveFeatures) {
      add("CDG2-080-feature-status", "unfinished Feature count exceeds maxActiveFeatures");
    }
  });

  run("CDG2-081-active-next", () => {
    ...
    if (activeWorkstreams.length > program.maxActiveFeatures) {
      add("CDG2-081-active-next", "active workstream count exceeds maxActiveFeatures");
    }
    if (program.status === "active") {
      if (program.activeFeatureId === null) {
        if (activeWorkstreams.length > 0) add("CDG2-081-active-next", "idle program contains active work");
        const next = workstreamList.find((item) => item?.featureId === program.nextFeatureId);
        if (next?.state !== "planned") add("CDG2-081-active-next", "idle nextFeatureId must identify planned work");
      } else {
        const active = workstreamList.find((item) => item?.featureId === program.activeFeatureId);
        if (active?.state !== "in_progress") add("CDG2-081-active-next", "activeFeatureId does not identify active work");
        if (program.nextFeatureId !== program.activeFeatureId) {
          add("CDG2-081-active-next", "nextFeatureId must equal activeFeatureId while active");
        }
      }
    } else if (program.activeFeatureId !== null || program.nextFeatureId !== null
      || workstreamList.some((item) => item?.state !== "completed")) {
      add("CDG2-081-active-next", "completed program must clear pointers and complete every workstream");
    }
  });

  run("CDG2-090-p107a-p108-lifecycle", () => {
    const expectedKeys = [
      "featureIds",
      "p107aFeatureId",
      "p107aWorkstreamId",
    ...
    if (!sameOrderedStrings(Object.keys(lifecycle).sort(), expectedKeys)
      ...
      add("CDG2-090-p107a-p108-lifecycle", "P107A/P108 lifecycle identity is not exact");
      return;
    }
    ...
    if (lifecycle.phase === "review_pre" || lifecycle.phase === "review_post") {
      if (p107aWorkstream?.state !== "in_progress" || p107aFeature?.status !== "in_progress"
        ...
        add("CDG2-090-p107a-p108-lifecycle", `${lifecycle.phase} state tuple is invalid`);
      }
      ...
      if (p107aWorkstream?.state !== "completed" || p107aFeature?.status !== "done"
        ...
        add("CDG2-090-p107a-p108-lifecycle", "anchored_planned state tuple is invalid");
      }
      ...
      if (p107aWorkstream?.state !== "completed" || p107aFeature?.status !== "done"
        ...
        add("CDG2-090-p107a-p108-lifecycle", "authorized_active state tuple is invalid");
      }
    } else {
      add("CDG2-090-p107a-p108-lifecycle", "unknown continuation lifecycle phase");
    }
  });

  run("CDG2-100-cd03-evidence-refs", () => {
    ...
    if (cd03?.state !== "completed" || !plainObject(contract)
      ...
      add("CDG2-100-cd03-evidence-refs", "CD03 completionContract reference shape is invalid");
    }
    ...
    if (!plainObject(artifact) || artifact.schemaVersion !== 1
      ...
      add("CDG2-100-cd03-evidence-refs", "accepted CD03 artifact review reference is invalid");
    }
    ...
    if (!plainObject(manifest) || manifest.schemaVersion !== 1
      ...
      add("CDG2-100-cd03-evidence-refs", "CD03 closure manifest reference shape is invalid");
    }
    ...
    if (!plainObject(anchor) || !sha256Digest(anchor.digest)
      ...
      add("CDG2-100-cd03-evidence-refs", "CD03 external anchor reference shape is invalid");
    }
    for (const manifestReceipt of receipts) {
      ...
      if (anchorReceipt?.canonicalDigest !== manifestReceipt?.canonicalDigest
        ...
        add("CDG2-100-cd03-evidence-refs", `CD03 ${String(manifestReceipt?.lane)} receipt reference is invalid`);
      }
    }
    ...
    if (!plainObject(externalAttestation)
      ...
      add("CDG2-100-cd03-evidence-refs", "CD03 external attestation reference shape is invalid");
    }
  });

  const ruleResults = PROGRAM_GOVERNANCE_V2_RULE_LEDGER.map((rule) => {
  ...
  return Object.freeze({
    schemaVersion: 2,
    kind: GOVERNANCE_V2_RESULT_KIND,
...
export function createProgramRootBindingV2(program) {
  const programRootDefinition = stableProgramRootDefinitionV2(program);
  return {
    programRootDefinition,
    programRootDefinitionDigest: hashCanonicalV2(programRootDefinition),
  };
}
