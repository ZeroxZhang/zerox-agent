...
import {
  ...
  CONTINUATION_V12_LIFECYCLE_PROFILE_PHASES,
...
import {
  PROGRAM_GOVERNANCE_V12_RULE_IDS,
  validateConversationDisclosureProgramGovernanceV12,
} from "./conversation-disclosure-program-governance-v12.mjs";
...
const CHECKER_RECEIPT_KIND = "cd03a-continuation-checker-v12-receipt";
...
export async function runConversationDisclosureContinuationCheckerV12(
  argv = process.argv.slice(2),
) {
  const options = parseOptionsV12(argv);
  ...
  const [policy, archive, rejection, snapshot, program, featureList] =
    await Promise.all([
      readControlJson(CONTINUATION_V12_POLICY_PATH, "Round12 policy", {
  ...
  for (const entry of snapshot.frozenFiles) {
    const capture = await captureControl(
      entry.path,
      `Round12 frozen subject ${entry.path}`,
      {
        privateEvidence: [
          CONTINUATION_V12_POLICY_PATH,
          CONTINUATION_V12_ROUND11_REVIEW_REJECTION_PATH,
    ...
    if (capture.digest !== entry.sha256) {
      throw new Error(`Round12 frozen subject drifted: ${entry.path}`);
    }
  }
  for (const entry of snapshot.transitionPayloadFiles) {
    const capture = await captureControl(
      entry.path,
      `Round12 transition payload ${entry.path}`,
    );
    if (capture.digest !== entry.sha256) {
      throw new Error(`Round12 transition payload drifted: ${entry.path}`);
    }
  }
  ...
  for (const entry of snapshot.baselineFiles) {
    if (POST_TRANSITION_PHASES.has(options.mode)
      ...
      await captureControl(entry.path, `Round12 mutable baseline ${entry.path}`);
      continue;
    }
    const capture = await captureControl(
      entry.path,
      `Round12 baseline ${entry.path}`,
    );
    if (capture.digest !== entry.sha256) {
      throw new Error(`Round12 baseline drifted: ${entry.path}`);
    }
  }
  ...
  for (const transition of policy.governanceTransitions) {
    const [live, staged] = await Promise.all([
      captureControl(transition.path, `Round12 transition live ${transition.path}`),
      captureControl(
        transition.stagedTargetPath,
        `Round12 transition staged ${transition.stagedTargetPath}`,
      ),
    ]);
  ...
  if (options.mode === "review_pre_transition") {
    for (const relativePath of snapshot.reviewOutputAbsentPaths) {
      if (relativePath === CONTINUATION_V12_REVIEW_SNAPSHOT_PATH) continue;
      await captureRequiredAbsentV12(
        path.join(controlRoot, relativePath),
        `Round12 pre-review output ${relativePath}`,
        { expectedRoot: controlRoot, ledger },
      );
    }
  } else {
  ...
  const lifecycleProfile = selectLifecycleProfileV12(policy, options.mode);
  if (!lifecycleProfile) {
    throw new Error(`Round12 lifecycle profile is missing: ${options.mode}`);
  }
  assertNoErrors(validateLifecycleStateV12({
    phase: options.mode,
    activeFeatureId: program.activeFeatureId,
    nextFeatureId: program.nextFeatureId,
    workstreams: program.workstreams,
    features: featureList.features,
  }, policy), "Round12 lifecycle state");
  const parentEvidence = await materializeParentEvidence(
    policy,
    baseAnchor,
    readControlJson,
  );
  const governance = validateConversationDisclosureProgramGovernanceV12({
    program,
    featureList,
    closedWorld: {
      schemaVersion: 12,
      ...
      ruleIds: [...PROGRAM_GOVERNANCE_V12_RULE_IDS],
  ...
  assertNoErrors(governance.errors, "Round12 program governance");
  ...
  return {
    ...
    digest: hashCanonicalV12(receiptWithoutDigest),
  };
}
...
async function validateContinuationEvidence({
  ...
  for (const lane of CONTINUATION_V12_REVIEW_LANES) {
    receipts.push(await readControlJson(
      ...
      `Round12 ${lane} receipt`,
      { privateEvidence: true },
    ));
  }
  const manifest = await readControlJson(
    CONTINUATION_V12_CLOSURE_MANIFEST_PATH,
    "Round12 closure manifest",
    { privateEvidence: true },
  );
  ...
  const [policyCapture, snapshotCapture, rejectionCapture, validatorCapture,
    ...
    captureControl(CONTINUATION_V12_POLICY_PATH, "Round12 manifest policy", {
      privateEvidence: true,
    }),
    captureControl(
      CONTINUATION_V12_REVIEW_SNAPSHOT_PATH,
      "Round12 manifest snapshot",
      { privateEvidence: true },
    ),
    captureControl(
      CONTINUATION_V12_ROUND11_REVIEW_REJECTION_PATH,
      "Round11 manifest rejection",
      { privateEvidence: true },
    ),
  ...
  const manifestBindings = {
    ...
    round11ReviewRejectionReference: {
  ...
  if (options.mode === "review_post_transition") {
    assertNoErrors(validateContinuationClosureManifestV12(
      manifest,
      manifestBindings,
    ), "Round12 pending closure manifest");
    ...
    await captureRequiredAbsentV12(
      path.join(controlRoot, CONTINUATION_V12_EXTERNAL_ATTESTATION_PATH),
      "Round12 pre-transaction external attestation",
      { expectedRoot: controlRoot, ledger },
    );
  ...
  if (ORDINARY_PHASES.has(options.mode)) {
    ...
    if (canonicalAnchor !== options.continuationAnchorPath
      ...
      throw new Error("Round12 continuation anchor must be canonical and external");
    }
    anchor = parseJson(
      (await capturePrivateEvidenceV12(
        canonicalAnchor,
        "Round12 continuation anchor",
        { ledger },
      )).bytes,
      "Round12 continuation anchor",
    );
  ...
  }), "Round12 final closure manifest");

  if (ORDINARY_PHASES.has(options.mode)) {
    assertNoErrors(validateContinuationExternalAnchorV12(anchor, {
    ...
    if (anchor.repositoryRealpath !== subjectRepositoryRealpath
      ...
      throw new Error("Round12 continuation anchor bindings are stale");
    }
  }
  return { receipts, manifest, attestation, anchor };
}

function buildExpectedPendingManifestV12({
  policy,
  policyReference,
  snapshotReference,
  round11ReviewRejectionReference,
  ...
  const base = {
    ...
    round11ReviewRejection: round11ReviewRejectionReference,
  ...
  return withCanonicalDigestV12({
    ...base,
    pendingManifestDigest: pendingManifestDigestV12(base),
  });
}

function parseOptionsV12(argv) {
  const options = {
    errors: [],
  };
  const map = new Map([
  ...
  if (!CONTINUATION_V12_LIFECYCLE_PROFILE_PHASES.includes(options.mode)) {
    options.errors.push("checker V12 --mode is invalid");
  }
  return options;
}
...
if (process.argv[1]
  ...
  runConversationDisclosureContinuationCheckerV12()
    .then((receipt) => {
      process.stdout.write("Conversation disclosure continuation v12 check passed.\n");
      process.stdout.write(`${JSON.stringify(receipt)}\n`);
    })
    .catch((error) => {
      process.stderr.write("Conversation disclosure continuation v12 check failed:\n");
