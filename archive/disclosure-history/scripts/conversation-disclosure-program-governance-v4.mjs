...
export const GOVERNANCE_V4_INPUT_KIND =
  "conversation-disclosure-program-governance-v4-input";
export const GOVERNANCE_V4_RESULT_KIND =
  "conversation-disclosure-program-governance-v4-result";
...
const P107A_WORKSTREAM_ID = CONTINUATION_V4_WORKSTREAM_ID;
const P107A_FEATURE_ID = CONTINUATION_V4_FEATURE_ID;
const P108_WORKSTREAM_ID = CONTINUATION_V4_SUCCESSOR_WORKSTREAM_ID;
const P108_FEATURE_ID = CONTINUATION_V4_SUCCESSOR_FEATURE_ID;
...
export const PROGRAM_GOVERNANCE_V4_RULE_LEDGER = Object.freeze([
...
export const PROGRAM_GOVERNANCE_V4_RULE_IDS = Object.freeze(
  PROGRAM_GOVERNANCE_V4_RULE_LEDGER.map((rule) => rule.id),
);

export function validateConversationDisclosureProgramGovernanceV4(input) {
  const issues = new Map(PROGRAM_GOVERNANCE_V4_RULE_IDS.map((id) => [id, []]));
  ...
  run("CDG4-001-input-contract", () => {
    ...
    if (!sameOrderedStrings(Object.keys(value).sort(), expectedKeys)) {
      add("CDG4-001-input-contract", "input keys must exactly match the v4 contract");
    }
    if (!sameOrderedStrings(Object.keys(roster).sort(), [
      ...
      "schemaVersion",
      "workstreamIds",
    ]) || roster.schemaVersion !== 4) {
      add("CDG4-001-input-contract", "unknown closed-world governance schema");
    }
    if (!sameOrderedStrings(roster.ruleIds, PROGRAM_GOVERNANCE_V4_RULE_IDS)) {
      add("CDG4-001-input-contract", "closedWorld.ruleIds must exactly match the known v4 ledger");
    }
  ...
  run("CDG4-011-program-root-contract", () => {
    if (!plainObject(roster.programRootDefinition)
      ...
      || roster.programRootDefinitionDigest
        !== hashCanonicalV4(roster.programRootDefinition)) {
      add("CDG4-011-program-root-contract", "frozen Program root binding is invalid");
      return;
    }
    const liveDefinition = stableProgramRootDefinitionV4(program);
    if (canonicalJsonV4(liveDefinition)
        !== canonicalJsonV4(roster.programRootDefinition)
      || hashCanonicalV4(liveDefinition) !== roster.programRootDefinitionDigest) {
      add(
  ...
  const ruleResults = PROGRAM_GOVERNANCE_V4_RULE_LEDGER.map((rule) => {
  ...
  return Object.freeze({
    schemaVersion: 4,
    kind: GOVERNANCE_V4_RESULT_KIND,
...
export function createProgramRootBindingV4(program) {
  const programRootDefinition = stableProgramRootDefinitionV4(program);
  return {
    programRootDefinition,
    programRootDefinitionDigest: hashCanonicalV4(programRootDefinition),
  };
}
