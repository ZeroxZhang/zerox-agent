...
export const PRIVATE_MODE_V5 = 0o600;

const LEDGER_KIND = "conversation-disclosure-runtime-capture-ledger-v5";

export class RuntimeIoV5Error extends Error {
  constructor(code, message) {
    super(message);
    this.name = "RuntimeIoV5Error";
    this.code = code;
  }
}

export function createCaptureLedgerV5() {
  return {
    kind: LEDGER_KIND,
    entries: [],
...
export function sha256BytesV5(bytes) {
...
export async function captureStableFileV5(
  absolutePath,
  label = absolutePath,
  {
    expectedRoot,
  ...
  await assertExpectedRootV5(absolutePath, expectedRoot, label, true);
  const parents = await captureParentIdentitiesV5(absolutePath, label);
  let before;
  try {
  ...
  const requiredMode = requirePrivate ? PRIVATE_MODE_V5 : expectedMode;
  ...
  try {
    ...
    await assertParentIdentitiesV5(parents, label);
    await assertExpectedRootV5(absolutePath, expectedRoot, label, true);
    const capture = {
      kind: "present",
      absolutePath,
      ...
      bytes,
      digest: sha256BytesV5(bytes),
      mode: Number(opened.mode & 0o777n),
      uid: Number(opened.uid),
    };
    recordCaptureV5(ledger, capture);
    return capture;
  } finally {
    await handle.close();
  }
...
export function capturePrivateEvidenceV5(
  absolutePath,
  label = absolutePath,
  options = {},
) {
  return captureStableFileV5(absolutePath, label, {
    ...options,
    requirePrivate: true,
  });
}

export async function captureRequiredAbsentV5(
  absolutePath,
  label = absolutePath,
  { expectedRoot, ledger } = {},
) {
  requireAbsolutePath(absolutePath, label);
  await assertExpectedRootV5(absolutePath, expectedRoot, label, false);
  const parents = await captureParentIdentitiesV5(absolutePath, label);
  try {
...
export async function postflightCaptureLedgerV5(ledger) {
  requireLedgerV5(ledger);
  for (const capture of ledger.entries) {
    await assertParentIdentitiesV5(capture.parents, capture.label);
    await assertExpectedRootV5(
    ...
    const current = await captureStableFileV5(
      capture.absolutePath,
      capture.label,
      {
...
export async function publishPrivateExactV5(
  absolutePath,
  bytes,
  ...
  try {
    ...
    const existing = await captureOptionalPrivateV5(absolutePath, label, expectedRoot);
    if (existing) {
      if (!existing.bytes.equals(bytes)) {
        throw failure("THIRD_STATE", `${label} contains different existing bytes`);
      }
      await assertOpenedDirectoryV5(directoryHandle, expectedParent, label);
      await assertParentIdentitiesV5(parents, label);
    ...
    try {
      ...
      try {
        await temporaryHandle.chmod(PRIVATE_MODE_V5);
      ...
      try {
        ...
        const raced = await capturePrivateEvidenceV5(absolutePath, label, { expectedRoot });
      ...
      await assertOpenedDirectoryV5(directoryHandle, expectedParent, label);
      await assertParentIdentitiesV5(parents, label);
      const published = await capturePrivateEvidenceV5(absolutePath, label, { expectedRoot });
...
async function captureOptionalPrivateV5(absolutePath, label, expectedRoot) {
  try {
    await lstat(absolutePath, { bigint: true });
  } catch (error) {
  ...
  return capturePrivateEvidenceV5(absolutePath, label, { expectedRoot });
}
...
function recordCaptureV5(ledger, capture) {
...
function requireLedgerV5(ledger) {
...
async function captureParentIdentitiesV5(absolutePath, label) {
...
async function assertParentIdentitiesV5(identities, label) {
...
async function assertOpenedDirectoryV5(handle, expected, label) {
...
async function assertExpectedRootV5(absolutePath, expectedRoot, label, leafExists) {
...
function failure(code, message) {
  return new RuntimeIoV5Error(`RUNTIME_IO_V5_${code}`, message);
}
