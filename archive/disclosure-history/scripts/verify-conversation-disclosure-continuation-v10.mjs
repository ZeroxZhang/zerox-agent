...
// CD03A external closure runner v10. This file is intentionally self-contained:
...
const ROUND9_REVIEW_REJECTION_PATH =
...
const ROUND9_POLICY_ROOT = Object.freeze({
  path:
...
const ROUND9_SNAPSHOT_ROOT = Object.freeze({
  path:
...
const ROUND9_RECEIPT_ROOTS = Object.freeze([
  Object.freeze({
    lane: "contract",
    path:
      ".zerox/verification/conversation-disclosure/CD03A-round9-contract-review.json",
    byteSha256:
      "sha256:2c016a0a0ee7336be0fa0d8bca59be80c0628dd08ff574ebe23355b249b6cd02",
    canonicalDigest:
      "sha256:5ce894889d0c44db5755dc28d94b7a886a304111ac407eadd5d518d3a304e3bf",
    challenge:
      "sha256:fde0959cf0eccdee4abc4fddc3826df72920222ae9bddbc0bd6d419d7f5de9a0",
    verdict: "failed",
  ...
  Object.freeze({
    ...
    byteSha256:
      "sha256:e6831b8f6837b139914c04b04f90b4a6384c1f937709180e83c56f882ddbeab2",
    canonicalDigest:
      "sha256:e07d67f1319d2c00cd4b482b2f92b2cd11511a3d93093111027e931ba52ab127",
    challenge:
      "sha256:2b913b16a55970cfbbf0d223636f14ecf93a80c89f41036dc62a0c8ee0cae8bf",
    verdict: "passed",
    findingCounts: Object.freeze({ critical: 0, major: 0, minor: 0 }),
  }),
]);
const ROUND9_FINDING_IDS = Object.freeze([
  "R9-CONTRACT-001",
  "R9-CONTRACT-002",
  "R9-CONTRACT-003",
  "R9-CONTRACT-004",
...
const ROUND9_FINDING_SET_DIGEST =
  "sha256:49875d7f1bfeaf06c5d46efde519c24b178adf0bf294c88fc77f5fb8859ba5c8";
const ROUND9_FINDING_COUNTS = Object.freeze({
  critical: 0,
  major: 5,
  minor: 0,
});
const ROUND9_FORBIDDEN_OUTPUTS = Object.freeze([
...
const JOURNAL_KIND =
  "conversation-disclosure-continuation-publication-transaction-v10";
...
const POLICY_KEYS = Object.freeze([
  ...
  "round1Rejection", "round2PrefreezeRejection", "round9ReviewRejection",
...
const SNAPSHOT_KEYS = Object.freeze([
  ...
  "round9ReviewRejectionDigest", "schemaVersion",
...
const MANIFEST_KEYS = Object.freeze([
  ...
  "policy", "programId", "reviewReceipts", "round", "round9ReviewRejection",
...
const CONTROL_SET_KIND = "conversation-disclosure-continuation-control-set-v10";
...
const RECEIPT_KEYS = Object.freeze([
  ...
  "round9ReviewRejectionDigest", "schemaVersion", "snapshotDigest",
...
const ATTESTATION_KEYS = Object.freeze([
  ...
  "repositoryRealpath", "reviewAssurance", "round9ReviewRejectionDigest",
...
const ANCHOR_KEYS = Object.freeze([
  ...
  "reviewAssurance", "round9ReviewRejectionDigest", "runnerDigest",
...
if (options.journalOutput !== `${options.externalAnchorOutput}.closure-v10.journal.json`) {
...
async function buildFreshTransaction() {
  ...
  await captureAndValidateRound9ReviewRejection(policy);
  ...
  if (!exactKeys(snapshot, SNAPSHOT_KEYS)
    ...
    || snapshot.round9ReviewRejectionDigest
      !== policy.round9ReviewRejection?.digest
  ...
  if (!exactKeys(manifest, MANIFEST_KEYS)
    ...
    || manifest.round9ReviewRejection?.path !== ROUND9_REVIEW_REJECTION_PATH
  ...
  if (transitions.length !== 4) {
    fail("runner v10 requires exactly four governance transitions");
  }
...
async function captureAndValidateRound9ReviewRejection(policy) {
  const embedded = policy.round9ReviewRejection;
  const capture = await captureRepositoryFile(
    ROUND9_REVIEW_REJECTION_PATH,
    "Round9 review-rejection witness",
    { requirePrivate: true },
  );
  const rejection = parseJson(capture.bytes, "Round9 review-rejection witness");
  validateCanonicalObject(rejection, "Round9 review-rejection witness");
  if (canonicalJson(rejection) !== canonicalJson(embedded)) {
    fail("continuation policy Round9 review-rejection binding is stale");
  }
  await validateRound9ReviewRejection(rejection, policy);
}

async function validateRound9ReviewRejection(rejection, policy) {
  const expectedKeys = [
  ...
  if (!exactKeys(rejection, expectedKeys)
    ...
    || rejection.recoveryRound !== 10
    || rejection.rejectedRound !== 9
    || rejection.status !== "rejected_after_review"
    || canonicalJson(rejection.sourcePolicy) !== canonicalJson(ROUND9_POLICY_ROOT)
    || canonicalJson(rejection.sourceSnapshot)
      !== canonicalJson({
        ...ROUND9_SNAPSHOT_ROOT,
        frozenFileCount: 258,
        transitionPayloadFileCount: 4,
        baselineFileCount: 12,
      })
    || canonicalJson(rejection.findingIds) !== canonicalJson(ROUND9_FINDING_IDS)
    || rejection.findingSetDigest !== ROUND9_FINDING_SET_DIGEST
    || canonicalJson(rejection.aggregateFindingCounts)
      !== canonicalJson(ROUND9_FINDING_COUNTS)
    || canonicalJson(rejection.repositoryForbiddenOutputs)
      !== canonicalJson(ROUND9_FORBIDDEN_OUTPUTS)
    || !Array.isArray(rejection.completedReceipts)
    || rejection.completedReceipts.length !== ROUND9_RECEIPT_ROOTS.length) {
    fail("Round9 review-rejection schema or lifecycle binding is stale");
  }
  for (const expected of [
    ROUND9_POLICY_ROOT,
    ROUND9_SNAPSHOT_ROOT,
    ...ROUND9_RECEIPT_ROOTS,
  ]) {
    const current = await captureRepositoryFile(
      expected.path,
      `Round9 rejection source ${expected.path}`,
      { requirePrivate: expected === ROUND9_POLICY_ROOT
        || expected === ROUND9_SNAPSHOT_ROOT
        || ROUND9_RECEIPT_ROOTS.includes(expected) },
    );
    if (current.digest !== expected.byteSha256) {
      fail(`Round9 rejection source bytes drifted: ${expected.path}`);
    }
    const value = parseJson(current.bytes, `Round9 rejection source ${expected.path}`);
    const canonicalDigest = value.digest ?? hashCanonical(value);
    if (canonicalDigest !== expected.canonicalDigest) {
      fail(`Round9 rejection source canonical digest drifted: ${expected.path}`);
    }
  }
  for (let index = 0; index < ROUND9_RECEIPT_ROOTS.length; index += 1) {
    const actual = rejection.completedReceipts[index];
    const expected = ROUND9_RECEIPT_ROOTS[index];
    if (actual?.lane !== expected.lane
      ...
      fail(`Round9 rejection receipt root is stale: ${expected.lane}`);
    }
  }
  for (const relativePath of rejection.repositoryForbiddenOutputs ?? []) {
    if (await repositoryEntryExists(repositoryRealpath, relativePath)) {
      fail(`Round9 forbidden repository output exists: ${relativePath}`);
    }
  }
}
...
async function convergePreparedTransaction(journal, { skipFaults }) {
  ...
  for (const [index, transition] of journal.governanceTransitions.entries()) {
    const requestedFault = skipFaults
      ? ""
      : process.env.ZEROX_CD03A_RUNNER_V10_TEST_FAULT ?? "";
    ...
    if (fault !== "none") fail(`injected runner v10 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
  ...
  for (const publication of journal.publications) {
    ...
    if (bridgeFault !== "none") fail(`injected runner v10 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
...
async function executeCandidatesInStage({ stagedTree, policy, baseAnchor, snapshot }) {
  const stageRoot = await realpath(
    await mkdtemp(path.join(os.tmpdir(), "zerox-cd03a-v10-stage-")),
  );
  ...
  try {
    ...
    for (const [kind, relativePath] of [["checker", checkerPath], ["harness", harnessPath]]) {
      ...
      const receiptKind = kind === "checker"
        ? "cd03a-continuation-checker-v10-receipt"
        : "cd03a-continuation-harness-v10-receipt";
...
async function removeValidatedStageRoot(stageRoot, controlRoot) {
  ...
  if (path.dirname(stageRealpath) !== temporaryRoot
    || !path.basename(stageRealpath).startsWith("zerox-cd03a-v10-stage-")
...
function validateJournal(journal, caller, repoRealpath, runnerDigest) {
  ...
  if (!exactKeys(journal, JOURNAL_KEYS)
    || journal.schemaVersion !== 10 || journal.kind !== JOURNAL_KIND
  ...
  if (!exactKeys(embeddedSnapshot, SNAPSHOT_KEYS)
    || embeddedSnapshot.schemaVersion !== 10
  ...
  if (!exactKeys(attestation, ATTESTATION_KEYS)
    ...
    || attestation?.round9ReviewRejectionDigest
      !== embeddedPendingManifest.round9ReviewRejection?.canonicalDigest
  ...
  if (!exactKeys(anchor, ANCHOR_KEYS)
    ...
    || anchor?.round9ReviewRejectionDigest
      !== embeddedPendingManifest.round9ReviewRejection?.canonicalDigest
...
function validateCandidateResultSet(
  results,
  checkerPath = "scripts/check-conversation-disclosure-continuation-v10.mjs",
) {
...
function validateReviewSet(receipts, snapshot, policy, verifierNow) {
  ...
  const validatorDigest = policy.continuationExecutables?.find(
    (entry) => entry.kind === "checker"
      && entry.path === "scripts/check-conversation-disclosure-continuation-v10.mjs",
  )?.sha256;
...
function parseOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    fail(`unknown runner v10 option: ${argument}`);
  }
  for (const key of [
    ...
    if (!values[key]) fail(`missing required runner v10 option: ${key}`);
  }
...
function injectFault(point) {
  if (process.env.ZEROX_CD03A_RUNNER_V10_TEST_FAULT === point) {
    fail(`injected runner v10 fault: ${point}`);
  }
}
...
function printPublicationReceipt(journal, recovered) {
  ...
  const receiptWithoutDigest = {
    kind: "cd03a-external-publication-v10-receipt",
    status: "passed",
    recovered,
...
function fail(message) {
  console.error("CD03A external continuation runner v10 failed:");
