...
try {
  ...
  const historicalRoot = await createFixture("zerox-r6-v3-history-");
  ...
  const round4Root = await createFixture("zerox-r6-v4-history-");
  await restoreRound4Semantics(round4Root);
  await runVitest(round4Root, ["--run", ...historicalV4Tests]);

  const targetRoot = await createFixture("zerox-r6-target-tests-");
  await applyRound6Targets(targetRoot);
...
async function applyRound6Targets(fixture) {
  const mappings = [
    [
