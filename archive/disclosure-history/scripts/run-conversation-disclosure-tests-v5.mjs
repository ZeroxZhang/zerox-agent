...
try {
  ...
  const historicalRoot = await createFixture("zerox-r5-v3-history-");
  await restoreRound3Semantics(historicalRoot);
  await runVitest(historicalRoot, ["--run", ...historicalTests]);
  ...
  const targetRoot = await createFixture("zerox-r5-target-tests-");
  await applyRound5Targets(targetRoot);
...
async function applyRound5Targets(fixture) {
  const mappings = [
    [
