...
function rehash<T extends Record<string, unknown>>(value: T): T & { digest: string } {
  ...
  delete copy.digest;
  copy.digest = contract.hashCanonicalV10(copy);
  return copy as T & { digest: string };
}

function makeRound9Rejection() {
  return contract.withCanonicalDigestV10({
    schemaVersion: 10,
    kind: contract.CONTINUATION_V10_REVIEW_REJECTION_KIND,
    algorithm: contract.CONTINUATION_V10_ALGORITHM,
    ...
    rejectedRound: contract.CONTINUATION_V10_REJECTED_ROUND,
    recoveryRound: contract.CONTINUATION_V10_ROUND,
    status: "rejected_after_review",
    sourcePolicy: structuredClone(
      contract.CONTINUATION_V10_ROUND9_POLICY_TRUST_ROOT,
    ),
    sourceSnapshot: structuredClone(
      contract.CONTINUATION_V10_ROUND9_SNAPSHOT_TRUST_ROOT,
    ),
    completedReceipts: contract.CONTINUATION_V10_ROUND9_RECEIPT_TRUST_ROOTS.map(
      (entry: Record<string, unknown>) => structuredClone(entry),
    ),
    findingIds: [...contract.CONTINUATION_V10_ROUND9_FINDING_IDS],
    findingSetDigest: contract.CONTINUATION_V10_ROUND9_FINDING_SET_DIGEST,
    aggregateFindingCounts: structuredClone(
      contract.CONTINUATION_V10_ROUND9_AGGREGATE_FINDING_COUNTS,
    ),
    repositoryForbiddenOutputs: [
      ...contract.CONTINUATION_V10_ROUND9_REPOSITORY_FORBIDDEN_OUTPUT_PATHS,
    ],
    externalAnchorRule: contract.expectedRejectedRound9AnchorRuleV10(),
    priorRejections: {
      round1CanonicalDigest: contract.CONTINUATION_V10_ROUND1_REJECTION_DIGEST,
      round2: structuredClone(
        contract.CONTINUATION_V10_ROUND2_REJECTION_TRUST_ROOT,
      ),
    },
  });
}
...
function makeArchive() {
  ...
  return rehash({
    ...source,
    schemaVersion: 10,
    kind: contract.CONTINUATION_V10_BASELINE_ARCHIVE_KIND,
    round: 10,
  });
}
...
function makePolicy() {
  ...
  const rejection = makeRound9Rejection();
  const archive = makeArchive();
  const admission = structuredClone(source.admission);
  const transitions = contract.CONTINUATION_V10_GOVERNANCE_TRANSITION_TRUST_ROOTS
    .map((entry: Record<string, unknown>) => structuredClone(entry));
  admission.reviewOutputPaths = [
    ...contract.CONTINUATION_V10_REVIEW_OUTPUT_PATHS,
  ];
  admission.featureDefinition.files = [...new Set([
    ...admission.featureDefinition.files,
    contract.CONTINUATION_V10_ROUND9_REVIEW_REJECTION_PATH,
    contract.CONTINUATION_V10_BASELINE_ARCHIVE_PATH,
    contract.CONTINUATION_V10_POLICY_PATH,
    ...Object.values(
      contract.CONTINUATION_V10_EXECUTABLE_PATH_BY_KIND as Record<string, string>,
    ),
    ...transitions.map((entry: any) => entry.stagedTargetPath),
    ...contract.CONTINUATION_V10_REVIEW_OUTPUT_PATHS,
    ...contract.CONTINUATION_V10_REJECTED_OUTPUT_ABSENT_PATHS,
  ])].sort();
  admission.featureDefinitionDigest = contract.hashCanonicalV10(
    admission.featureDefinition,
  );
  admission.featureFileSetDigest = contract.hashCanonicalV10(
    admission.featureDefinition.files,
  );
  const admissionCoverage = contract.buildAdmissionCoverageV10(
    admission,
    transitions,
  );
  admission.reviewCoverageDigest = contract.hashCanonicalV10(admissionCoverage);
  const continuationExecutables = contract.CONTINUATION_V10_EXECUTABLE_KINDS.map(
    (kind: string, index: number) => ({
      kind,
      path: contract.CONTINUATION_V10_EXECUTABLE_PATH_BY_KIND[kind],
  ...
  return contract.withCanonicalDigestV10({
    schemaVersion: 10,
    kind: contract.CONTINUATION_V10_POLICY_KIND,
    algorithm: contract.CONTINUATION_V10_ALGORITHM,
    policyId: contract.CONTINUATION_V10_POLICY_ID,
    ...
    admission,
    admissionClassSet: [...contract.CONTINUATION_V10_ADMISSION_CLASSES],
    admissionClassSetDigest:
      contract.CONTINUATION_V10_ADMISSION_CLASS_SET_DIGEST,
    admissionCoverage,
    ...
    baselineArchive: {
      path: contract.CONTINUATION_V10_BASELINE_ARCHIVE_PATH,
      digest: archive.digest,
      entrySetDigest: archive.entrySetDigest,
    },
    reviewSnapshot: { path: contract.CONTINUATION_V10_REVIEW_SNAPSHOT_PATH },
    reviewAssurancePolicy: {
      callerDispatchAssurance:
        contract.CONTINUATION_V10_CALLER_DISPATCH_ASSURANCE,
      identityAssurance: contract.CONTINUATION_V10_IDENTITY_ASSURANCE,
      independenceClaim: contract.CONTINUATION_V10_INDEPENDENCE_CLAIM,
      localIdentityProof: false,
    },
    externalAnchorPolicy: contract.expectedRejectedRound9AnchorRuleV10(),
...
function makeSnapshot(policy: any) {
  ...
  for (const reference of [
    contract.CONTINUATION_V10_ROUND9_POLICY_TRUST_ROOT,
    contract.CONTINUATION_V10_ROUND9_SNAPSHOT_TRUST_ROOT,
    ...contract.CONTINUATION_V10_ROUND9_RECEIPT_TRUST_ROOTS,
  ]) {
    frozen.get(reference.path)!.sha256 = reference.byteSha256;
  }
  frozen.get(
    contract.CONTINUATION_V10_ROUND9_REVIEW_REJECTION_PATH,
  )!.sha256 = contract.sha256BytesV10(
    contract.serializeRound9ReviewRejectionV10(
      policy.round9ReviewRejection,
    ),
  );
  ...
  return contract.withCanonicalDigestV10({
    schemaVersion: 10,
    kind: contract.CONTINUATION_V10_SNAPSHOT_KIND,
    algorithm: contract.CONTINUATION_V10_ALGORITHM,
    ...
    pathAuthorityDigest: contract.hashCanonicalV10(policy.pathAuthorities),
...
function makeReceipt(policy: any, snapshot: any) {
  const dispatchEntry = {
    lane: "contract",
    assurance: contract.CONTINUATION_V10_CALLER_DISPATCH_ASSURANCE,
  ...
  return {
    schemaVersion: 10,
    kind: contract.CONTINUATION_V10_RECEIPT_KIND,
    programId: policy.programId,
    workstreamId: contract.CONTINUATION_V10_WORKSTREAM_ID,
    featureId: contract.CONTINUATION_V10_FEATURE_ID,
    round: contract.CONTINUATION_V10_ROUND,
    ...
    callerDispatchEntryDigest: contract.hashCanonicalV10(dispatchEntry),
    callerDispatchSetDigest: contract.hashCanonicalV10(dispatchSet),
    claimedReviewOrigin: {
      taskPath: "caller/contract",
      agentLabel: "claimed-reviewer",
      transport: "codex-collaboration",
...
describe("conversation disclosure continuation contract v10", () => {
  test("binds the real runtime I/O API instead of the abandoned adapter", () => {
    expect(contract.validateRuntimeIoCaptureAdapterV10(runtimeIo)).toEqual([]);
    expect(contract.CONTINUATION_V10_RUNTIME_IO_INTERFACE.methods).not.toContain(
      "capturePresent",
    );
  });

  test("validates the exact Round9 rejection and both roots of every source", () => {
    const witness = makeRound9Rejection();
    expect(contract.validateRound9ReviewRejectionV10(witness)).toEqual([]);
    const references = [
    ...
    for (const reference of references) {
      ...
      expect(contract.validateRound9ReviewRejectionV10(rehash(mutant)))
        .not.toEqual([]);
    }
  });
  ...
  test("hard-roots the exact ordered six-class set", () => {
    ...
    expect(contract.validateContinuationPolicyV10(policy)).toEqual([]);
    for (const classSet of [
      ...
      mutant.admissionClassSetDigest = contract.hashCanonicalV10(classSet);
      expect(contract.validateContinuationPolicyV10(rehash(mutant)))
  ...
  test("rejects policy key omission and extension", () => {
    ...
    expect(contract.validateContinuationPolicyV10(omitted))
      .toContain("continuation policy must contain the exact V10 keys");
    const extended = { ...policy, candidateField: true };
    expect(contract.validateContinuationPolicyV10(extended))
      .toContain("continuation policy must contain the exact V10 keys");
  });
  ...
  test("binds policy programId to the closed-world Program root", () => {
    ...
    expect(contract.validateContinuationPolicyV10(mutant, {
  ...
  test("validates the V10 archive and snapshot bindings", () => {
    const policy = makePolicy();
    const archive = makeArchive();
    const snapshot = makeSnapshot(policy);
    expect(contract.validateBaselineArchiveV10(archive, policy)).toEqual([]);
    expect(contract.validateContinuationReviewSnapshotV10(snapshot, policy, {
      verifierNow: Date.parse(snapshot.frozenAt),
    })).toEqual([]);
    const mutant = rehash({
      ...snapshot,
      round9ReviewRejectionDigest: zeroDigest,
    });
    expect(contract.validateContinuationReviewSnapshotV10(mutant, policy, {
      verifierNow: Date.parse(snapshot.frozenAt),
    })).toContain(
      "continuation review snapshot round9ReviewRejectionDigest binding is stale",
    );
  });
  ...
  test("rejects stronger identity assurance in policy and receipts", () => {
    ...
    expect(contract.validateContinuationPolicyV10(rehash(policyMutant)))
      .toContain(
    ...
    expect(contract.validateContinuationReviewReceiptV10(
      receipt,
      snapshot,
      policy,
      {
    ...
    expect(contract.validateContinuationReviewReceiptV10(
      ...
      "continuation review receipt V10 identity/assurance is invalid",
    );
  });
  ...
  test("uses canonical deterministic gzip archive entries", () => {
    const bytes = Buffer.from("round10 baseline\n", "utf8");
    const entry = {
      path: "package.json",
      source: "governance_transition",
      sha256: contract.sha256BytesV10(bytes),
      encoding: "gzip-base64-v1",
      bytes: gzipSync(bytes, { level: 9 } as any).toString("base64"),
    };
    const archive = contract.withCanonicalDigestV10({
      schemaVersion: 10,
      kind: contract.CONTINUATION_V10_BASELINE_ARCHIVE_KIND,
      algorithm: contract.CONTINUATION_V10_ALGORITHM,
      programId: "conversation-progressive-disclosure-v3.9.2-2026-08",
      ...
      round: contract.CONTINUATION_V10_ROUND,
      entries: [entry],
      entrySetDigest: contract.hashCanonicalV10([entry]),
    });
    expect(contract.validateBaselineArchiveV10(archive)).toEqual([]);
    const mutant = structuredClone(archive);
    mutant.entries[0].bytes = gzipSync(bytes, { level: 1 } as any)
      .toString("base64");
    expect(contract.validateBaselineArchiveV10(rehash(mutant)).join("\n"))
