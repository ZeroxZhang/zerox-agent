...
const checker = await import(
  ...
  "../../scripts/check-conversation-disclosure-continuation-v8.mjs"
);
...
describe("conversation disclosure continuation checker v8", () => {
  test("rejects an unpinned or unknown mode before reading evidence", async () => {
    await expect(
      checker.runConversationDisclosureContinuationCheckerV8([]),
    ).rejects.toThrow("checker option is required");
    await expect(
      checker.runConversationDisclosureContinuationCheckerV8([
        "--mode", "candidate_self_authorized",
      ]),
    ).rejects.toThrow("checker V8 --mode is invalid");
  });
