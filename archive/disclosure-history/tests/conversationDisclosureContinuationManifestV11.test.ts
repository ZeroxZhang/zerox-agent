...
describe("conversation disclosure continuation manifest v11", () => {
  test("requires external dispatch, policy, and snapshot caller pins", async () => {
    await expect(
      manifestBuilder.buildConversationDisclosureContinuationManifestV11({
  ...
  test("derives one pending digest across pending and final forms", () => {
    ...
    const pendingDigest = contract.pendingManifestDigestV11(base);
    const final = {
      ...base,
    ...
    expect(contract.pendingManifestDigestV11(final)).toBe(pendingDigest);
  });
  ...
  test("captures the private external dispatch set and postflights before publish", () => {
    ...
    expect(source).toContain("validateContinuationReviewSetV11(");
