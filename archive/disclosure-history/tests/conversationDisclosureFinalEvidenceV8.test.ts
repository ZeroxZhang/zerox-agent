...
function buildFixture() {
  const policy = {
    ...
    round7ReviewRejection: { digest: digest("3") },
    successor: {
    ...
    continuationExecutables: [{
      kind: "checker",
      path: "scripts/check-conversation-disclosure-continuation-v8.mjs",
      sha256: digest("6"),
    }],
  };
  const snapshot = {
  ...
  const dispatchSet = ["contract", "runtime", "governance"].map(
    (lane, index) => ({
      ...
      taskPath: `round8/${lane}`,
      agentLabel: `round8-${lane}-reviewer`,
      transport: "codex-collaboration",
    }),
  );
  const policyReference = {
    path: contract.CONTINUATION_V8_POLICY_PATH,
    byteSha256: digest("a"),
    canonicalDigest: policy.digest,
  };
  const snapshotReference = {
    path: contract.CONTINUATION_V8_REVIEW_SNAPSHOT_PATH,
    byteSha256: digest("b"),
    canonicalDigest: snapshot.digest,
  };
  const rejectionReference = {
    path: contract.CONTINUATION_V8_ROUND7_REVIEW_REJECTION_PATH,
    byteSha256: digest("c"),
  ...
  const runnerReference = {
    path: "scripts/verify-conversation-disclosure-continuation-v8.mjs",
    sha256: digest("d"),
  };
  const pendingBase = {
    schemaVersion: 8,
    kind: contract.CONTINUATION_V8_MANIFEST_KIND,
    ...
    round7ReviewRejection: rejectionReference,
    reviewReceipts: reviewReferences,
    callerDispatchSet: dispatchSet,
    validator: validatorReference,
    externalRunner: runnerReference,
    externalAttestation: {
      path: contract.CONTINUATION_V8_EXTERNAL_ATTESTATION_PATH,
      canonicalDigest: null,
    },
  };
  const pendingManifestDigest =
    contract.pendingManifestDigestV8(pendingBase);
  const pendingManifest = contract.withCanonicalDigestV8({
    ...pendingBase,
  ...
  const attestation = contract.withCanonicalDigestV8({
    schemaVersion: 8,
    kind: contract.CONTINUATION_V8_ATTESTATION_KIND,
    status: "passed",
    governancePhase: "review_post_transition",
    identityAssurance: contract.CONTINUATION_V8_IDENTITY_ASSURANCE,
    reviewAssurance: contract.CONTINUATION_V8_CALLER_DISPATCH_ASSURANCE,
    repositoryRealpath: "/tmp/round8-subject",
  ...
  const finalManifest = contract.withCanonicalDigestV8(
    contract.finalManifestProjectionV8(pendingManifest, attestation.digest),
  );
  const anchor = contract.withCanonicalDigestV8({
    schemaVersion: 8,
    kind: contract.CONTINUATION_V8_ANCHOR_KIND,
    identityAssurance: contract.CONTINUATION_V8_IDENTITY_ASSURANCE,
    reviewAssurance: contract.CONTINUATION_V8_CALLER_DISPATCH_ASSURANCE,
    repositoryRealpath: "/tmp/round8-subject",
  ...
  const manifestBindings = {
    ...
    policyReference,
    snapshotReference,
    round7ReviewRejectionReference: rejectionReference,
    reviewReferences,
    callerDispatchSet: dispatchSet,
    validatorReference,
    runnerReference,
...
function rehash<T extends Record<string, any>>(
  ...
  delete clone.digest;
  clone.digest = contract.hashCanonicalV8(clone);
  return clone as T & { digest: string };
}

describe("conversation disclosure final evidence v8", () => {
  test("accepts one completely bound pending/final/attestation/anchor chain", () => {
    const fixture = buildFixture();
    expect(contract.validateContinuationClosureManifestV8(
      fixture.pendingManifest,
      fixture.manifestBindings,
    )).toEqual([]);
    expect(contract.validateContinuationExternalAttestationV8(
      fixture.attestation,
    ...
    expect(contract.validateContinuationExternalAnchorV8(
      fixture.anchor,
  ...
  test("rejects every stale attestation semantic root", () => {
    ...
    dispatch[0].taskPath = "round8/other";
    const attestation = rehash({
      ...fixture.attestation,
    ...
    for (const mutation of [
      ...
      },
      {
        round7ReviewRejection: {
          ...fixture.finalManifest.round7ReviewRejection,
          canonicalDigest: digest("0"),
        },
      },
      ...
      expect(contract.validateContinuationClosureManifestV8(manifest, {
        ...fixture.manifestBindings,
        pendingManifest: fixture.pendingManifest,
        externalAttestation: fixture.attestation,
      })).not.toEqual([]);
  ...
  test("rejects stale anchor roots, future time, and missing bindings", () => {
    ...
    for (const key of [
      ...
      expect(contract.validateContinuationExternalAnchorV8(rootMutant, {
        ...fixture.anchorBindings,
    ...
    expect(contract.validateContinuationExternalAnchorV8(dispatchMutant, {
      ...fixture.anchorBindings,
    ...
    expect(contract.validateContinuationExternalAnchorV8(future, {
      ...fixture.anchorBindings,
