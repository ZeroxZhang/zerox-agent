...
const governance = await import(
  ...
  "../../scripts/conversation-disclosure-program-governance-v12.mjs"
);
...
describe("conversation disclosure program governance v12", () => {
  it("accepts the closed-world P107A admission lifecycle and reports every rule", () => {
    ...
    expect(result.kind).toBe(governance.GOVERNANCE_V12_RESULT_KIND);
    expect(result.errors).toEqual([]);
    expect(result.ruleResults.map((rule: RuleResult) => rule.id)).toEqual(
      governance.PROGRAM_GOVERNANCE_V12_RULE_IDS,
    );
  ...
  it("accepts only the exact all-from or all-to governance target state", () => {
    ...
    expect(contract.validateGovernanceTransitionStateV12(
      transitions,
  ...
  it("keeps the Round12 harness diagnostic-only by default and identity claims not-signed", () => {
    ...
    expect(contract.CONTINUATION_V12_CALLER_DISPATCH_ASSURANCE).toBe(
      "caller-attested-not-signed",
    );
    expect(contract.CONTINUATION_V12_IDENTITY_ASSURANCE).toBe("not-signed");
    expect(contract.CONTINUATION_V12_INDEPENDENCE_CLAIM).toBe(
      "caller-attested-distinct-review-contexts",
    );
  });

  it("changes the Round11 package target only for V12 tests and authoritative program checking", () => {
  ...
  it("keeps pre-freeze target tests on the published Round11 baseline archive", () => {
    const target = readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round12-program-test.target.ts",
    ), "utf8");
    expect(target).toContain("CD03A-round11-baseline-archive.json");
    expect(target).not.toContain("CD03A-round12-baseline-archive.json");
  });
  ...
  it("rejects an unknown required rule id", () => {
    ...
    expect(ruleResult(validate(input), "CDG4-001-input-contract").message).toContain(
      "closedWorld.ruleIds must exactly match the known v12 ledger",
    );
  });
  ...
  it("rejects unsatisfiable or incomplete P107A completion artifacts", () => {
    ...
    p107a.completionArtifacts = [
      ...governance.P107A_V12_COMPLETION_ARTIFACTS,
    ...
    expect(result.message).toContain("exact satisfiable Round12 closure set");
  ...
  it("runs the authoritative completion checker in the current review state", async () => {
    await expect(
      completionChecker.checkConversationDisclosureProgramV12({
        repositoryRoot: process.cwd(),
      }),
    ).resolves.toMatchObject({
      status: "passed",
      phase: "review",
      completionArtifactCount: 88,
    });
...
function validate(input: GovernanceInput) {
  return governance.validateConversationDisclosureProgramGovernanceV12(input);
}
...
function createInput() {
  ...
  const workstreams = [
    ...
    createWorkstream("CD08", "P112-v12.9.2-disclosure-hardening", "planned", ["CD07"], ["D1"]),
    createWorkstream("CD09", "P113-v12.9.2-disclosure-adversarial-acceptance", "planned", ["CD08"], ["D1"]),
  ];
  ...
  const program = {
      schemaVersion: 1,
      programId: "conversation-progressive-disclosure-v12.9.2-2026-08",
      status: "active",
      maxActiveFeatures: 1,
  ...
  const programRootBinding = governance.createProgramRootBindingV12(program);
  return {
    program,
    ...
    closedWorld: {
      schemaVersion: 12,
      ...programRootBinding,
      ruleIds: [...governance.PROGRAM_GOVERNANCE_V12_RULE_IDS],
...
function createTransitions() {
  return Object.entries(
    contract.CONTINUATION_V12_GOVERNANCE_TRANSITIONS as Record<string, string>,
  )
    ...
    .map(([livePath, kind], index) => ({
      ...
      kind,
      stagedTargetPath:
        `.zerox/verification/conversation-disclosure/CD03A-round12-target-${index}`,
...
type GovernanceResult = ReturnType<typeof governance.validateConversationDisclosureProgramGovernanceV12>;
