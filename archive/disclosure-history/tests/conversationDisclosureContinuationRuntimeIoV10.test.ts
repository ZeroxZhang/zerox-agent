...
describe("conversation disclosure continuation runtime I/O v10", () => {
  it("captures one stable file and validates the complete ledger postflight", async () => {
    ...
    const ledger = runtimeIo.createCaptureLedgerV10();

    const capture = await runtimeIo.captureStableFileV10(filePath, "stable input", {
      expectedRoot: root,
      ledger,
    });

    expect(capture.bytes.equals(bytes)).toBe(true);
    expect(capture.digest).toBe(runtimeIo.sha256BytesV10(bytes));
    expect(capture.identity.nlink).toBe(1n);
    expect(ledger.entries).toHaveLength(1);
    expect(await runtimeIo.postflightCaptureLedgerV10(ledger)).toEqual({
      status: "passed",
      captureCount: 1,
    });
  });
  ...
  it("requires private evidence to be an owned 0600 single-link file", async () => {
    ...
    const captured = await runtimeIo.capturePrivateEvidenceV10(
      evidencePath,
      "private evidence",
      { expectedRoot: root },
    );
    ...
    await expect(runtimeIo.capturePrivateEvidenceV10(
      evidencePath,
      "private evidence",
      { expectedRoot: root },
    )).rejects.toMatchObject({ code: "RUNTIME_IO_V10_MODE" });
  });
  ...
  it("throws for every present required-absent leaf type", async () => {
    ...
    for (const presentPath of [filePath, directoryPath, symlinkPath]) {
      await expect(runtimeIo.captureRequiredAbsentV10(
        presentPath,
        "required absent output",
        { expectedRoot: root },
      )).rejects.toMatchObject({ code: "RUNTIME_IO_V10_EXPECTED_ABSENT" });
    }
  });
  ...
  it("records required absence and rejects a postflight appearance", async () => {
    ...
    await runtimeIo.captureRequiredAbsentV10(outputPath, "future output", {
      expectedRoot: root,
      ledger,
    });
    expect(ledger.entries).toHaveLength(1);
    ...
    await expect(runtimeIo.postflightCaptureLedgerV10(ledger))
      .rejects.toMatchObject({ code: "RUNTIME_IO_V10_APPEARED" });
  });
  ...
  it("rejects postflight mode drift", async () => {
    ...
    const ledger = runtimeIo.createCaptureLedgerV10();
    await runtimeIo.captureStableFileV10(filePath, "mode input", {
      expectedRoot: root,
      ledger,
    });
    ...
    await expect(runtimeIo.postflightCaptureLedgerV10(ledger))
      .rejects.toMatchObject({ code: "RUNTIME_IO_V10_CAPTURE_DRIFT" });
  });
  ...
  it("rejects postflight inode replacement even when bytes are unchanged", async () => {
    ...
    await writeFile(filePath, bytes, { mode: 0o644 });
    const ledger = runtimeIo.createCaptureLedgerV10();
    await runtimeIo.captureStableFileV10(filePath, "inode input", {
      expectedRoot: root,
      ledger,
    });
  ...
  it("rejects postflight hard-link drift", async () => {
    ...
    const ledger = runtimeIo.createCaptureLedgerV10();
    await runtimeIo.captureStableFileV10(filePath, "linked input", {
      expectedRoot: root,
      ledger,
    });
    ...
    await expect(runtimeIo.postflightCaptureLedgerV10(ledger))
      .rejects.toMatchObject({ code: "RUNTIME_IO_V10_INVALID_LEAF" });
  });
  ...
  it("rejects postflight parent identity replacement", async () => {
    ...
    const ledger = runtimeIo.createCaptureLedgerV10();
    await runtimeIo.captureStableFileV10(filePath, "parent-bound input", {
      expectedRoot: root,
      ledger,
    });
    ...
    await writeFile(filePath, "authority\n", { mode: 0o644 });
    await expect(runtimeIo.postflightCaptureLedgerV10(ledger))
      .rejects.toMatchObject({ code: "RUNTIME_IO_V10_PARENT_DRIFT" });
  });
  ...
  it("publishes exact private bytes once and reruns idempotently", async () => {
    ...
    const first = await runtimeIo.publishPrivateExactV10(outputPath, bytes, {
      expectedRoot: root,
      label: "runtime receipt",
    });
    const firstStat = await stat(outputPath);
    const second = await runtimeIo.publishPrivateExactV10(outputPath, bytes, {
      expectedRoot: root,
      label: "runtime receipt",
    });
    ...
    expect(first.digest).toBe(runtimeIo.sha256BytesV10(bytes));
  ...
  it("recovers deterministic private publication temps and linked crash pairs", async () => {
    ...
    for (const state of ["temp_only", "linked_pair"]) {
      ...
      const temporaryPath = path.join(
        root,
        `.${basename}.${runtimeIo.sha256BytesV10(bytes).slice(7, 31)}.tmp`,
      );
      await writeFile(temporaryPath, bytes, { mode: 0o600 });
      if (state === "linked_pair") await link(temporaryPath, outputPath);

      const result = await runtimeIo.publishPrivateExactV10(outputPath, bytes, {
        expectedRoot: root,
        label: state,
      });
  ...
  it("rejects partial, different, non-private, and symlink publication states", async () => {
    ...
    for (const existingPath of [partialPath, differentPath]) {
      await expect(runtimeIo.publishPrivateExactV10(existingPath, expected, {
        expectedRoot: root,
      })).rejects.toMatchObject({ code: "RUNTIME_IO_V10_THIRD_STATE" });
    }
    await expect(runtimeIo.publishPrivateExactV10(publicPath, expected, {
      expectedRoot: root,
    })).rejects.toMatchObject({ code: "RUNTIME_IO_V10_MODE" });
    await expect(runtimeIo.publishPrivateExactV10(symlinkPath, expected, {
      expectedRoot: root,
    })).rejects.toMatchObject({ code: "RUNTIME_IO_V10_ROOT_ESCAPE" });
  });
});

async function createRoot() {
  const created = await mkdtemp(path.join(os.tmpdir(), "runtime-io-v10-"));
