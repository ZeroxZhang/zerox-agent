...
describe("conversation disclosure continuation manifest v7", () => {
  test("requires external dispatch, policy, and snapshot caller pins", async () => {
    await expect(
      manifestBuilder.buildConversationDisclosureContinuationManifestV7({
  ...
  test("derives one pending digest across pending and final forms", () => {
    ...
    const pendingDigest = contract.pendingManifestDigestV7(base);
    const final = {
      ...base,
    ...
    expect(contract.pendingManifestDigestV7(final)).toBe(pendingDigest);
  });
