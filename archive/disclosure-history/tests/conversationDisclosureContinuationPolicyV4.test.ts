...
function makeArchive() {
  ...
  delete value.digest;
  value.digest = contract.hashCanonicalV4(value);
  return value;
}
...
function makePolicyInput(rejection: any) {
  ...
  const feature = featureList.features.find(
    (entry: any) => entry.id === contract.CONTINUATION_V4_FEATURE_ID,
  );
  feature.files = [...new Set([
    ...feature.files,
    ...policyBuilder.CONTINUATION_POLICY_V4_REQUIRED_ROSTER_PATHS,
  ])];
  const continuationExecutables =
    contract.CONTINUATION_V4_EXECUTABLE_KINDS.map(
...
describe("conversation disclosure continuation policy v4 builder", () => {
  test("builds one exact six-class policy from stable live definitions", async () => {
    const rejection =
      (await rejectionBuilder.buildConversationDisclosureReviewRejectionV4({
        repositoryRoot: root,
      })).witness;
    const input = makePolicyInput(rejection);
    const policy =
      policyBuilder.createConversationDisclosureContinuationPolicyV4(input);

    expect(contract.validateContinuationPolicyV4(policy, {
      expectedDigest: policy.digest,
      baselineArchive: input.baselineArchive,
    })).toEqual([]);
    expect(policy.admissionClassSet).toEqual(
      contract.CONTINUATION_V4_ADMISSION_CLASSES,
    );
    expect(new Set(policy.admissionCoverage.map((entry: any) => entry.class)))
      .toEqual(new Set(contract.CONTINUATION_V4_ADMISSION_CLASSES));
    expect(policy.round3ReviewRejection.digest).toBe(rejection.digest);
  ...
  test("rejects an incomplete finalized roster before policy construction", async () => {
    ...
    feature.files = feature.files.filter(
      (entry: string) => entry !== contract.CONTINUATION_V4_POLICY_PATH,
    );

    expect(() =>
      policyBuilder.createConversationDisclosureContinuationPolicyV4(input))
      .toThrow(`P107A V4 roster misses required path: ${
  ...
  test("rebinds exactly six present and three absent bookkeeping paths", async () => {
    ...
    const absent: string[] = [];
    const rebound = await policyBuilder.rebindRound4BookkeepingBaselinesV4(
      source,
      {
    ...
    expect(present.sort()).toEqual([
      ...policyBuilder.CONTINUATION_POLICY_V4_PRESENT_BOOKKEEPING_PATHS,
    ].sort());
    expect(absent.sort()).toEqual([
      ...policyBuilder.CONTINUATION_POLICY_V4_ABSENT_BOOKKEEPING_PATHS,
    ].sort());
  ...
  test("publishes the deterministic rejection witness privately and idempotently", async () => {
    const fixture = await createRound3Fixture();
    const outputPath =
  ...
  test("rejects Round3 receipt byte drift before creating a witness", async () => {
    const fixture = await createRound3Fixture();
    const receiptPath = path.join(
      fixture,
      contract.CONTINUATION_V4_ROUND3_RECEIPT_TRUST_ROOTS[0].path,
    );
    await appendFile(receiptPath, "\n");

    await expect(
      rejectionBuilder.buildConversationDisclosureReviewRejectionV4({
        repositoryRoot: fixture,
      }),
...
async function createRound3Fixture() {
  ...
  const required = [
    contract.CONTINUATION_V4_ROUND3_POLICY_TRUST_ROOT.path,
    contract.CONTINUATION_V4_ROUND3_SNAPSHOT_TRUST_ROOT.path,
    ...contract.CONTINUATION_V4_ROUND3_RECEIPT_TRUST_ROOTS.map(
