...
const runnerPath = path.join(
  process.cwd(),
  "scripts/verify-conversation-disclosure-continuation-v7.mjs",
);
const source = readFileSync(runnerPath, "utf8");

describe("conversation disclosure continuation runner v7", () => {
  test("is self-contained and rejects unpinned execution", async () => {
    ...
    await expect(execFileAsync(process.execPath, [runnerPath], {
      ...
      stderr: expect.stringContaining("missing required runner v7 option"),
    });
  });
  ...
  test("rejects preload state and binds V7 assurance fields", () => {
    expect(source).toContain("rejectPreloadEnvironment()");
    expect(source).toContain("ZEROX_CD03A_RUNNER_V7_TEST_FAULT");
  ...
  test("hard-roots all six classes and the rejected Round6 subject", () => {
    expect(source).toContain('"rejected_output_absent"');
    expect(source).toContain("ROUND6_POLICY_ROOT");
    expect(source).toContain("ROUND6_SNAPSHOT_ROOT");
    expect(source).toContain("ROUND6_RECEIPT_ROOTS");
    expect(source).toContain("ROUND6_FINDING_SET_DIGEST");
  });
});
