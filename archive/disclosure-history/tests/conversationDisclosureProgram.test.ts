...
async function restoreHistoricalHarnessFromArchive(
  ...
  const archive = await readJsonFile(path.join(
    ...
    "verification",
    "conversation-disclosure",
    "CD03A-round11-baseline-archive.json",
  ));
  ...
  if (!entry || entry.source !== "governance_transition"
    ...
    throw new Error("Round12 archive lacks the historical harness baseline");
  }
  const historicalBytes = gunzipSync(Buffer.from(entry.bytes, "base64"));
  if (hashText(historicalBytes) !== entry.sha256) {
    throw new Error("Round12 historical harness archive hash is stale");
  }
...
async function writeCompletedCd03ReviewState(
  ...
  // The live repository program has since advanced past CD03 (v3.9.2 closed
  // the successor program as "completed"). This dry-run simulates the CD03
  // closure moment, which is mid-flight for the overall program: downstream
  // workstreams were just reset to planned above, so the simulated program
  // must read as active regardless of the live file's current status.
