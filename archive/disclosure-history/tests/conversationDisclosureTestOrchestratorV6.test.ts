...
describe("conversation disclosure test orchestrator v6", () => {
  ...
  test("executes Program and package tests against all four Round6 targets", () => {
    expect(source).toContain("applyRound6Targets");
    expect(source).toContain("CD03A-round6-package.target.json");
    expect(source).toContain("CD03A-round6-harness.target.mjs");
    expect(source).toContain("CD03A-round6-program-test.target.ts");
    expect(source).toContain("CD03A-round6-package-scripts-test.target.ts");
  });
});
