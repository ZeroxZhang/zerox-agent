...
describe("conversation disclosure continuation checker v4", () => {
  test("rejects an unpinned or unknown mode before reading evidence", async () => {
    await expect(
      checker.runConversationDisclosureContinuationCheckerV4([]),
    ).rejects.toThrow("checker option is required");
    await expect(
      checker.runConversationDisclosureContinuationCheckerV4([
        "--mode", "candidate_self_authorized",
      ]),
    ).rejects.toThrow("checker V4 --mode is invalid");
  });
