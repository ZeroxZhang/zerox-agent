...
const freezer = await import(
  ...
  "../../scripts/freeze-conversation-disclosure-continuation-v11.mjs"
);
...
describe("conversation disclosure continuation freezer v11", () => {
  test("requires a caller-fixed canonical freeze time", async () => {
    await expect(freezer.freezeConversationDisclosureContinuationV11({
  ...
  test("captures every admission category in one postflight ledger", () => {
    ...
    expect(source).toContain("capturePrivateEvidenceV11");
    expect(source).toContain("captureRequiredAbsentV11");
    expect(source).toContain("postflightCaptureLedgerV11(ledger)");
    expect(source.indexOf("postflightCaptureLedgerV11(ledger)"))
      .toBeLessThan(source.indexOf("publishPrivateExactV11(outputAbsolute"));
  });

  test("uses the shared transition and snapshot validators", () => {
    expect(source).toContain("validateGovernanceTransitionStateV11(");
    expect(source).toContain("validateContinuationReviewSnapshotV11(");
    expect(source).toContain(
      "output must be the exact Round11 review snapshot path",
    );
  });
});
