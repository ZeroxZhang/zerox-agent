...
const round2PrefreezeRejectionPath =
...
const runnerRepoPath = "scripts/verify-conversation-disclosure-continuation-v3.mjs";
...
describe("CD03A external continuation runner v3", () => {
  ...
  it("forward-recovers a crash after attestation publication", async () => {
    ...
    await expect(runRunner(fixture, { fault: "after-attestation" }))
      .rejects.toContain("injected runner v3 fault: after-attestation");
  ...
  it("fails closed on third-state live bytes during recovery", async () => {
    ...
    await expect(runRunner(fixture, { fault: "after-transition-1" }))
      .rejects.toContain("injected runner v3 fault: after-transition-1");
  ...
  it("rejects a transition payload that is misclassified as a frozen file", async () => {
    const fixture = await createFixture({ overlapTransitionPayload: true });

    await expect(runRunner(fixture)).rejects.toContain(
      "snapshot transition payload entry is invalid or overlaps another category",
    );
    await expect(pathExists(fixture.journal)).resolves.toBe(false);
    await expect(readLiveDigests(fixture)).resolves.toEqual(fixture.originalDigests);
  });

  it("rejects Round2 pre-freeze rejection drift before journal preparation", async () => {
    const fixture = await createFixture();
    const rejectionFile = path.join(fixture.repo, round2PrefreezeRejectionPath);
    const rejection = JSON.parse(await readFile(rejectionFile, "utf8")) as any;
    rejection.status = "passed";
    await writeJson(rejectionFile, resign(rejection), 0o600);

    await expect(runRunner(fixture)).rejects.toContain(
      "continuation policy Round2 pre-freeze rejection binding is stale",
    );
    await expect(pathExists(fixture.journal)).resolves.toBe(false);
    await expect(readLiveDigests(fixture)).resolves.toEqual(fixture.originalDigests);
  });
  ...
  it("rejects non-transaction control drift after the prepared journal is durable", async () => {
    ...
    await expect(runRunner(fixture, { fault: "after-journal" }))
      .rejects.toContain("injected runner v3 fault: after-journal");
    await writeFile(
  ...
  it("forward-recovers a crash after target rename but before directory fsync", async () => {
    ...
    await expect(runRunner(fixture, { fault: "commit-transition-2" }))
      .rejects.toContain("injected runner v3 fault: commit-transition-2");
    await expect(pathExists(fixture.journal)).resolves.toBe(true);
...
async function createFixture(options: {
  ...
  overlapTransitionPayload?: boolean;
  preplantUnknownReviewOutput?: boolean;
} = {}): Promise<Fixture> {
  const createdRoot = await mkdtemp(path.join(os.tmpdir(), "cd03a-runner-v3-test-"));
  ...
  const externalRunner = path.join(externalRoot, "runner-v3.mjs");
  ...
  const checkerSource = options.checkerFails
    ...
    : candidateScript("cd03a-continuation-checker-v3-receipt");
  await writeRepoFile(repo, checkerPath, checkerSource);

  const transitionSources = [
    { path: "package.json", stagedTargetPath: ".zerox/verification/conversation-disclosure/CD03A-round3-package.target.json", kind: "package-structure-migration", original: "{\"name\":\"before\"}\n", target: "{\"name\":\"after\"}\n" },
    { path: "scripts/check-harness-state.mjs", stagedTargetPath: ".zerox/verification/conversation-disclosure/CD03A-round3-harness.target.mjs", kind: "harness-delegation-migration", original: "process.exit(91);\n", target: candidateScript("cd03a-continuation-harness-v3-receipt") },
    { path: "src/shared/conversationDisclosureProgram.test.ts", stagedTargetPath: ".zerox/verification/conversation-disclosure/CD03A-round3-program-test.target.ts", kind: "program-test-migration", original: "export const state = 'before';\n", target: "export const state = 'after';\n" },
    { path: "src/shared/packageScripts.test.ts", stagedTargetPath: ".zerox/verification/conversation-disclosure/CD03A-round3-package-scripts-test.target.ts", kind: "package-test-migration", original: "export const packageState = 'before';\n", target: "export const packageState = 'after';\n" },
  ];
  ...
  const round2PrefreezeRejection = createRound2PrefreezeRejection(transitions);
  await writeRepoJson(repo, round2PrefreezeRejectionPath, round2PrefreezeRejection, 0o600);
  const round2PrefreezeRejectionDigest = await sha256File(
    path.join(repo, round2PrefreezeRejectionPath),
  );
  const admissionCoverage = [
    ...transitions.flatMap((entry) => [
      { class: "transition_live", path: entry.path },
      { class: "transition_payload", path: entry.stagedTargetPath },
    ]),
    { class: "frozen_file", path: checkerPath },
    { class: "frozen_file", path: round2PrefreezeRejectionPath },
    { class: "frozen_file", path: runnerRepoPath },
    ...reviewOutputPaths.map((entry) => ({
      class: "review_output_absent",
      path: entry,
    })),
  ...
  const policy = resign({
    ...
    algorithm: "sha256-canonical-json-v1",
    policyId: "CD03A-round3-P108-admission-v3",
    programId: "fixture-program",
    workstreamId: "CD03A",
    featureId: "P107A-conversation-disclosure-successor-admission",
    round: 3,
    ...
    admission: {
      reviewOutputPaths,
      reviewCoverageDigest: hashCanonical(admissionCoverage),
    },
    admissionCoverage,
    successor: {
  ...
  const snapshotDraft: Record<string, unknown> = {
    ...
    governanceTransitions: transitions,
    transitionPayloadFiles: transitions.map((entry) => ({
      path: entry.stagedTargetPath,
      sha256: entry.toSha256,
    })).sort((left, right) => left.path.localeCompare(right.path)),
    frozenFiles: [
      { path: checkerPath, sha256: checkerDigest },
      { path: round2PrefreezeRejectionPath, sha256: round2PrefreezeRejectionDigest },
      { path: runnerRepoPath, sha256: runnerDigest },
      ...(options.overlapTransitionPayload
        ? [{
            path: transitions[0]!.stagedTargetPath,
            sha256: transitions[0]!.toSha256,
          }]
        : []),
    ].sort((left, right) => left.path.localeCompare(right.path)),
    baselineFiles,
  ...
  const anchorOutput = path.join(externalRoot, "round3-anchor.json");
  const journal = `${anchorOutput}.closure-v3.journal.json`;
  const args = [
    "--repo", repo,
...
async function runRunner(
  ...
  if (options.fault) env.ZEROX_CD03A_RUNNER_V3_TEST_FAULT = options.fault;
...
function createRound2PrefreezeRejection(transitions: Transition[]) {
  const withoutDigest = {
    schemaVersion: 3,
    kind: "conversation-disclosure-continuation-prefreeze-rejection",
    algorithm: "sha256-canonical-json-v1",
    programId: "fixture-program",
    workstreamId: "CD03A",
    featureId: "P107A-conversation-disclosure-successor-admission",
    ...
    contradiction: {
      kind: "transition-payload-frozen-live-contradiction",
      status: "confirmed",
    },
    sourcePolicy: {
      path: ".zerox/verification/conversation-disclosure/CD03A-round2-successor-evolution-policy.json",
      digest: digest("b"),
    },
    baselineArchive: {
      path: ".zerox/verification/conversation-disclosure/CD03A-round2-baseline-archive.json",
      digest: digest("c"),
    },
    continuationExecutables: [checkerPath, runnerRepoPath],
    governanceTransitions: transitions,
    liveTransitionFiles: transitions.map((entry) => ({
      path: entry.path,
