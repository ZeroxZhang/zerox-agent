...
const governance = await import(
  ...
  "../../scripts/conversation-disclosure-program-governance-v10.mjs"
);
...
describe("conversation disclosure program governance v10", () => {
  it("accepts the closed-world P107A admission lifecycle and reports every rule", () => {
    ...
    expect(result.kind).toBe(governance.GOVERNANCE_V10_RESULT_KIND);
    expect(result.errors).toEqual([]);
    expect(result.ruleResults.map((rule: RuleResult) => rule.id)).toEqual(
      governance.PROGRAM_GOVERNANCE_V10_RULE_IDS,
    );
  ...
  it("accepts only the exact all-from or all-to governance target state", () => {
    ...
    expect(contract.validateGovernanceTransitionStateV10(
      transitions,
  ...
  it("keeps the Round10 harness diagnostic-only by default and identity claims not-signed", () => {
    ...
    expect(contract.CONTINUATION_V10_CALLER_DISPATCH_ASSURANCE).toBe(
      "caller-attested-not-signed",
    );
    expect(contract.CONTINUATION_V10_IDENTITY_ASSURANCE).toBe("not-signed");
    expect(contract.CONTINUATION_V10_INDEPENDENCE_CLAIM).toBe(
      "caller-attested-distinct-review-contexts",
    );
  });

  it("changes the Round9 package target only for V10 tests and authoritative program checking", () => {
  ...
  it("keeps pre-freeze target tests on the published Round9 baseline archive", () => {
    const target = readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round10-program-test.target.ts",
    ), "utf8");
    expect(target).toContain("CD03A-round9-baseline-archive.json");
    expect(target).not.toContain("CD03A-round10-baseline-archive.json");
  });
  ...
  it("rejects an unknown required rule id", () => {
    ...
    expect(ruleResult(validate(input), "CDG4-001-input-contract").message).toContain(
      "closedWorld.ruleIds must exactly match the known v10 ledger",
    );
  });
  ...
  it("rejects unsatisfiable or incomplete P107A completion artifacts", () => {
    ...
    p107a.completionArtifacts = [
      ...governance.P107A_V10_COMPLETION_ARTIFACTS,
    ...
    expect(result.message).toContain("exact satisfiable Round10 closure set");
  ...
  it("runs the authoritative completion checker in the current review state", async () => {
    await expect(
      completionChecker.checkConversationDisclosureProgramV10({
        repositoryRoot: process.cwd(),
      }),
    ).resolves.toMatchObject({
      status: "passed",
      phase: "review",
      completionArtifactCount: 72,
    });
...
function validate(input: GovernanceInput) {
  return governance.validateConversationDisclosureProgramGovernanceV10(input);
}
...
function createInput() {
  ...
  const workstreams = [
    ...
    createWorkstream("CD08", "P112-v10.9.2-disclosure-hardening", "planned", ["CD07"], ["D1"]),
    createWorkstream("CD09", "P113-v10.9.2-disclosure-adversarial-acceptance", "planned", ["CD08"], ["D1"]),
  ];
  ...
  const program = {
      schemaVersion: 1,
      programId: "conversation-progressive-disclosure-v10.9.2-2026-08",
      status: "active",
      maxActiveFeatures: 1,
  ...
  const programRootBinding = governance.createProgramRootBindingV10(program);
  return {
    program,
    ...
    closedWorld: {
      schemaVersion: 10,
      ...programRootBinding,
      ruleIds: [...governance.PROGRAM_GOVERNANCE_V10_RULE_IDS],
...
function createTransitions() {
  return Object.entries(
    contract.CONTINUATION_V10_GOVERNANCE_TRANSITIONS as Record<string, string>,
  )
    ...
    .map(([livePath, kind], index) => ({
      ...
      kind,
      stagedTargetPath:
        `.zerox/verification/conversation-disclosure/CD03A-round10-target-${index}`,
...
type GovernanceResult = ReturnType<typeof governance.validateConversationDisclosureProgramGovernanceV10>;
