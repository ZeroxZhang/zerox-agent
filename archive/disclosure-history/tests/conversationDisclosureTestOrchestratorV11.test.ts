...
describe("conversation disclosure test orchestrator v11", () => {
  ...
  test("executes immutable V3, V4, V7, V8, V9, and V10 tests in reconstructed historical states", () => {
  ...
  test("executes Program and package tests against all four Round11 targets", () => {
    expect(source).toContain("applyRound11Targets");
    expect(source).toContain("CD03A-round11-package.target.json");
    expect(source).toContain("CD03A-round11-harness.target.mjs");
    expect(source).toContain("CD03A-round11-program-test.target.ts");
    expect(source).toContain("CD03A-round11-package-scripts-test.target.ts");
  });
  ...
  test("reconstructs the all-from current lane when invoked from all-to", async () => {
    ...
    for (const transition of orchestrator.ROUND11_TRANSITION_MAPPINGS) {
      await copyFixtureFile(transition.source, fixture);
      await copyFixtureFile(transition.destination, fixture);
    }
    await expect(
      orchestrator.detectRound11TransitionState(fixture),
    ...
    await orchestrator.restoreCurrentRound11Sources(fixture);
    await expect(
      orchestrator.detectRound11TransitionState(fixture),
    ).resolves.toBe("source");
  });
