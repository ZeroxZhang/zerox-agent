...
- Publication evidence:
  - Release commit `76d89fbe9fcee5a237dbeb0eb60876cb7c4ef685` pushed on branch `codex/3.3.0`.
...
- Publication evidence:
  - Release commit `8b6cb33305182e3ee4390a176545577b3fdc857d` pushed on branch `codex/3.4.0`.
...
## 2026-08-30 - Round-11 re-anchor: regenerated verification artifacts committed

- At the user's request, the anchor-regenerated verification artifacts
  (`.zerox/verification/conversation-disclosure/` scenario outputs, CD08
  hardening receipt, CD09 real-app acceptance receipt) and the re-signed
  CD09 review receipts were committed as a third commit on `codex/3.9.2`,
  so the working tree carries no uncommitted evidence. Build outputs and
  `release-test-*` residuals remain deliberately uncommitted.
- Ceremony order per the B10 lesson: this evidence section was written
  first, the receipt cascade was re-run (receipts re-signed against the
  post-edit manifest), the commit was created, the caller git pins
  (`ZEROX_V392_ACCEPTANCE_GIT_HEAD/_GIT_TREE`) were updated to the new
  HEAD, and only then was the round-11 anchor run. The round-11 anchor
  path/digest are recorded in `/private/tmp/zerox-v392-pins/checker-env.sh`.
- Known characteristic: CD09 scenario outputs (JSON receipts with run
  timestamps, renderer screenshots) are regenerated by every anchor run and
  are excluded from the acceptance-input manifest; a working tree freshly
