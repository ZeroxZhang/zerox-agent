...
describe("package scripts", () => {
  ...
  it("exposes harness engineering commands", () => {
    ...
    expect(round11PackageTarget).toEqual({
      ...round10PackageTarget,
      scripts: {
        ...round10PackageTarget.scripts,
    ...
    expect(packageJson.scripts?.["program:check"]).toContain(
      "check-conversation-disclosure-program-v11.mjs",
    );
    ...
    expect(harnessSource).toContain(
      "const checkerReceipt = await runConversationDisclosureContinuationCheckerV11(",
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v11-receipt"',
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v11-local-diagnostic"',
    );
    expect(harnessSource).toContain("schemaVersion: 11");
    expect(harnessSource).toContain("CD03A-round11-successor-evolution-policy.json");
    expect(harnessSource).toContain("CD03A-round11-review-snapshot.json");
    expect(harnessSource).toContain("CD03A-round10-review-rejection.json");
