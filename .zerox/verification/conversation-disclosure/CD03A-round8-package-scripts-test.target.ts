...
describe("package scripts", () => {
  ...
  it("exposes harness engineering commands", () => {
    ...
    expect(round8PackageTarget).toEqual({
      ...round7PackageTarget,
      scripts: {
        ...round7PackageTarget.scripts,
        test: "node scripts/run-conversation-disclosure-tests-v8.mjs",
        "program:check":
          "node scripts/check-runtime-convergence-program.mjs && node scripts/check-kernel-migration-program.mjs && node scripts/check-storage-convergence-program.mjs && node scripts/check-release-program.mjs && node scripts/check-conversation-disclosure-program-v8.mjs && node scripts/check-harness-state.mjs --diagnostic-only",
      },
    });
    ...
    expect(packageJson.scripts?.["program:check"]).toContain(
      "check-conversation-disclosure-program-v8.mjs",
    );
    ...
    expect(harnessSource).toContain(
      "const checkerReceipt = await runConversationDisclosureContinuationCheckerV8(",
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v8-receipt"',
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v8-local-diagnostic"',
    );
    expect(harnessSource).toContain("schemaVersion: 8");
    expect(harnessSource).toContain("CD03A-round8-successor-evolution-policy.json");
    expect(harnessSource).toContain("CD03A-round8-review-snapshot.json");
    expect(harnessSource).toContain("CD03A-round7-review-rejection.json");
