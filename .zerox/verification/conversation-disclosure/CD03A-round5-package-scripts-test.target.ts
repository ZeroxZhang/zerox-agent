...
describe("package scripts", () => {
  ...
  it("exposes harness engineering commands", () => {
    ...
    const round4PackageTarget = JSON.parse(readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round4-package.target.json",
    ), "utf8")) as PackageJson;
    const round5PackageTarget = JSON.parse(readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round5-package.target.json",
    ), "utf8")) as PackageJson;
    expect(round5PackageTarget).toEqual({
      ...round4PackageTarget,
      scripts: {
        ...round4PackageTarget.scripts,
        test: "node scripts/run-conversation-disclosure-tests-v5.mjs",
      },
    });
    ...
    expect(packageJson.scripts).toMatchObject({
      ...
      "program:check":
        "node scripts/check-runtime-convergence-program.mjs && node scripts/check-kernel-migration-program.mjs && node scripts/check-storage-convergence-program.mjs && node scripts/check-release-program.mjs && node scripts/check-harness-state.mjs --diagnostic-only",
    ...
    expect(harnessSource).toContain(
      "runConversationDisclosureContinuationCheckerV5",
    );
    expect(harnessSource).toContain(
      "const checkerReceipt = await runConversationDisclosureContinuationCheckerV5(",
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v5-receipt"',
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v5-local-diagnostic"',
    );
    expect(harnessSource).toContain("schemaVersion: 5");
    expect(harnessSource).toContain("CD03A-round5-successor-evolution-policy.json");
    expect(harnessSource).toContain("CD03A-round5-review-snapshot.json");
    expect(harnessSource).toContain("CD03A-round4-review-rejection.json");
