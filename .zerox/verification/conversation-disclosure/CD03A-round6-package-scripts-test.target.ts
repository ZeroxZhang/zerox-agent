...
describe("package scripts", () => {
  ...
  it("exposes harness engineering commands", () => {
    ...
    expect(round6PackageTarget).toEqual({
      ...round5PackageTarget,
      scripts: {
        ...round5PackageTarget.scripts,
        test: "node scripts/run-conversation-disclosure-tests-v6.mjs",
      },
    });
    ...
    expect(harnessSource).toContain(
      "const checkerReceipt = await runConversationDisclosureContinuationCheckerV6(",
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v6-receipt"',
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v6-local-diagnostic"',
    );
    expect(harnessSource).toContain("schemaVersion: 6");
    expect(harnessSource).toContain("CD03A-round6-successor-evolution-policy.json");
    expect(harnessSource).toContain("CD03A-round6-review-snapshot.json");
    expect(harnessSource).toContain("CD03A-round5-review-rejection.json");
