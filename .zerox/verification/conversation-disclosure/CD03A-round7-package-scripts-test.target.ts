...
describe("package scripts", () => {
  ...
  it("exposes harness engineering commands", () => {
    ...
    const round6PackageTarget = JSON.parse(readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round6-package.target.json",
    ), "utf8")) as PackageJson;
    const round7PackageTarget = JSON.parse(readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round7-package.target.json",
    ), "utf8")) as PackageJson;
    expect(round7PackageTarget).toEqual({
      ...round6PackageTarget,
      scripts: {
        ...round6PackageTarget.scripts,
        test: "node scripts/run-conversation-disclosure-tests-v7.mjs",
        "program:check":
          "node scripts/check-runtime-convergence-program.mjs && node scripts/check-kernel-migration-program.mjs && node scripts/check-storage-convergence-program.mjs && node scripts/check-release-program.mjs && node scripts/check-conversation-disclosure-program-v7.mjs && node scripts/check-harness-state.mjs --diagnostic-only",
      },
    });
    ...
    expect(packageJson.scripts?.["program:check"]).toContain(
      "check-conversation-disclosure-program-v7.mjs",
    );
    ...
    expect(harnessSource).toContain(
      "const checkerReceipt = await runConversationDisclosureContinuationCheckerV7(",
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v7-receipt"',
    );
    expect(harnessSource).toContain(
      'kind: "cd03a-continuation-harness-v7-local-diagnostic"',
    );
    expect(harnessSource).toContain("schemaVersion: 7");
    expect(harnessSource).toContain("CD03A-round7-successor-evolution-policy.json");
    expect(harnessSource).toContain("CD03A-round7-review-snapshot.json");
    expect(harnessSource).toContain("CD03A-round6-review-rejection.json");
