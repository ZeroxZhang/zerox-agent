...
**三级密度（2026-09 原型评审后新增，用户明确要求）**

单块两态（摘要行 ↔ 内容）不足以避免堆积。过程流必须有三层：

```
L0  折叠组      「更早的过程 · K 步」            ← 更早的普通块全部收进这一行
L1  摘要行      每个块默认只给一行              ← 最近 N 条 + 全部注意力块
L2  块内容      展开后的推理文本 / 参数 / 结果
```

规则：

1. **注意力块永不进折叠组**：`attention !== normal`（失败、阻塞、等待授权、被拒绝）始终留在 L1，且按上表自动展开到 L2。密度策略不得掩盖可操作状态（继承 P104 D9）。
2. **最近窗口**：非注意力块中最后 **N = 3** 条留在 L1；更早的非注意力块进入 L0 折叠组，默认收起，组头显示「更早的过程 · K 步」与末条摘要。
3. **L0 可单独展开**，展开后组内块仍是 L1（不会一次性铺开全部内容）。
4. **本轮结束后**：整个过程流折叠为一行「本轮过程 · M 步 · 用时 T · N 次工具」，点击展开回到上面的分层。
5. **偏好覆盖**：`open` 强制展开 L0 与全部 L2；`compact` 强制收起 L0 与全部 L2；`pinned` 保留用户手动打开的 L0/块。

对应到既有解析器：`resolveConversationDisclosurePolicy` 的 `volume > 4` 只是"更紧凑"的雏形，需要升级为显式的 `recentWindowSize` + `groupOlder` 两个决策，且注意力块豁免。
...
├── 用户消息
├── L0 折叠组：「更早的过程 · K 步」        ← 更早的非注意力块
├── 过程块流（按 sequence 排序，L1 摘要行）
...
├── 本轮结束后：整段折叠为「本轮过程 · M 步 · 用时 T」
└── 助手答案（流式，始终可见）
```

**右栏职责收窄（2026-09 原型评审后确定）**：右栏保留，但只放**全局信息与目标/任务阶段**——目标与阶段、会话上下文占用、运行环境（工作区/模型）。**不再重复本轮的步骤清单**，那部分归主对话的过程块。
