...
describe("conversation disclosure continuation manifest v8", () => {
  test("requires external dispatch, policy, and snapshot caller pins", async () => {
    await expect(
      manifestBuilder.buildConversationDisclosureContinuationManifestV8({
  ...
  test("derives one pending digest across pending and final forms", () => {
    ...
    const pendingDigest = contract.pendingManifestDigestV8(base);
    const final = {
      ...base,
    ...
    expect(contract.pendingManifestDigestV8(final)).toBe(pendingDigest);
  });
