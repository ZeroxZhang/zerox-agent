...
const governance = await import(
  ...
  "../../scripts/conversation-disclosure-program-governance-v3.mjs"
);
...
describe("conversation disclosure program governance v3", () => {
  it("accepts the closed-world P107A admission lifecycle and reports every rule", () => {
    ...
    expect(result.ruleResults.map((rule: RuleResult) => rule.id)).toEqual(
      governance.PROGRAM_GOVERNANCE_V3_RULE_IDS,
    );
  ...
  it.each([
    ...
    },
    {
      ...
      rule: "CDG3-011-program-root-contract",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG3-041-scenario-categories",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG3-042-scenario-coverage",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG3-050-workstream-schema",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG3-052-dependency-graph",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG3-070-implementation-gates",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG3-090-p107a-p108-lifecycle",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG3-100-cd03-evidence-refs",
      mutate(input: GovernanceInput) {
  ...
  it("rejects an unknown required rule id", () => {
    const input = createInput();
    input.closedWorld.ruleIds.push("CDG3-666-candidate-rule");

    expect(ruleResult(validate(input), "CDG3-001-input-contract").message).toContain(
      "closedWorld.ruleIds must exactly match the known v3 ledger",
    );
  });
  ...
  it("rejects an unknown workstream even when its fields and dependency are valid", () => {
    ...
    expect(ruleResult(validate(input), "CDG3-051-closed-roster").status).toBe("failed");
  });
  ...
  it("rejects P108 done until CD04 publishes a next-version reviewed delta trust head", () => {
    ...
    expect(ruleResult(
      validate(input),
      "CDG3-090-p107a-p108-lifecycle",
    ).message).toContain("unknown continuation lifecycle phase");
  });
});

function validate(input: GovernanceInput) {
  return governance.validateConversationDisclosureProgramGovernanceV3(input);
}
...
function createInput() {
  ...
  const programRootBinding = governance.createProgramRootBindingV3(program);
  return {
    program,
    ...
    closedWorld: {
      schemaVersion: 3,
      ...programRootBinding,
      ruleIds: [...governance.PROGRAM_GOVERNANCE_V3_RULE_IDS],
...
type GovernanceResult = ReturnType<typeof governance.validateConversationDisclosureProgramGovernanceV3>;
