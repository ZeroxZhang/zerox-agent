...
function buildFixture() {
  const policy = {
    ...
    round8ReviewRejection: { digest: digest("3") },
    successor: {
    ...
    continuationExecutables: [{
      kind: "checker",
      path: "scripts/check-conversation-disclosure-continuation-v9.mjs",
      sha256: digest("6"),
    }],
  };
  const snapshot = {
  ...
  const dispatchSet = ["contract", "runtime", "governance"].map(
    (lane, index) => ({
      ...
      taskPath: `round9/${lane}`,
      agentLabel: `round9-${lane}-reviewer`,
      transport: "codex-collaboration",
    }),
  );
  const policyReference = {
    path: contract.CONTINUATION_V9_POLICY_PATH,
    byteSha256: digest("a"),
    canonicalDigest: policy.digest,
  };
  const snapshotReference = {
    path: contract.CONTINUATION_V9_REVIEW_SNAPSHOT_PATH,
    byteSha256: digest("b"),
    canonicalDigest: snapshot.digest,
  };
  const rejectionReference = {
    path: contract.CONTINUATION_V9_ROUND8_REVIEW_REJECTION_PATH,
    byteSha256: digest("c"),
  ...
  const runnerReference = {
    path: "scripts/verify-conversation-disclosure-continuation-v9.mjs",
    sha256: digest("d"),
  };
  const pendingBase = {
    schemaVersion: 9,
    kind: contract.CONTINUATION_V9_MANIFEST_KIND,
    ...
    round8ReviewRejection: rejectionReference,
    reviewReceipts: reviewReferences,
    callerDispatchSet: dispatchSet,
    validator: validatorReference,
    externalRunner: runnerReference,
    externalAttestation: {
      path: contract.CONTINUATION_V9_EXTERNAL_ATTESTATION_PATH,
      canonicalDigest: null,
    },
  };
  const pendingManifestDigest =
    contract.pendingManifestDigestV9(pendingBase);
  const pendingManifest = contract.withCanonicalDigestV9({
    ...pendingBase,
  ...
  const attestation = contract.withCanonicalDigestV9({
    schemaVersion: 9,
    kind: contract.CONTINUATION_V9_ATTESTATION_KIND,
    status: "passed",
    governancePhase: "review_post_transition",
    identityAssurance: contract.CONTINUATION_V9_IDENTITY_ASSURANCE,
    reviewAssurance: contract.CONTINUATION_V9_CALLER_DISPATCH_ASSURANCE,
    repositoryRealpath: "/tmp/round9-subject",
  ...
  const finalManifest = contract.withCanonicalDigestV9(
    contract.finalManifestProjectionV9(pendingManifest, attestation.digest),
  );
  const anchor = contract.withCanonicalDigestV9({
    schemaVersion: 9,
    kind: contract.CONTINUATION_V9_ANCHOR_KIND,
    identityAssurance: contract.CONTINUATION_V9_IDENTITY_ASSURANCE,
    reviewAssurance: contract.CONTINUATION_V9_CALLER_DISPATCH_ASSURANCE,
    repositoryRealpath: "/tmp/round9-subject",
  ...
  const manifestBindings = {
    ...
    policyReference,
    snapshotReference,
    round8ReviewRejectionReference: rejectionReference,
    reviewReferences,
    callerDispatchSet: dispatchSet,
    validatorReference,
    runnerReference,
...
function rehash<T extends Record<string, any>>(
  ...
  delete clone.digest;
  clone.digest = contract.hashCanonicalV9(clone);
  return clone as T & { digest: string };
}

describe("conversation disclosure final evidence v9", () => {
  test("accepts one completely bound pending/final/attestation/anchor chain", () => {
    const fixture = buildFixture();
    expect(contract.validateContinuationClosureManifestV9(
      fixture.pendingManifest,
      fixture.manifestBindings,
    )).toEqual([]);
    expect(contract.validateContinuationExternalAttestationV9(
      fixture.attestation,
    ...
    expect(contract.validateContinuationExternalAnchorV9(
      fixture.anchor,
  ...
  test("rejects every stale attestation semantic root", () => {
    ...
    dispatch[0].taskPath = "round9/other";
    const attestation = rehash({
      ...fixture.attestation,
    ...
    for (const mutation of [
      ...
      },
      {
        round8ReviewRejection: {
          ...fixture.finalManifest.round8ReviewRejection,
          canonicalDigest: digest("0"),
        },
      },
      ...
      expect(contract.validateContinuationClosureManifestV9(manifest, {
        ...fixture.manifestBindings,
        pendingManifest: fixture.pendingManifest,
        externalAttestation: fixture.attestation,
      })).not.toEqual([]);
  ...
  test("rejects stale anchor roots, future time, and missing bindings", () => {
    ...
    for (const key of [
      ...
      expect(contract.validateContinuationExternalAnchorV9(rootMutant, {
        ...fixture.anchorBindings,
    ...
    expect(contract.validateContinuationExternalAnchorV9(dispatchMutant, {
      ...fixture.anchorBindings,
    ...
    expect(contract.validateContinuationExternalAnchorV9(future, {
      ...fixture.anchorBindings,
