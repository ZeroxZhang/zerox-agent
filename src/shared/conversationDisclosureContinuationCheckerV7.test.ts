...
const checker = await import(
  ...
  "../../scripts/check-conversation-disclosure-continuation-v7.mjs"
);
...
describe("conversation disclosure continuation checker v7", () => {
  test("rejects an unpinned or unknown mode before reading evidence", async () => {
    await expect(
      checker.runConversationDisclosureContinuationCheckerV7([]),
    ).rejects.toThrow("checker option is required");
    await expect(
      checker.runConversationDisclosureContinuationCheckerV7([
        "--mode", "candidate_self_authorized",
      ]),
    ).rejects.toThrow("checker V7 --mode is invalid");
  });
