...
function rehash<T extends Record<string, unknown>>(value: T): T & { digest: string } {
  ...
  delete copy.digest;
  copy.digest = contract.hashCanonicalV6(copy);
  return copy as T & { digest: string };
}

function makeRound5Rejection() {
  return contract.withCanonicalDigestV6({
    schemaVersion: 6,
    kind: contract.CONTINUATION_V6_REVIEW_REJECTION_KIND,
    algorithm: contract.CONTINUATION_V6_ALGORITHM,
    ...
    rejectedRound: contract.CONTINUATION_V6_REJECTED_ROUND,
    recoveryRound: contract.CONTINUATION_V6_ROUND,
    status: "rejected_after_review",
    sourcePolicy: structuredClone(
      contract.CONTINUATION_V6_ROUND5_POLICY_TRUST_ROOT,
    ),
    sourceSnapshot: structuredClone(
      contract.CONTINUATION_V6_ROUND5_SNAPSHOT_TRUST_ROOT,
    ),
    failedReceipts: contract.CONTINUATION_V6_ROUND5_RECEIPT_TRUST_ROOTS.map(
      (entry: Record<string, unknown>) => structuredClone(entry),
    ),
    findingIds: [...contract.CONTINUATION_V6_ROUND5_FINDING_IDS],
    findingSetDigest: contract.CONTINUATION_V6_ROUND5_FINDING_SET_DIGEST,
    aggregateFindingCounts: structuredClone(
      contract.CONTINUATION_V6_ROUND5_AGGREGATE_FINDING_COUNTS,
    ),
    repositoryForbiddenOutputs: [
      ...contract.CONTINUATION_V6_ROUND5_REPOSITORY_FORBIDDEN_OUTPUT_PATHS,
    ],
    externalAnchorRule: contract.expectedRejectedRound5AnchorRuleV6(),
    priorRejections: {
      round1CanonicalDigest: contract.CONTINUATION_V6_ROUND1_REJECTION_DIGEST,
      round2: structuredClone(
        contract.CONTINUATION_V6_ROUND2_REJECTION_TRUST_ROOT,
      ),
    },
  });
}
...
function makeArchive() {
  ...
  return rehash({
    ...source,
    schemaVersion: 6,
    kind: contract.CONTINUATION_V6_BASELINE_ARCHIVE_KIND,
    round: 6,
  });
}
...
function makePolicy() {
  ...
  const rejection = makeRound5Rejection();
  const archive = makeArchive();
  const admission = structuredClone(source.admission);
  const transitions = contract.CONTINUATION_V6_GOVERNANCE_TRANSITION_TRUST_ROOTS
    .map((entry: Record<string, unknown>) => structuredClone(entry));
  admission.reviewOutputPaths = [
    ...contract.CONTINUATION_V6_REVIEW_OUTPUT_PATHS,
  ];
  admission.featureDefinition.files = [...new Set([
    ...admission.featureDefinition.files,
    contract.CONTINUATION_V6_ROUND5_REVIEW_REJECTION_PATH,
    contract.CONTINUATION_V6_BASELINE_ARCHIVE_PATH,
    contract.CONTINUATION_V6_POLICY_PATH,
    ...Object.values(
      contract.CONTINUATION_V6_EXECUTABLE_PATH_BY_KIND as Record<string, string>,
    ),
    ...transitions.map((entry: any) => entry.stagedTargetPath),
    ...contract.CONTINUATION_V6_REVIEW_OUTPUT_PATHS,
    ...contract.CONTINUATION_V6_REJECTED_OUTPUT_ABSENT_PATHS,
  ])].sort();
  admission.featureDefinitionDigest = contract.hashCanonicalV6(
    admission.featureDefinition,
  );
  admission.featureFileSetDigest = contract.hashCanonicalV6(
    admission.featureDefinition.files,
  );
  const admissionCoverage = contract.buildAdmissionCoverageV6(
    admission,
    transitions,
  );
  admission.reviewCoverageDigest = contract.hashCanonicalV6(admissionCoverage);
  const continuationExecutables = contract.CONTINUATION_V6_EXECUTABLE_KINDS.map(
    (kind: string, index: number) => ({
      kind,
      path: contract.CONTINUATION_V6_EXECUTABLE_PATH_BY_KIND[kind],
  ...
  return contract.withCanonicalDigestV6({
    schemaVersion: 6,
    kind: contract.CONTINUATION_V6_POLICY_KIND,
    algorithm: contract.CONTINUATION_V6_ALGORITHM,
    policyId: contract.CONTINUATION_V6_POLICY_ID,
    ...
    admission,
    admissionClassSet: [...contract.CONTINUATION_V6_ADMISSION_CLASSES],
    admissionClassSetDigest:
      contract.CONTINUATION_V6_ADMISSION_CLASS_SET_DIGEST,
    admissionCoverage,
    ...
    baselineArchive: {
      path: contract.CONTINUATION_V6_BASELINE_ARCHIVE_PATH,
      digest: archive.digest,
      entrySetDigest: archive.entrySetDigest,
    },
    reviewSnapshot: { path: contract.CONTINUATION_V6_REVIEW_SNAPSHOT_PATH },
    reviewAssurancePolicy: {
      callerDispatchAssurance:
        contract.CONTINUATION_V6_CALLER_DISPATCH_ASSURANCE,
      identityAssurance: contract.CONTINUATION_V6_IDENTITY_ASSURANCE,
      independenceClaim: contract.CONTINUATION_V6_INDEPENDENCE_CLAIM,
      localIdentityProof: false,
    },
    externalAnchorPolicy: contract.expectedRejectedRound5AnchorRuleV6(),
...
function makeSnapshot(policy: any) {
  ...
  for (const reference of [
    contract.CONTINUATION_V6_ROUND5_POLICY_TRUST_ROOT,
    contract.CONTINUATION_V6_ROUND5_SNAPSHOT_TRUST_ROOT,
    ...contract.CONTINUATION_V6_ROUND5_RECEIPT_TRUST_ROOTS,
  ]) {
    frozen.get(reference.path)!.sha256 = reference.byteSha256;
  }
  frozen.get(
    contract.CONTINUATION_V6_ROUND5_REVIEW_REJECTION_PATH,
  )!.sha256 = contract.sha256BytesV6(
    contract.serializeRound5ReviewRejectionV6(
      policy.round5ReviewRejection,
    ),
  );
  ...
  return contract.withCanonicalDigestV6({
    schemaVersion: 6,
    kind: contract.CONTINUATION_V6_SNAPSHOT_KIND,
    algorithm: contract.CONTINUATION_V6_ALGORITHM,
    ...
    pathAuthorityDigest: contract.hashCanonicalV6(policy.pathAuthorities),
...
function makeReceipt(policy: any, snapshot: any) {
  const dispatchEntry = {
    lane: "contract",
    assurance: contract.CONTINUATION_V6_CALLER_DISPATCH_ASSURANCE,
  ...
  return {
    schemaVersion: 6,
    kind: contract.CONTINUATION_V6_RECEIPT_KIND,
    programId: policy.programId,
    workstreamId: contract.CONTINUATION_V6_WORKSTREAM_ID,
    featureId: contract.CONTINUATION_V6_FEATURE_ID,
    round: contract.CONTINUATION_V6_ROUND,
    ...
    callerDispatchEntryDigest: contract.hashCanonicalV6(dispatchEntry),
    callerDispatchSetDigest: contract.hashCanonicalV6(dispatchSet),
    claimedReviewOrigin: {
      taskPath: "caller/contract",
      agentLabel: "claimed-reviewer",
      transport: "codex-collaboration",
...
describe("conversation disclosure continuation contract v6", () => {
  test("binds the real runtime I/O API instead of the abandoned adapter", () => {
    expect(contract.validateRuntimeIoCaptureAdapterV6(runtimeIo)).toEqual([]);
    expect(contract.CONTINUATION_V6_RUNTIME_IO_INTERFACE.methods).not.toContain(
      "capturePresent",
    );
  });

  test("validates the exact Round5 rejection and both roots of every source", () => {
    const witness = makeRound5Rejection();
    expect(contract.validateRound5ReviewRejectionV6(witness)).toEqual([]);
    const references = [
    ...
    for (const reference of references) {
      ...
      expect(contract.validateRound5ReviewRejectionV6(rehash(mutant)))
        .not.toEqual([]);
    }
  });
  ...
  test("hard-roots the exact ordered six-class set", () => {
    ...
    expect(contract.validateContinuationPolicyV6(policy)).toEqual([]);
    for (const classSet of [
      ...
      mutant.admissionClassSetDigest = contract.hashCanonicalV6(classSet);
      expect(contract.validateContinuationPolicyV6(rehash(mutant)))
  ...
  test("rejects policy key omission and extension", () => {
    ...
    expect(contract.validateContinuationPolicyV6(omitted))
      .toContain("continuation policy must contain the exact V6 keys");
    const extended = { ...policy, candidateField: true };
    expect(contract.validateContinuationPolicyV6(extended))
      .toContain("continuation policy must contain the exact V6 keys");
  });

  test("validates the V6 archive and snapshot bindings", () => {
    const policy = makePolicy();
    const archive = makeArchive();
    const snapshot = makeSnapshot(policy);
    expect(contract.validateBaselineArchiveV6(archive, policy)).toEqual([]);
    expect(contract.validateContinuationReviewSnapshotV6(snapshot, policy, {
      verifierNow: Date.parse(snapshot.frozenAt),
    })).toEqual([]);
    const mutant = rehash({
      ...snapshot,
      round5ReviewRejectionDigest: zeroDigest,
    });
    expect(contract.validateContinuationReviewSnapshotV6(mutant, policy, {
      verifierNow: Date.parse(snapshot.frozenAt),
    })).toContain(
      "continuation review snapshot round5ReviewRejectionDigest binding is stale",
    );
  });
  ...
  test("rejects stronger identity assurance in policy and receipts", () => {
    ...
    expect(contract.validateContinuationPolicyV6(rehash(policyMutant)))
      .toContain(
    ...
    expect(contract.validateContinuationReviewReceiptV6(
      receipt,
      snapshot,
      policy,
      { callerPin },
    ...
    expect(contract.validateContinuationReviewReceiptV6(
      ...
      "continuation review receipt V6 identity/assurance is invalid",
    );
  });

  test("uses canonical deterministic gzip archive entries", () => {
    const bytes = Buffer.from("round6 baseline\n", "utf8");
    const entry = {
      path: "package.json",
      source: "governance_transition",
      sha256: contract.sha256BytesV6(bytes),
      encoding: "gzip-base64-v1",
      bytes: gzipSync(bytes, { level: 9 } as any).toString("base64"),
    };
    const archive = contract.withCanonicalDigestV6({
      schemaVersion: 6,
      kind: contract.CONTINUATION_V6_BASELINE_ARCHIVE_KIND,
      algorithm: contract.CONTINUATION_V6_ALGORITHM,
      programId: "conversation-progressive-disclosure-v3.9.2-2026-08",
      ...
      round: contract.CONTINUATION_V6_ROUND,
      entries: [entry],
      entrySetDigest: contract.hashCanonicalV6([entry]),
    });
    expect(contract.validateBaselineArchiveV6(archive)).toEqual([]);
    const mutant = structuredClone(archive);
    mutant.entries[0].bytes = gzipSync(bytes, { level: 1 } as any)
      .toString("base64");
    expect(contract.validateBaselineArchiveV6(rehash(mutant)).join("\n"))
