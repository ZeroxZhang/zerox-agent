...
const checker = await import(
  ...
  "../../scripts/check-conversation-disclosure-continuation-v6.mjs"
);
...
describe("conversation disclosure continuation checker v6", () => {
  test("rejects an unpinned or unknown mode before reading evidence", async () => {
    await expect(
      checker.runConversationDisclosureContinuationCheckerV6([]),
    ).rejects.toThrow("checker option is required");
    await expect(
      checker.runConversationDisclosureContinuationCheckerV6([
        "--mode", "candidate_self_authorized",
      ]),
    ).rejects.toThrow("checker V6 --mode is invalid");
  });
