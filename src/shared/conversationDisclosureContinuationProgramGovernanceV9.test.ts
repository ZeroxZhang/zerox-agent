...
const governance = await import(
  ...
  "../../scripts/conversation-disclosure-program-governance-v9.mjs"
);
...
describe("conversation disclosure program governance v9", () => {
  it("accepts the closed-world P107A admission lifecycle and reports every rule", () => {
    ...
    expect(result.kind).toBe(governance.GOVERNANCE_V9_RESULT_KIND);
    expect(result.errors).toEqual([]);
    expect(result.ruleResults.map((rule: RuleResult) => rule.id)).toEqual(
      governance.PROGRAM_GOVERNANCE_V9_RULE_IDS,
    );
  ...
  it("accepts only the exact all-from or all-to governance target state", () => {
    ...
    expect(contract.validateGovernanceTransitionStateV9(
      transitions,
  ...
  it("keeps the Round9 harness diagnostic-only by default and identity claims not-signed", () => {
    ...
    expect(contract.CONTINUATION_V9_CALLER_DISPATCH_ASSURANCE).toBe(
      "caller-attested-not-signed",
    );
    expect(contract.CONTINUATION_V9_IDENTITY_ASSURANCE).toBe("not-signed");
    expect(contract.CONTINUATION_V9_INDEPENDENCE_CLAIM).toBe(
      "caller-attested-distinct-review-contexts",
    );
  });

  it("changes the Round8 package target only for V9 tests and authoritative program checking", () => {
    const round8 = JSON.parse(readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round8-package.target.json",
    ), "utf8"));
    const round9 = JSON.parse(readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round9-package.target.json",
    ), "utf8"));
    expect(round9).toEqual({
      ...round8,
      scripts: {
        ...round8.scripts,
  ...
  it("keeps pre-freeze target tests on the published Round8 baseline archive", () => {
    const target = readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round9-program-test.target.ts",
    ), "utf8");
    expect(target).toContain("CD03A-round8-baseline-archive.json");
    expect(target).not.toContain("CD03A-round9-baseline-archive.json");
  });
  ...
  it("rejects an unknown required rule id", () => {
    ...
    expect(ruleResult(validate(input), "CDG4-001-input-contract").message).toContain(
      "closedWorld.ruleIds must exactly match the known v9 ledger",
    );
  });
  ...
  it("rejects unsatisfiable or incomplete P107A completion artifacts", () => {
    ...
    p107a.completionArtifacts = [
      ...governance.P107A_V9_COMPLETION_ARTIFACTS,
    ...
    expect(result.message).toContain("exact satisfiable Round9 closure set");
  ...
  it("runs the authoritative completion checker in the current review state", async () => {
    await expect(
      completionChecker.checkConversationDisclosureProgramV9({
        repositoryRoot: process.cwd(),
      }),
    ).resolves.toMatchObject({
      status: "passed",
      phase: "review",
      completionArtifactCount: 64,
    });
...
function validate(input: GovernanceInput) {
  return governance.validateConversationDisclosureProgramGovernanceV9(input);
}
...
function createInput() {
  ...
  const workstreams = [
    ...
    createWorkstream("CD08", "P112-v9.9.2-disclosure-hardening", "planned", ["CD07"], ["D1"]),
    createWorkstream("CD09", "P113-v9.9.2-disclosure-adversarial-acceptance", "planned", ["CD08"], ["D1"]),
  ];
  ...
  const program = {
      schemaVersion: 1,
      programId: "conversation-progressive-disclosure-v9.9.2-2026-08",
      status: "active",
      maxActiveFeatures: 1,
  ...
  const programRootBinding = governance.createProgramRootBindingV9(program);
  return {
    program,
    ...
    closedWorld: {
      schemaVersion: 9,
      ...programRootBinding,
      ruleIds: [...governance.PROGRAM_GOVERNANCE_V9_RULE_IDS],
...
function createTransitions() {
  return Object.entries(
    contract.CONTINUATION_V9_GOVERNANCE_TRANSITIONS as Record<string, string>,
  )
    ...
    .map(([livePath, kind], index) => ({
      ...
      kind,
      stagedTargetPath:
        `.zerox/verification/conversation-disclosure/CD03A-round9-target-${index}`,
...
type GovernanceResult = ReturnType<typeof governance.validateConversationDisclosureProgramGovernanceV9>;
