...
function makeArchive() {
  ...
  delete value.digest;
  value.digest = contract.hashCanonicalV7(value);
  return value;
}
...
function makePolicyInput(rejection: any) {
  ...
  const feature = featureList.features.find(
    (entry: any) => entry.id === contract.CONTINUATION_V7_FEATURE_ID,
  );
  feature.files = [...new Set([
    ...feature.files,
    ...policyBuilder.CONTINUATION_POLICY_V7_REQUIRED_ROSTER_PATHS,
  ])];
  const continuationExecutables =
    contract.CONTINUATION_V7_EXECUTABLE_KINDS.map(
...
describe("conversation disclosure continuation policy v7 builder", () => {
  test("builds one exact six-class policy from stable live definitions", async () => {
    const rejection =
      (await rejectionBuilder.buildConversationDisclosureReviewRejectionV7({
        repositoryRoot: root,
      })).witness;
    const input = makePolicyInput(rejection);
    const policy =
      policyBuilder.createConversationDisclosureContinuationPolicyV7(input);

    expect(contract.validateContinuationPolicyV7(policy, {
      expectedDigest: policy.digest,
      baselineArchive: input.baselineArchive,
    })).toEqual([]);
    expect(policy.admissionClassSet).toEqual(
      contract.CONTINUATION_V7_ADMISSION_CLASSES,
    );
    expect(new Set(policy.admissionCoverage.map((entry: any) => entry.class)))
      .toEqual(new Set(contract.CONTINUATION_V7_ADMISSION_CLASSES));
    expect(policy.round6ReviewRejection.digest).toBe(rejection.digest);
    ...
    expect(policy.successor.featureDefinition.verification).toContain(
      policyBuilder.CONTINUATION_POLICY_V7_SUCCESSOR_CHECKER_VERIFICATION,
    );
    expect(policy.successor.featureDefinition.verification.some(
      (entry: string) =>
        entry.includes("check-conversation-disclosure-continuation-v6.mjs"),
    )).toBe(false);
  });
  ...
  test("rejects an incomplete finalized roster before policy construction", async () => {
    ...
    feature.files = feature.files.filter(
      (entry: string) => entry !== contract.CONTINUATION_V7_POLICY_PATH,
    );

    expect(() =>
      policyBuilder.createConversationDisclosureContinuationPolicyV7(input))
      .toThrow(`P107A V7 roster misses required path: ${
  ...
  test("rebinds exactly six present and three absent bookkeeping paths", async () => {
    ...
    const absent: string[] = [];
    const rebound = await policyBuilder.rebindRound7BookkeepingBaselinesV7(
      source,
      {
    ...
    expect(present.sort()).toEqual([
      ...policyBuilder.CONTINUATION_POLICY_V7_PRESENT_BOOKKEEPING_PATHS,
    ].sort());
    expect(absent.sort()).toEqual([
      ...policyBuilder.CONTINUATION_POLICY_V7_ABSENT_BOOKKEEPING_PATHS,
    ].sort());
  ...
  test("publishes the deterministic rejection witness privately and idempotently", async () => {
    const fixture = await createRound6Fixture();
    const outputPath =
  ...
  test("rejects Round6 receipt byte drift before creating a witness", async () => {
    const fixture = await createRound6Fixture();
    const receiptPath = path.join(
      fixture,
      contract.CONTINUATION_V7_ROUND6_RECEIPT_TRUST_ROOTS[0].path,
    );
    await appendFile(receiptPath, "\n");

    await expect(
      rejectionBuilder.buildConversationDisclosureReviewRejectionV7({
        repositoryRoot: fixture,
      }),
...
async function createRound6Fixture() {
  ...
  const required = [
    contract.CONTINUATION_V7_ROUND6_POLICY_TRUST_ROOT.path,
    contract.CONTINUATION_V7_ROUND6_SNAPSHOT_TRUST_ROOT.path,
    ...contract.CONTINUATION_V7_ROUND6_RECEIPT_TRUST_ROOTS.map(
