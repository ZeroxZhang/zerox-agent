...
function buildFixture() {
  const policy = {
    ...
    round4ReviewRejection: { digest: digest("3") },
    successor: {
    ...
    continuationExecutables: [{
      kind: "checker",
      path: "scripts/check-conversation-disclosure-continuation-v5.mjs",
      sha256: digest("6"),
    }],
  };
  const snapshot = {
  ...
  const dispatchSet = ["contract", "runtime", "governance"].map(
    (lane, index) => ({
      ...
      taskPath: `round5/${lane}`,
      agentLabel: `round5-${lane}-reviewer`,
      transport: "codex-collaboration",
    }),
  );
  const policyReference = {
    path: contract.CONTINUATION_V5_POLICY_PATH,
    byteSha256: digest("a"),
    canonicalDigest: policy.digest,
  };
  const snapshotReference = {
    path: contract.CONTINUATION_V5_REVIEW_SNAPSHOT_PATH,
    byteSha256: digest("b"),
    canonicalDigest: snapshot.digest,
  };
  const rejectionReference = {
    path: contract.CONTINUATION_V5_ROUND4_REVIEW_REJECTION_PATH,
    byteSha256: digest("c"),
  ...
  const runnerReference = {
    path: "scripts/verify-conversation-disclosure-continuation-v5.mjs",
    sha256: digest("d"),
  };
  const pendingBase = {
    schemaVersion: 5,
    kind: contract.CONTINUATION_V5_MANIFEST_KIND,
    ...
    round4ReviewRejection: rejectionReference,
    reviewReceipts: reviewReferences,
    callerDispatchSet: dispatchSet,
    validator: validatorReference,
    externalRunner: runnerReference,
    externalAttestation: {
      path: contract.CONTINUATION_V5_EXTERNAL_ATTESTATION_PATH,
      canonicalDigest: null,
    },
  };
  const pendingManifestDigest =
    contract.pendingManifestDigestV5(pendingBase);
  const pendingManifest = contract.withCanonicalDigestV5({
    ...pendingBase,
  ...
  const attestation = contract.withCanonicalDigestV5({
    schemaVersion: 5,
    kind: contract.CONTINUATION_V5_ATTESTATION_KIND,
    status: "passed",
    governancePhase: "review_post_transition",
    identityAssurance: contract.CONTINUATION_V5_IDENTITY_ASSURANCE,
    reviewAssurance: contract.CONTINUATION_V5_CALLER_DISPATCH_ASSURANCE,
    repositoryRealpath: "/tmp/round5-subject",
    completedAt: "2026-08-24T12:10:00.000Z",
    parentEvidenceBundleDigest: policy.parentEvidence.bundleDigest,
    pendingManifestDigest,
    policyDigest: policy.digest,
    snapshotDigest: snapshot.digest,
    round4ReviewRejectionDigest: rejection.digest,
    validatorDigest: validatorReference.sha256,
    runnerDigest: runnerReference.sha256,
    callerDispatchSet: dispatchSet,
    candidateResults,
  });
  const finalManifest = contract.withCanonicalDigestV5(
    contract.finalManifestProjectionV5(pendingManifest, attestation.digest),
  );
  const anchor = contract.withCanonicalDigestV5({
    schemaVersion: 5,
    kind: contract.CONTINUATION_V5_ANCHOR_KIND,
    identityAssurance: contract.CONTINUATION_V5_IDENTITY_ASSURANCE,
    reviewAssurance: contract.CONTINUATION_V5_CALLER_DISPATCH_ASSURANCE,
    repositoryRealpath: "/tmp/round5-subject",
  ...
  const manifestBindings = {
    ...
    policyReference,
    snapshotReference,
    round4ReviewRejectionReference: rejectionReference,
    reviewReferences,
    callerDispatchSet: dispatchSet,
    validatorReference,
    runnerReference,
...
function rehash<T extends Record<string, any>>(
  ...
  delete clone.digest;
  clone.digest = contract.hashCanonicalV5(clone);
  return clone as T & { digest: string };
}

describe("conversation disclosure final evidence v5", () => {
  test("accepts one completely bound pending/final/attestation/anchor chain", () => {
    const fixture = buildFixture();
    expect(contract.validateContinuationClosureManifestV5(
      fixture.pendingManifest,
      fixture.manifestBindings,
    )).toEqual([]);
    expect(contract.validateContinuationExternalAttestationV5(
      fixture.attestation,
    ...
    expect(contract.validateContinuationExternalAnchorV5(
      fixture.anchor,
  ...
  test("rejects every stale attestation semantic root", () => {
    ...
    dispatch[0].taskPath = "round5/other";
    const attestation = rehash({
      ...fixture.attestation,
    ...
    for (const mutation of [
      ...
      },
      {
        round4ReviewRejection: {
          ...fixture.finalManifest.round4ReviewRejection,
          canonicalDigest: digest("0"),
        },
      },
      ...
      expect(contract.validateContinuationClosureManifestV5(manifest, {
        ...fixture.manifestBindings,
        pendingManifest: fixture.pendingManifest,
        externalAttestation: fixture.attestation,
      })).not.toEqual([]);
  ...
  test("rejects stale anchor roots, future time, and missing bindings", () => {
    ...
    for (const key of [
      ...
      expect(contract.validateContinuationExternalAnchorV5(rootMutant, {
        ...fixture.anchorBindings,
    ...
    expect(contract.validateContinuationExternalAnchorV5(dispatchMutant, {
      ...fixture.anchorBindings,
    ...
    expect(contract.validateContinuationExternalAnchorV5(future, {
      ...fixture.anchorBindings,
