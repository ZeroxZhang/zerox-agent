...
function makeArchive() {
  ...
  delete value.digest;
  value.digest = contract.hashCanonicalV12(value);
  return value;
}
...
function makePolicyInput(rejection: any) {
  ...
  const feature = featureList.features.find(
    (entry: any) => entry.id === contract.CONTINUATION_V12_FEATURE_ID,
  );
  feature.files = [...new Set([
    ...feature.files,
    ...policyBuilder.CONTINUATION_POLICY_V12_REQUIRED_ROSTER_PATHS,
  ])];
  const continuationExecutables =
    contract.CONTINUATION_V12_EXECUTABLE_KINDS.map(
...
describe("conversation disclosure continuation policy v12 builder", () => {
  test("builds one exact six-class policy from stable live definitions", async () => {
    const rejection =
      (await rejectionBuilder.buildConversationDisclosureReviewRejectionV12({
        repositoryRoot: root,
      })).witness;
    const input = makePolicyInput(rejection);
    const policy =
      policyBuilder.createConversationDisclosureContinuationPolicyV12(input);

    expect(contract.validateContinuationPolicyV12(policy, {
      expectedDigest: policy.digest,
      baselineArchive: input.baselineArchive,
    })).toEqual([]);
    expect(policy.admissionClassSet).toEqual(
      contract.CONTINUATION_V12_ADMISSION_CLASSES,
    );
    expect(new Set(policy.admissionCoverage.map((entry: any) => entry.class)))
      .toEqual(new Set(contract.CONTINUATION_V12_ADMISSION_CLASSES));
    expect(policy.round11ReviewRejection.digest).toBe(rejection.digest);
    ...
    expect(policy.successor.featureDefinition.verification).toContain(
      policyBuilder.CONTINUATION_POLICY_V12_SUCCESSOR_CHECKER_VERIFICATION,
    );
    expect(policy.successor.featureDefinition.verification.some(
      (entry: string) =>
        entry.includes("check-conversation-disclosure-continuation-v11.mjs"),
    )).toBe(false);
  });
  ...
  test("rejects an incomplete finalized roster before policy construction", async () => {
    ...
    feature.files = feature.files.filter(
      (entry: string) => entry !== contract.CONTINUATION_V12_POLICY_PATH,
    );

    expect(() =>
      policyBuilder.createConversationDisclosureContinuationPolicyV12(input))
      .toThrow(`P107A V12 roster misses required path: ${
  ...
  test("rebinds exactly six present and three absent bookkeeping paths", async () => {
    ...
    const absent: string[] = [];
    const rebound = await policyBuilder.rebindRound12BookkeepingBaselinesV12(
      source,
      {
    ...
    expect(present.sort()).toEqual([
      ...policyBuilder.CONTINUATION_POLICY_V12_PRESENT_BOOKKEEPING_PATHS,
    ].sort());
    expect(absent.sort()).toEqual([
      ...policyBuilder.CONTINUATION_POLICY_V12_ABSENT_BOOKKEEPING_PATHS,
    ].sort());
  ...
  test("publishes the deterministic rejection witness privately and idempotently", async () => {
    const fixture = await createRound11Fixture();
    const outputPath =
  ...
  test("rejects Round11 receipt byte drift before creating a witness", async () => {
    const fixture = await createRound11Fixture();
    const receiptPath = path.join(
      fixture,
      contract.CONTINUATION_V12_ROUND11_RECEIPT_TRUST_ROOTS[0].path,
    );
    await appendFile(receiptPath, "\n");

    await expect(
      rejectionBuilder.buildConversationDisclosureReviewRejectionV12({
        repositoryRoot: fixture,
      }),
...
async function createRound11Fixture() {
  ...
  const required = [
    contract.CONTINUATION_V12_ROUND11_POLICY_TRUST_ROOT.path,
    contract.CONTINUATION_V12_ROUND11_SNAPSHOT_TRUST_ROOT.path,
    ...contract.CONTINUATION_V12_ROUND11_RECEIPT_TRUST_ROOTS.map(
