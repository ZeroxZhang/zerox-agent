...
const governance = await import(
  ...
  "../../scripts/conversation-disclosure-program-governance-v2.mjs"
);
...
describe("conversation disclosure program governance v2", () => {
  it("accepts the closed-world P107A admission lifecycle and reports every rule", () => {
    ...
    expect(result.ruleResults.map((rule: RuleResult) => rule.id)).toEqual(
      governance.PROGRAM_GOVERNANCE_V2_RULE_IDS,
    );
  ...
  it.each([
    {
      ...
      rule: "CDG2-001-input-contract",
      mutate(input: GovernanceInput) {
        input.closedWorld.schemaVersion = 3;
      },
    },
    {
      name: "program shape",
      rule: "CDG2-010-program-shape",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG2-011-program-root-contract",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG2-041-scenario-categories",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG2-042-scenario-coverage",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG2-050-workstream-schema",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG2-052-dependency-graph",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG2-070-implementation-gates",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG2-090-p107a-p108-lifecycle",
      mutate(input: GovernanceInput) {
    ...
    },
    {
      ...
      rule: "CDG2-100-cd03-evidence-refs",
      mutate(input: GovernanceInput) {
  ...
  it("rejects an unknown required rule id", () => {
    const input = createInput();
    input.closedWorld.ruleIds.push("CDG2-666-candidate-rule");

    expect(ruleResult(validate(input), "CDG2-001-input-contract").message).toContain(
      "closedWorld.ruleIds must exactly match the known v2 ledger",
    );
  });
  ...
  it("rejects an unknown workstream even when its fields and dependency are valid", () => {
    ...
    expect(ruleResult(validate(input), "CDG2-051-closed-roster").status).toBe("failed");
  });
  ...
  it("rejects P108 done until CD04 publishes a next-version reviewed delta trust head", () => {
    ...
    expect(ruleResult(
      validate(input),
      "CDG2-090-p107a-p108-lifecycle",
    ).message).toContain("unknown continuation lifecycle phase");
  });
});

function validate(input: GovernanceInput) {
  return governance.validateConversationDisclosureProgramGovernanceV2(input);
}
...
function createInput() {
  ...
  const workstreams = [
    ...
    createWorkstream("CD08", "P112-v3.9.2-disclosure-hardening", "planned", ["CD07"], ["D1"]),
    createWorkstream("CD09", "P113-v3.9.2-disclosure-adversarial-acceptance", "planned", ["CD08"], ["D1"]),
  ];
  ...
  const programRootBinding = governance.createProgramRootBindingV2(program);
  return {
    program,
    ...
    closedWorld: {
      schemaVersion: 2,
      ...programRootBinding,
      ruleIds: [...governance.PROGRAM_GOVERNANCE_V2_RULE_IDS],
...
type GovernanceResult = ReturnType<typeof governance.validateConversationDisclosureProgramGovernanceV2>;
