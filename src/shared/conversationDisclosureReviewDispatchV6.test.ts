...
function fixture() {
  return {
    policy: {
      digest: digest("a"),
      round5ReviewRejection: { digest: digest("b") },
    },
    snapshot: {
      digest: digest("c"),
...
describe("conversation disclosure review dispatch v6", () => {
  test("hashes the exact published instruction bytes including final newline", () => {
    const result = builder.buildReviewDispatchArtifactsV6(fixture());
  ...
  test("rejects missing, malformed, or duplicate challenges", () => {
    ...
    for (const challenges of [
      ...
      expect(() => builder.buildReviewDispatchArtifactsV6({
        ...valid,
        challenges,
  ...
  test("publishes every instruction and the dispatch set through private no-replace I/O", () => {
    expect(source).toContain("publishPrivateExactV6(");
