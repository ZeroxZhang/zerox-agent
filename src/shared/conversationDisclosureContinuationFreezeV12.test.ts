...
const freezer = await import(
  ...
  "../../scripts/freeze-conversation-disclosure-continuation-v12.mjs"
);
...
describe("conversation disclosure continuation freezer v12", () => {
  test("requires a caller-fixed canonical freeze time", async () => {
    await expect(freezer.freezeConversationDisclosureContinuationV12({
  ...
  test("captures every admission category in one postflight ledger", () => {
    ...
    expect(source).toContain("captureRequiredAbsentV12");
    expect(source).toContain("postflightCaptureLedgerV12(ledger)");
    expect(source.indexOf("postflightCaptureLedgerV12(ledger)"))
      .toBeLessThan(source.indexOf("publishPrivateExactV12(outputAbsolute"));
  });

  test("uses the shared transition and snapshot validators", () => {
    expect(source).toContain("validateGovernanceTransitionStateV12(");
    expect(source).toContain("validateContinuationReviewSnapshotV12(");
    expect(source).toContain(
      "output must be the exact Round12 review snapshot path",
    );
  });
});
