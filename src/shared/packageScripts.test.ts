...
describe("package scripts", () => {
  ...
  it("requires final acceptance to run from a caller-pinned external copy", () => {
    ...
    expect(runner).toContain('"host-toolchain-command-failure-isolation"');
    expect(runner).toContain('"host-toolchain-electron-success-isolation"');
    expect(runner).toContain('"host-toolchain-electron-failure-isolation"');
    expect(runner).not.toContain("const xcrunEphemeralPrefixes =");
    expect(runner).not.toContain("const xcrunCacheFiles =");
    expect(runner).not.toContain('path.join(electronTemp, "xcrun_db-")');
    expect(runner).not.toContain("writableFiles:");
    expect(runner).not.toContain("writableFiles = []");
