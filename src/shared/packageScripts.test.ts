...
describe("package scripts", () => {
  ...
  it("requires final acceptance to run from a caller-pinned external copy", () => {
    ...
    expect(runner).toContain("await verifyHostXcrunCacheIsolation({");
    expect(runner).toContain('process.argv[2] === "--self-test-host-xcrun-isolation"');
    expect(runner).toContain('"host-xcrun-cache-success-isolation"');
    expect(runner).toContain('"host-xcrun-cache-failure-isolation"');
