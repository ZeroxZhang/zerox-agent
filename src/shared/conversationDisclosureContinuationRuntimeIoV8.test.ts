...
describe("conversation disclosure continuation runtime I/O v8", () => {
  it("captures one stable file and validates the complete ledger postflight", async () => {
    ...
    const ledger = runtimeIo.createCaptureLedgerV8();

    const capture = await runtimeIo.captureStableFileV8(filePath, "stable input", {
      expectedRoot: root,
      ledger,
    });

    expect(capture.bytes.equals(bytes)).toBe(true);
    expect(capture.digest).toBe(runtimeIo.sha256BytesV8(bytes));
    expect(capture.identity.nlink).toBe(1n);
    expect(ledger.entries).toHaveLength(1);
    expect(await runtimeIo.postflightCaptureLedgerV8(ledger)).toEqual({
      status: "passed",
      captureCount: 1,
    });
  });
  ...
  it("requires private evidence to be an owned 0600 single-link file", async () => {
    ...
    const captured = await runtimeIo.capturePrivateEvidenceV8(
      evidencePath,
      "private evidence",
      { expectedRoot: root },
    );
    ...
    await expect(runtimeIo.capturePrivateEvidenceV8(
      evidencePath,
      "private evidence",
      { expectedRoot: root },
    )).rejects.toMatchObject({ code: "RUNTIME_IO_V8_MODE" });
  });
  ...
  it("throws for every present required-absent leaf type", async () => {
    ...
    for (const presentPath of [filePath, directoryPath, symlinkPath]) {
      await expect(runtimeIo.captureRequiredAbsentV8(
        presentPath,
        "required absent output",
        { expectedRoot: root },
      )).rejects.toMatchObject({ code: "RUNTIME_IO_V8_EXPECTED_ABSENT" });
    }
  });
  ...
  it("records required absence and rejects a postflight appearance", async () => {
    ...
    await runtimeIo.captureRequiredAbsentV8(outputPath, "future output", {
      expectedRoot: root,
      ledger,
    });
    expect(ledger.entries).toHaveLength(1);
    ...
    await expect(runtimeIo.postflightCaptureLedgerV8(ledger))
      .rejects.toMatchObject({ code: "RUNTIME_IO_V8_APPEARED" });
  });
  ...
  it("rejects postflight mode drift", async () => {
    ...
    const ledger = runtimeIo.createCaptureLedgerV8();
    await runtimeIo.captureStableFileV8(filePath, "mode input", {
      expectedRoot: root,
      ledger,
    });
    ...
    await expect(runtimeIo.postflightCaptureLedgerV8(ledger))
      .rejects.toMatchObject({ code: "RUNTIME_IO_V8_CAPTURE_DRIFT" });
  });
  ...
  it("rejects postflight inode replacement even when bytes are unchanged", async () => {
    ...
    await writeFile(filePath, bytes, { mode: 0o644 });
    const ledger = runtimeIo.createCaptureLedgerV8();
    await runtimeIo.captureStableFileV8(filePath, "inode input", {
      expectedRoot: root,
      ledger,
    });
  ...
  it("rejects postflight hard-link drift", async () => {
    ...
    const ledger = runtimeIo.createCaptureLedgerV8();
    await runtimeIo.captureStableFileV8(filePath, "linked input", {
      expectedRoot: root,
      ledger,
    });
    ...
    await expect(runtimeIo.postflightCaptureLedgerV8(ledger))
      .rejects.toMatchObject({ code: "RUNTIME_IO_V8_INVALID_LEAF" });
  });
  ...
  it("rejects postflight parent identity replacement", async () => {
    ...
    const ledger = runtimeIo.createCaptureLedgerV8();
    await runtimeIo.captureStableFileV8(filePath, "parent-bound input", {
      expectedRoot: root,
      ledger,
    });
    ...
    await writeFile(filePath, "authority\n", { mode: 0o644 });
    await expect(runtimeIo.postflightCaptureLedgerV8(ledger))
      .rejects.toMatchObject({ code: "RUNTIME_IO_V8_PARENT_DRIFT" });
  });
  ...
  it("publishes exact private bytes once and reruns idempotently", async () => {
    ...
    const first = await runtimeIo.publishPrivateExactV8(outputPath, bytes, {
      expectedRoot: root,
      label: "runtime receipt",
    });
    const firstStat = await stat(outputPath);
    const second = await runtimeIo.publishPrivateExactV8(outputPath, bytes, {
      expectedRoot: root,
      label: "runtime receipt",
    });
    ...
    expect(first.digest).toBe(runtimeIo.sha256BytesV8(bytes));
  ...
  it("recovers deterministic private publication temps and linked crash pairs", async () => {
    ...
    for (const state of ["temp_only", "linked_pair"]) {
      ...
      const temporaryPath = path.join(
        root,
        `.${basename}.${runtimeIo.sha256BytesV8(bytes).slice(7, 31)}.tmp`,
      );
      await writeFile(temporaryPath, bytes, { mode: 0o600 });
      if (state === "linked_pair") await link(temporaryPath, outputPath);

      const result = await runtimeIo.publishPrivateExactV8(outputPath, bytes, {
        expectedRoot: root,
        label: state,
      });
  ...
  it("rejects partial, different, non-private, and symlink publication states", async () => {
    ...
    for (const existingPath of [partialPath, differentPath]) {
      await expect(runtimeIo.publishPrivateExactV8(existingPath, expected, {
        expectedRoot: root,
      })).rejects.toMatchObject({ code: "RUNTIME_IO_V8_THIRD_STATE" });
    }
    await expect(runtimeIo.publishPrivateExactV8(publicPath, expected, {
      expectedRoot: root,
    })).rejects.toMatchObject({ code: "RUNTIME_IO_V8_MODE" });
    await expect(runtimeIo.publishPrivateExactV8(symlinkPath, expected, {
      expectedRoot: root,
    })).rejects.toMatchObject({ code: "RUNTIME_IO_V8_ROOT_ESCAPE" });
  });
});

async function createRoot() {
  const created = await mkdtemp(path.join(os.tmpdir(), "runtime-io-v8-"));
