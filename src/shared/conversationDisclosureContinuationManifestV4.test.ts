...
describe("conversation disclosure continuation manifest v4", () => {
  test("requires external dispatch, policy, and snapshot caller pins", async () => {
    await expect(
      manifestBuilder.buildConversationDisclosureContinuationManifestV4({
  ...
  test("derives one pending digest across pending and final forms", () => {
    ...
    const pendingDigest = contract.pendingManifestDigestV4(base);
    const final = {
      ...base,
    ...
    expect(contract.pendingManifestDigestV4(final)).toBe(pendingDigest);
  });
