...
describe("conversation disclosure test orchestrator v12", () => {
  ...
  test("executes immutable V3, V4, and V7 through V11 tests in reconstructed historical states", () => {
    ...
    expect(source).toContain("restoreRound10Semantics");
    expect(source).toContain("restoreRound11Semantics");
    ...
    expect(source).toContain("historicalV10Tests");
    expect(source).toContain("historicalV11Tests");
  ...
  test("executes Program and package tests against all four Round12 targets", () => {
    expect(source).toContain("applyRound12Targets");
    expect(source).toContain("CD03A-round12-package.target.json");
    expect(source).toContain("CD03A-round12-harness.target.mjs");
    expect(source).toContain("CD03A-round12-program-test.target.ts");
    expect(source).toContain("CD03A-round12-package-scripts-test.target.ts");
  });

  test("reconstructs the all-from current lane when invoked from all-to", async () => {
    const fixture = await mkdtemp(path.join(os.tmpdir(), "zerox-r11-state-"));
    ...
    for (const transition of orchestrator.ROUND12_TRANSITION_MAPPINGS) {
      await copyFixtureFile(transition.source, fixture);
      await copyFixtureFile(transition.destination, fixture);
    }
    await expect(
      orchestrator.detectRound12TransitionState(fixture),
    ...
    await orchestrator.restoreCurrentRound12Sources(fixture);
    await expect(
      orchestrator.detectRound12TransitionState(fixture),
    ).resolves.toBe("source");
  });
