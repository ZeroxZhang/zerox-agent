...
const runnerPath = path.join(
  process.cwd(),
  "scripts/verify-conversation-disclosure-continuation-v6.mjs",
);
const source = readFileSync(runnerPath, "utf8");

describe("conversation disclosure continuation runner v6", () => {
  test("is self-contained and rejects unpinned execution", async () => {
    ...
    await expect(execFileAsync(process.execPath, [runnerPath], {
      ...
      stderr: expect.stringContaining("missing required runner v6 option"),
    });
  });
  ...
  test("rejects preload state and binds V6 assurance fields", () => {
    expect(source).toContain("rejectPreloadEnvironment()");
    expect(source).toContain("ZEROX_CD03A_RUNNER_V6_TEST_FAULT");
  ...
  test("hard-roots all six classes and the rejected Round5 subject", () => {
    expect(source).toContain('"rejected_output_absent"');
    expect(source).toContain("ROUND5_POLICY_ROOT");
    expect(source).toContain("ROUND5_SNAPSHOT_ROOT");
    expect(source).toContain("ROUND5_RECEIPT_ROOTS");
    expect(source).toContain("ROUND5_FINDING_SET_DIGEST");
  });
});
