...
function buildFixture() {
  const policy = {
    ...
    round6ReviewRejection: { digest: digest("3") },
    successor: {
    ...
    continuationExecutables: [{
      kind: "checker",
      path: "scripts/check-conversation-disclosure-continuation-v7.mjs",
      sha256: digest("6"),
    }],
  };
  const snapshot = {
  ...
  const dispatchSet = ["contract", "runtime", "governance"].map(
    (lane, index) => ({
      ...
      taskPath: `round7/${lane}`,
      agentLabel: `round7-${lane}-reviewer`,
      transport: "codex-collaboration",
    }),
  );
  const policyReference = {
    path: contract.CONTINUATION_V7_POLICY_PATH,
    byteSha256: digest("a"),
    canonicalDigest: policy.digest,
  };
  const snapshotReference = {
    path: contract.CONTINUATION_V7_REVIEW_SNAPSHOT_PATH,
    byteSha256: digest("b"),
    canonicalDigest: snapshot.digest,
  };
  const rejectionReference = {
    path: contract.CONTINUATION_V7_ROUND6_REVIEW_REJECTION_PATH,
    byteSha256: digest("c"),
  ...
  const runnerReference = {
    path: "scripts/verify-conversation-disclosure-continuation-v7.mjs",
    sha256: digest("d"),
  };
  const pendingBase = {
    schemaVersion: 7,
    kind: contract.CONTINUATION_V7_MANIFEST_KIND,
    ...
    round6ReviewRejection: rejectionReference,
    reviewReceipts: reviewReferences,
    callerDispatchSet: dispatchSet,
    validator: validatorReference,
    externalRunner: runnerReference,
    externalAttestation: {
      path: contract.CONTINUATION_V7_EXTERNAL_ATTESTATION_PATH,
      canonicalDigest: null,
    },
  };
  const pendingManifestDigest =
    contract.pendingManifestDigestV7(pendingBase);
  const pendingManifest = contract.withCanonicalDigestV7({
    ...pendingBase,
  ...
  const attestation = contract.withCanonicalDigestV7({
    schemaVersion: 7,
    kind: contract.CONTINUATION_V7_ATTESTATION_KIND,
    status: "passed",
    governancePhase: "review_post_transition",
    identityAssurance: contract.CONTINUATION_V7_IDENTITY_ASSURANCE,
    reviewAssurance: contract.CONTINUATION_V7_CALLER_DISPATCH_ASSURANCE,
    repositoryRealpath: "/tmp/round7-subject",
  ...
  const finalManifest = contract.withCanonicalDigestV7(
    contract.finalManifestProjectionV7(pendingManifest, attestation.digest),
  );
  const anchor = contract.withCanonicalDigestV7({
    schemaVersion: 7,
    kind: contract.CONTINUATION_V7_ANCHOR_KIND,
    identityAssurance: contract.CONTINUATION_V7_IDENTITY_ASSURANCE,
    reviewAssurance: contract.CONTINUATION_V7_CALLER_DISPATCH_ASSURANCE,
    repositoryRealpath: "/tmp/round7-subject",
  ...
  const manifestBindings = {
    ...
    policyReference,
    snapshotReference,
    round6ReviewRejectionReference: rejectionReference,
    reviewReferences,
    callerDispatchSet: dispatchSet,
    validatorReference,
    runnerReference,
...
function rehash<T extends Record<string, any>>(
  ...
  delete clone.digest;
  clone.digest = contract.hashCanonicalV7(clone);
  return clone as T & { digest: string };
}

describe("conversation disclosure final evidence v7", () => {
  test("accepts one completely bound pending/final/attestation/anchor chain", () => {
    const fixture = buildFixture();
    expect(contract.validateContinuationClosureManifestV7(
      fixture.pendingManifest,
      fixture.manifestBindings,
    )).toEqual([]);
    expect(contract.validateContinuationExternalAttestationV7(
      fixture.attestation,
    ...
    expect(contract.validateContinuationExternalAnchorV7(
      fixture.anchor,
  ...
  test("rejects every stale attestation semantic root", () => {
    ...
    dispatch[0].taskPath = "round7/other";
    const attestation = rehash({
      ...fixture.attestation,
    ...
    for (const mutation of [
      ...
      },
      {
        round6ReviewRejection: {
          ...fixture.finalManifest.round6ReviewRejection,
          canonicalDigest: digest("0"),
        },
      },
      ...
      expect(contract.validateContinuationClosureManifestV7(manifest, {
        ...fixture.manifestBindings,
        pendingManifest: fixture.pendingManifest,
        externalAttestation: fixture.attestation,
      })).not.toEqual([]);
  ...
  test("rejects stale anchor roots, future time, and missing bindings", () => {
    ...
    for (const key of [
      ...
      expect(contract.validateContinuationExternalAnchorV7(rootMutant, {
        ...fixture.anchorBindings,
    ...
    expect(contract.validateContinuationExternalAnchorV7(dispatchMutant, {
      ...fixture.anchorBindings,
    ...
    expect(contract.validateContinuationExternalAnchorV7(future, {
      ...fixture.anchorBindings,
