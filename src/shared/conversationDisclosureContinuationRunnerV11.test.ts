...
describe("conversation disclosure continuation runner v11", () => {
  test("is self-contained and rejects unpinned execution", async () => {
    ...
    await expect(execFileAsync(process.execPath, [runnerPath], {
      ...
      stderr: expect.stringContaining("missing required runner v11 option"),
    });
  });
  ...
  test("rejects preload state and binds V11 assurance fields", () => {
    expect(source).toContain("rejectPreloadEnvironment()");
    expect(source).toContain("ZEROX_CD03A_RUNNER_V11_TEST_FAULT");
  ...
  test("hard-roots all six classes and the rejected Round10 subject", () => {
    expect(source).toContain('"rejected_output_absent"');
    expect(source).toContain("ROUND10_POLICY_ROOT");
    expect(source).toContain("ROUND10_SNAPSHOT_ROOT");
    expect(source).toContain("ROUND10_RECEIPT_ROOTS");
    expect(source).toContain("ROUND10_FINDING_SET_DIGEST");
  });

  test("uses V11 round constants and atomic no-replace for absent publications", () => {
    expect(source).toContain("policy.round !== 11");
    expect(source).toContain("snapshot.round !== 11");
    expect(source).toContain("manifest.round !== 11");
    expect(source).toContain("archive.round !== 11");
    expect(source).toContain("rejection.rejectedRound !== 10");
    expect(source).toContain("receipt.round !== 11");
