...
describe("conversation disclosure test orchestrator v4", () => {
  ...
  test("executes Program and package tests against all four Round4 targets", () => {
    expect(source).toContain("applyRound4Targets");
    expect(source).toContain("CD03A-round4-package.target.json");
    expect(source).toContain("CD03A-round4-harness.target.mjs");
    expect(source).toContain("CD03A-round4-program-test.target.ts");
    expect(source).toContain("CD03A-round4-package-scripts-test.target.ts");
  });
});
