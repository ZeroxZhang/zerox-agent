...
const governance = await import(
  ...
  "../../scripts/conversation-disclosure-program-governance-v5.mjs"
);
...
describe("conversation disclosure program governance v5", () => {
  it("accepts the closed-world P107A admission lifecycle and reports every rule", () => {
    ...
    expect(result.kind).toBe(governance.GOVERNANCE_V5_RESULT_KIND);
    expect(result.errors).toEqual([]);
    expect(result.ruleResults.map((rule: RuleResult) => rule.id)).toEqual(
      governance.PROGRAM_GOVERNANCE_V5_RULE_IDS,
    );
  ...
  it("accepts only the exact all-from or all-to governance target state", () => {
    ...
    expect(contract.validateGovernanceTransitionStateV5(
      transitions,
  ...
  it("keeps the Round5 harness diagnostic-only by default and identity claims not-signed", () => {
    ...
    expect(contract.CONTINUATION_V5_CALLER_DISPATCH_ASSURANCE).toBe(
      "caller-attested-not-signed",
    );
    expect(contract.CONTINUATION_V5_IDENTITY_ASSURANCE).toBe("not-signed");
    expect(contract.CONTINUATION_V5_INDEPENDENCE_CLAIM).toBe(
      "caller-attested-distinct-review-contexts",
    );
  });

  it("changes the Round4 package target only for the V5 test orchestrator", () => {
    const round4 = JSON.parse(readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round4-package.target.json",
    ), "utf8"));
    const round5 = JSON.parse(readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round5-package.target.json",
    ), "utf8"));
    expect(round5).toEqual({
      ...round4,
      scripts: {
        ...round4.scripts,
  ...
  it("keeps pre-freeze target tests on the published Round4 baseline archive", () => {
    const target = readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round5-program-test.target.ts",
    ), "utf8");
    expect(target).toContain("CD03A-round4-baseline-archive.json");
    expect(target).not.toContain("CD03A-round5-baseline-archive.json");
  });
  ...
  it("rejects an unknown required rule id", () => {
    ...
    expect(ruleResult(validate(input), "CDG4-001-input-contract").message).toContain(
      "closedWorld.ruleIds must exactly match the known v5 ledger",
    );
  });
...
function validate(input: GovernanceInput) {
  return governance.validateConversationDisclosureProgramGovernanceV5(input);
}
...
function createInput() {
  ...
  const workstreams = [
    ...
    createWorkstream("CD08", "P112-v5.9.2-disclosure-hardening", "planned", ["CD07"], ["D1"]),
    createWorkstream("CD09", "P113-v5.9.2-disclosure-adversarial-acceptance", "planned", ["CD08"], ["D1"]),
  ];
  ...
  const program = {
      schemaVersion: 1,
      programId: "conversation-progressive-disclosure-v5.9.2-2026-08",
      status: "active",
      maxActiveFeatures: 1,
  ...
  const programRootBinding = governance.createProgramRootBindingV5(program);
  return {
    program,
    ...
    closedWorld: {
      schemaVersion: 5,
      ...programRootBinding,
      ruleIds: [...governance.PROGRAM_GOVERNANCE_V5_RULE_IDS],
...
function createTransitions() {
  return Object.entries(
    contract.CONTINUATION_V5_GOVERNANCE_TRANSITIONS as Record<string, string>,
  )
    ...
    .map(([livePath, kind], index) => ({
      ...
      kind,
      stagedTargetPath:
        `.zerox/verification/conversation-disclosure/CD03A-round5-target-${index}`,
...
type GovernanceResult = ReturnType<typeof governance.validateConversationDisclosureProgramGovernanceV5>;
