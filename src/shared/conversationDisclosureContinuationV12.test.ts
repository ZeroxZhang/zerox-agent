...
function rehash<T extends Record<string, unknown>>(value: T): T & { digest: string } {
  ...
  delete copy.digest;
  copy.digest = contract.hashCanonicalV12(copy);
  return copy as T & { digest: string };
}

function makeRound11Rejection() {
  return contract.withCanonicalDigestV12({
    schemaVersion: 12,
    kind: contract.CONTINUATION_V12_REVIEW_REJECTION_KIND,
    algorithm: contract.CONTINUATION_V12_ALGORITHM,
    ...
    rejectedRound: contract.CONTINUATION_V12_REJECTED_ROUND,
    recoveryRound: contract.CONTINUATION_V12_ROUND,
    status: "rejected_after_review",
    sourcePolicy: structuredClone(
      contract.CONTINUATION_V12_ROUND11_POLICY_TRUST_ROOT,
    ),
    sourceSnapshot: structuredClone(
      contract.CONTINUATION_V12_ROUND11_SNAPSHOT_TRUST_ROOT,
    ),
    completedReceipts: contract.CONTINUATION_V12_ROUND11_RECEIPT_TRUST_ROOTS.map(
      (entry: Record<string, unknown>) => structuredClone(entry),
    ),
    findingIds: [...contract.CONTINUATION_V12_ROUND11_FINDING_IDS],
    findingSetDigest: contract.CONTINUATION_V12_ROUND11_FINDING_SET_DIGEST,
    aggregateFindingCounts: structuredClone(
      contract.CONTINUATION_V12_ROUND11_AGGREGATE_FINDING_COUNTS,
    ),
    repositoryForbiddenOutputs: [
      ...contract.CONTINUATION_V12_ROUND11_REPOSITORY_FORBIDDEN_OUTPUT_PATHS,
    ],
    externalAnchorRule: contract.expectedRejectedRound11AnchorRuleV12(),
    priorRejections: {
      round1CanonicalDigest: contract.CONTINUATION_V12_ROUND1_REJECTION_DIGEST,
      round2: structuredClone(
        contract.CONTINUATION_V12_ROUND2_REJECTION_TRUST_ROOT,
      ),
    },
  });
}
...
function makeArchive() {
  ...
  return rehash({
    ...source,
    schemaVersion: 12,
    kind: contract.CONTINUATION_V12_BASELINE_ARCHIVE_KIND,
    round: 12,
  });
}
...
function makePolicy() {
  ...
  const rejection = makeRound11Rejection();
  const archive = makeArchive();
  const admission = structuredClone(source.admission);
  const transitions = contract.CONTINUATION_V12_GOVERNANCE_TRANSITION_TRUST_ROOTS
    .map((entry: Record<string, unknown>) => structuredClone(entry));
  admission.reviewOutputPaths = [
    ...contract.CONTINUATION_V12_REVIEW_OUTPUT_PATHS,
  ];
  admission.featureDefinition.files = [...new Set([
    ...admission.featureDefinition.files,
    contract.CONTINUATION_V12_ROUND11_REVIEW_REJECTION_PATH,
    contract.CONTINUATION_V12_BASELINE_ARCHIVE_PATH,
    contract.CONTINUATION_V12_POLICY_PATH,
    ...Object.values(
      contract.CONTINUATION_V12_EXECUTABLE_PATH_BY_KIND as Record<string, string>,
    ),
    ...transitions.map((entry: any) => entry.stagedTargetPath),
    ...contract.CONTINUATION_V12_REVIEW_OUTPUT_PATHS,
    ...contract.CONTINUATION_V12_REJECTED_OUTPUT_ABSENT_PATHS,
  ])].sort();
  admission.featureDefinitionDigest = contract.hashCanonicalV12(
    admission.featureDefinition,
  );
  admission.featureFileSetDigest = contract.hashCanonicalV12(
    admission.featureDefinition.files,
  );
  const admissionCoverage = contract.buildAdmissionCoverageV12(
    admission,
    transitions,
  );
  admission.reviewCoverageDigest = contract.hashCanonicalV12(admissionCoverage);
  const continuationExecutables = contract.CONTINUATION_V12_EXECUTABLE_KINDS.map(
    (kind: string, index: number) => ({
      kind,
      path: contract.CONTINUATION_V12_EXECUTABLE_PATH_BY_KIND[kind],
  ...
  return contract.withCanonicalDigestV12({
    schemaVersion: 12,
    kind: contract.CONTINUATION_V12_POLICY_KIND,
    algorithm: contract.CONTINUATION_V12_ALGORITHM,
    policyId: contract.CONTINUATION_V12_POLICY_ID,
    ...
    admission,
    admissionClassSet: [...contract.CONTINUATION_V12_ADMISSION_CLASSES],
    admissionClassSetDigest:
      contract.CONTINUATION_V12_ADMISSION_CLASS_SET_DIGEST,
    admissionCoverage,
    ...
    baselineArchive: {
      path: contract.CONTINUATION_V12_BASELINE_ARCHIVE_PATH,
      digest: archive.digest,
      entrySetDigest: archive.entrySetDigest,
    },
    reviewSnapshot: { path: contract.CONTINUATION_V12_REVIEW_SNAPSHOT_PATH },
    reviewAssurancePolicy: {
      callerDispatchAssurance:
        contract.CONTINUATION_V12_CALLER_DISPATCH_ASSURANCE,
      identityAssurance: contract.CONTINUATION_V12_IDENTITY_ASSURANCE,
      independenceClaim: contract.CONTINUATION_V12_INDEPENDENCE_CLAIM,
      localIdentityProof: false,
    },
    externalAnchorPolicy: contract.expectedRejectedRound11AnchorRuleV12(),
...
function makeSnapshot(policy: any) {
  ...
  for (const reference of [
    contract.CONTINUATION_V12_ROUND11_POLICY_TRUST_ROOT,
    contract.CONTINUATION_V12_ROUND11_SNAPSHOT_TRUST_ROOT,
    ...contract.CONTINUATION_V12_ROUND11_RECEIPT_TRUST_ROOTS,
  ]) {
    frozen.get(reference.path)!.sha256 = reference.byteSha256;
  }
  frozen.get(
    contract.CONTINUATION_V12_ROUND11_REVIEW_REJECTION_PATH,
  )!.sha256 = contract.sha256BytesV12(
    contract.serializeRound11ReviewRejectionV12(
      policy.round11ReviewRejection,
    ),
  );
  ...
  return contract.withCanonicalDigestV12({
    schemaVersion: 12,
    kind: contract.CONTINUATION_V12_SNAPSHOT_KIND,
    algorithm: contract.CONTINUATION_V12_ALGORITHM,
    ...
    pathAuthorityDigest: contract.hashCanonicalV12(policy.pathAuthorities),
...
function makeReceipt(policy: any, snapshot: any) {
  const dispatchEntry = {
    lane: "contract",
    assurance: contract.CONTINUATION_V12_CALLER_DISPATCH_ASSURANCE,
  ...
  return {
    schemaVersion: 12,
    kind: contract.CONTINUATION_V12_RECEIPT_KIND,
    programId: policy.programId,
    workstreamId: contract.CONTINUATION_V12_WORKSTREAM_ID,
    featureId: contract.CONTINUATION_V12_FEATURE_ID,
    round: contract.CONTINUATION_V12_ROUND,
    ...
    callerDispatchEntryDigest: contract.hashCanonicalV12(dispatchEntry),
    callerDispatchSetDigest: contract.hashCanonicalV12(dispatchSet),
    claimedReviewOrigin: {
      taskPath: "caller/contract",
      agentLabel: "claimed-reviewer",
      transport: "codex-collaboration",
...
describe("conversation disclosure continuation contract v12", () => {
  test("binds the real runtime I/O API instead of the abandoned adapter", () => {
    expect(contract.validateRuntimeIoCaptureAdapterV12(runtimeIo)).toEqual([]);
    expect(contract.CONTINUATION_V12_RUNTIME_IO_INTERFACE.methods).not.toContain(
      "capturePresent",
    );
  });

  test("validates the exact Round11 rejection and both roots of every source", () => {
    const witness = makeRound11Rejection();
    expect(contract.validateRound11ReviewRejectionV12(witness)).toEqual([]);
    const references = [
    ...
    for (const reference of references) {
      ...
      expect(contract.validateRound11ReviewRejectionV12(rehash(mutant)))
        .not.toEqual([]);
    }
  });
  ...
  test("hard-roots the exact ordered six-class set", () => {
    ...
    expect(contract.validateContinuationPolicyV12(policy)).toEqual([]);
    for (const classSet of [
      ...
      mutant.admissionClassSetDigest = contract.hashCanonicalV12(classSet);
      expect(contract.validateContinuationPolicyV12(rehash(mutant)))
  ...
  test("rejects policy key omission and extension", () => {
    ...
    expect(contract.validateContinuationPolicyV12(omitted))
      .toContain("continuation policy must contain the exact V12 keys");
    const extended = { ...policy, candidateField: true };
    expect(contract.validateContinuationPolicyV12(extended))
      .toContain("continuation policy must contain the exact V12 keys");
  });
  ...
  test("binds policy programId to the closed-world Program root", () => {
    ...
    expect(contract.validateContinuationPolicyV12(mutant, {
  ...
  test("binds the predecessor rejection programId to the continuation policy", () => {
    ...
    expect(contract.validateContinuationPolicyV12(mutant, {
      ...
      "Round11 rejection programId differs from the continuation policy",
    );
  });

  test("validates the V12 archive and snapshot bindings", () => {
    const policy = makePolicy();
    const archive = makeArchive();
    const snapshot = makeSnapshot(policy);
    expect(contract.validateBaselineArchiveV12(archive, policy)).toEqual([]);
    expect(contract.validateContinuationReviewSnapshotV12(snapshot, policy, {
      verifierNow: Date.parse(snapshot.frozenAt),
    })).toEqual([]);
    const mutant = rehash({
      ...snapshot,
      round11ReviewRejectionDigest: zeroDigest,
    });
    expect(contract.validateContinuationReviewSnapshotV12(mutant, policy, {
      verifierNow: Date.parse(snapshot.frozenAt),
    })).toContain(
      "continuation review snapshot round11ReviewRejectionDigest binding is stale",
    );
  });
  ...
  test("rejects stronger identity assurance in policy and receipts", () => {
    ...
    expect(contract.validateContinuationPolicyV12(rehash(policyMutant)))
      .toContain(
    ...
    expect(contract.validateContinuationReviewReceiptV12(
      receipt,
      snapshot,
      policy,
      {
    ...
    expect(contract.validateContinuationReviewReceiptV12(
      ...
      "continuation review receipt V12 identity/assurance is invalid",
    );
  });
  ...
  test("uses canonical deterministic gzip archive entries", () => {
    const bytes = Buffer.from("round12 baseline\n", "utf8");
    const entry = {
      path: "package.json",
      source: "governance_transition",
      sha256: contract.sha256BytesV12(bytes),
      encoding: "gzip-base64-v1",
      bytes: gzipSync(bytes, { level: 9 } as any).toString("base64"),
    };
    const archive = contract.withCanonicalDigestV12({
      schemaVersion: 12,
      kind: contract.CONTINUATION_V12_BASELINE_ARCHIVE_KIND,
      algorithm: contract.CONTINUATION_V12_ALGORITHM,
      programId: "conversation-progressive-disclosure-v3.9.2-2026-08",
      ...
      round: contract.CONTINUATION_V12_ROUND,
      entries: [entry],
      entrySetDigest: contract.hashCanonicalV12([entry]),
    });
    expect(contract.validateBaselineArchiveV12(archive)).toEqual([]);
    const mutant = structuredClone(archive);
    mutant.entries[0].bytes = gzipSync(bytes, { level: 1 } as any)
      .toString("base64");
    expect(contract.validateBaselineArchiveV12(rehash(mutant)).join("\n"))
