...
const checker = await import(
  ...
  "../../scripts/check-conversation-disclosure-continuation-v9.mjs"
);
...
describe("conversation disclosure continuation checker v9", () => {
  test("rejects an unpinned or unknown mode before reading evidence", async () => {
    await expect(
      checker.runConversationDisclosureContinuationCheckerV9([]),
    ).rejects.toThrow("checker option is required");
    await expect(
      checker.runConversationDisclosureContinuationCheckerV9([
        "--mode", "candidate_self_authorized",
      ]),
    ).rejects.toThrow("checker V9 --mode is invalid");
  });
