...
const freezer = await import(
  ...
  "../../scripts/freeze-conversation-disclosure-continuation-v5.mjs"
);
...
describe("conversation disclosure continuation freezer v5", () => {
  test("requires a caller-fixed canonical freeze time", async () => {
    await expect(freezer.freezeConversationDisclosureContinuationV5({
  ...
  test("captures every admission category in one postflight ledger", () => {
    ...
    expect(source).toContain("captureRequiredAbsentV5");
    expect(source).toContain("postflightCaptureLedgerV5(ledger)");
    expect(source.indexOf("postflightCaptureLedgerV5(ledger)"))
      .toBeLessThan(source.indexOf("publishPrivateExactV5(outputAbsolute"));
  });

  test("uses the shared transition and snapshot validators", () => {
    expect(source).toContain("validateGovernanceTransitionStateV5(");
    expect(source).toContain("validateContinuationReviewSnapshotV5(");
    expect(source).toContain(
      "output must be the exact Round5 review snapshot path",
    );
  });
});
