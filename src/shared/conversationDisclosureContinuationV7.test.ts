...
const contract = await import(
  ...
  "../../scripts/conversation-disclosure-continuation-contract-v7.mjs"
);
...
function rehash<T extends Record<string, unknown>>(value: T): T & { digest: string } {
  ...
  delete copy.digest;
  copy.digest = contract.hashCanonicalV7(copy);
  return copy as T & { digest: string };
}

function makeRound6Rejection() {
  return contract.withCanonicalDigestV7({
    schemaVersion: 7,
    kind: contract.CONTINUATION_V7_REVIEW_REJECTION_KIND,
    algorithm: contract.CONTINUATION_V7_ALGORITHM,
    programId: "conversation-progressive-disclosure-v3.9.2-2026-08",
    workstreamId: contract.CONTINUATION_V7_WORKSTREAM_ID,
    featureId: contract.CONTINUATION_V7_FEATURE_ID,
    rejectedRound: contract.CONTINUATION_V7_REJECTED_ROUND,
    recoveryRound: contract.CONTINUATION_V7_ROUND,
    status: "rejected_after_review",
    sourcePolicy: structuredClone(
      contract.CONTINUATION_V7_ROUND6_POLICY_TRUST_ROOT,
    ),
    sourceSnapshot: structuredClone(
      contract.CONTINUATION_V7_ROUND6_SNAPSHOT_TRUST_ROOT,
    ),
    failedReceipts: contract.CONTINUATION_V7_ROUND6_RECEIPT_TRUST_ROOTS.map(
      (entry: Record<string, unknown>) => structuredClone(entry),
    ),
    findingIds: [...contract.CONTINUATION_V7_ROUND6_FINDING_IDS],
    findingSetDigest: contract.CONTINUATION_V7_ROUND6_FINDING_SET_DIGEST,
    aggregateFindingCounts: structuredClone(
      contract.CONTINUATION_V7_ROUND6_AGGREGATE_FINDING_COUNTS,
    ),
    repositoryForbiddenOutputs: [
      ...contract.CONTINUATION_V7_ROUND6_REPOSITORY_FORBIDDEN_OUTPUT_PATHS,
    ],
    externalAnchorRule: contract.expectedRejectedRound6AnchorRuleV7(),
    priorRejections: {
      round1CanonicalDigest: contract.CONTINUATION_V7_ROUND1_REJECTION_DIGEST,
      round2: structuredClone(
        contract.CONTINUATION_V7_ROUND2_REJECTION_TRUST_ROOT,
      ),
    },
  });
}
...
function makeArchive() {
  ...
  return rehash({
    ...source,
    schemaVersion: 7,
    kind: contract.CONTINUATION_V7_BASELINE_ARCHIVE_KIND,
    round: 7,
  });
}
...
function makePolicy() {
  ...
  const rejection = makeRound6Rejection();
  const archive = makeArchive();
  const admission = structuredClone(source.admission);
  const transitions = contract.CONTINUATION_V7_GOVERNANCE_TRANSITION_TRUST_ROOTS
    .map((entry: Record<string, unknown>) => structuredClone(entry));
  admission.reviewOutputPaths = [
    ...contract.CONTINUATION_V7_REVIEW_OUTPUT_PATHS,
  ];
  admission.featureDefinition.files = [...new Set([
    ...admission.featureDefinition.files,
    contract.CONTINUATION_V7_ROUND6_REVIEW_REJECTION_PATH,
    contract.CONTINUATION_V7_BASELINE_ARCHIVE_PATH,
    contract.CONTINUATION_V7_POLICY_PATH,
    ...Object.values(
      contract.CONTINUATION_V7_EXECUTABLE_PATH_BY_KIND as Record<string, string>,
    ),
    ...transitions.map((entry: any) => entry.stagedTargetPath),
    ...contract.CONTINUATION_V7_REVIEW_OUTPUT_PATHS,
    ...contract.CONTINUATION_V7_REJECTED_OUTPUT_ABSENT_PATHS,
  ])].sort();
  admission.featureDefinitionDigest = contract.hashCanonicalV7(
    admission.featureDefinition,
  );
  admission.featureFileSetDigest = contract.hashCanonicalV7(
    admission.featureDefinition.files,
  );
  const admissionCoverage = contract.buildAdmissionCoverageV7(
    admission,
    transitions,
  );
  admission.reviewCoverageDigest = contract.hashCanonicalV7(admissionCoverage);
  const continuationExecutables = contract.CONTINUATION_V7_EXECUTABLE_KINDS.map(
    (kind: string, index: number) => ({
      kind,
      path: contract.CONTINUATION_V7_EXECUTABLE_PATH_BY_KIND[kind],
  ...
  return contract.withCanonicalDigestV7({
    schemaVersion: 7,
    kind: contract.CONTINUATION_V7_POLICY_KIND,
    algorithm: contract.CONTINUATION_V7_ALGORITHM,
    policyId: contract.CONTINUATION_V7_POLICY_ID,
    programId: source.programId,
    workstreamId: contract.CONTINUATION_V7_WORKSTREAM_ID,
    featureId: contract.CONTINUATION_V7_FEATURE_ID,
    round: contract.CONTINUATION_V7_ROUND,
    status: "frozen",
    ...
    admission,
    admissionClassSet: [...contract.CONTINUATION_V7_ADMISSION_CLASSES],
    admissionClassSetDigest:
      contract.CONTINUATION_V7_ADMISSION_CLASS_SET_DIGEST,
    admissionCoverage,
    ...
    baselineArchive: {
      path: contract.CONTINUATION_V7_BASELINE_ARCHIVE_PATH,
      digest: archive.digest,
      entrySetDigest: archive.entrySetDigest,
    },
    reviewSnapshot: { path: contract.CONTINUATION_V7_REVIEW_SNAPSHOT_PATH },
    reviewAssurancePolicy: {
      callerDispatchAssurance:
        contract.CONTINUATION_V7_CALLER_DISPATCH_ASSURANCE,
      identityAssurance: contract.CONTINUATION_V7_IDENTITY_ASSURANCE,
      independenceClaim: contract.CONTINUATION_V7_INDEPENDENCE_CLAIM,
      localIdentityProof: false,
    },
    externalAnchorPolicy: contract.expectedRejectedRound6AnchorRuleV7(),
...
function makeSnapshot(policy: any) {
  ...
  for (const reference of [
    contract.CONTINUATION_V7_ROUND6_POLICY_TRUST_ROOT,
    contract.CONTINUATION_V7_ROUND6_SNAPSHOT_TRUST_ROOT,
    ...contract.CONTINUATION_V7_ROUND6_RECEIPT_TRUST_ROOTS,
  ]) {
    frozen.get(reference.path)!.sha256 = reference.byteSha256;
  }
  frozen.get(
    contract.CONTINUATION_V7_ROUND6_REVIEW_REJECTION_PATH,
  )!.sha256 = contract.sha256BytesV7(
    contract.serializeRound6ReviewRejectionV7(
      policy.round6ReviewRejection,
    ),
  );
  ...
  return contract.withCanonicalDigestV7({
    schemaVersion: 7,
    kind: contract.CONTINUATION_V7_SNAPSHOT_KIND,
    algorithm: contract.CONTINUATION_V7_ALGORITHM,
    ...
    pathAuthorityDigest: contract.hashCanonicalV7(policy.pathAuthorities),
...
function makeReceipt(policy: any, snapshot: any) {
  const dispatchEntry = {
    lane: "contract",
    assurance: contract.CONTINUATION_V7_CALLER_DISPATCH_ASSURANCE,
  ...
  return {
    schemaVersion: 7,
    kind: contract.CONTINUATION_V7_RECEIPT_KIND,
    programId: policy.programId,
    workstreamId: contract.CONTINUATION_V7_WORKSTREAM_ID,
    featureId: contract.CONTINUATION_V7_FEATURE_ID,
    round: contract.CONTINUATION_V7_ROUND,
    ...
    callerDispatchEntryDigest: contract.hashCanonicalV7(dispatchEntry),
    callerDispatchSetDigest: contract.hashCanonicalV7(dispatchSet),
    claimedReviewOrigin: {
      taskPath: "caller/contract",
      agentLabel: "claimed-reviewer",
      transport: "codex-collaboration",
...
describe("conversation disclosure continuation contract v7", () => {
  test("binds the real runtime I/O API instead of the abandoned adapter", () => {
    expect(contract.validateRuntimeIoCaptureAdapterV7(runtimeIo)).toEqual([]);
    expect(contract.CONTINUATION_V7_RUNTIME_IO_INTERFACE.methods).not.toContain(
      "capturePresent",
    );
  });

  test("validates the exact Round6 rejection and both roots of every source", () => {
    const witness = makeRound6Rejection();
    expect(contract.validateRound6ReviewRejectionV7(witness)).toEqual([]);
    const references = [
    ...
    for (const reference of references) {
      ...
      expect(contract.validateRound6ReviewRejectionV7(rehash(mutant)))
        .not.toEqual([]);
    }
  });
  ...
  test("hard-roots the exact ordered six-class set", () => {
    ...
    expect(contract.validateContinuationPolicyV7(policy)).toEqual([]);
    for (const classSet of [
      ...
      mutant.admissionClassSetDigest = contract.hashCanonicalV7(classSet);
      expect(contract.validateContinuationPolicyV7(rehash(mutant)))
  ...
  test("rejects policy key omission and extension", () => {
    ...
    expect(contract.validateContinuationPolicyV7(omitted))
      .toContain("continuation policy must contain the exact V7 keys");
    const extended = { ...policy, candidateField: true };
    expect(contract.validateContinuationPolicyV7(extended))
      .toContain("continuation policy must contain the exact V7 keys");
  });

  test("validates the V7 archive and snapshot bindings", () => {
    const policy = makePolicy();
    const archive = makeArchive();
    const snapshot = makeSnapshot(policy);
    expect(contract.validateBaselineArchiveV7(archive, policy)).toEqual([]);
    expect(contract.validateContinuationReviewSnapshotV7(snapshot, policy, {
      verifierNow: Date.parse(snapshot.frozenAt),
    })).toEqual([]);
    const mutant = rehash({
      ...snapshot,
      round6ReviewRejectionDigest: zeroDigest,
    });
    expect(contract.validateContinuationReviewSnapshotV7(mutant, policy, {
      verifierNow: Date.parse(snapshot.frozenAt),
    })).toContain(
      "continuation review snapshot round6ReviewRejectionDigest binding is stale",
    );
  });
  ...
  test("rejects stronger identity assurance in policy and receipts", () => {
    ...
    expect(contract.validateContinuationPolicyV7(rehash(policyMutant)))
      .toContain(
    ...
    expect(contract.validateContinuationReviewReceiptV7(
      receipt,
      snapshot,
      policy,
      { callerPin },
    ...
    expect(contract.validateContinuationReviewReceiptV7(
      ...
      "continuation review receipt V7 identity/assurance is invalid",
    );
  });

  test("uses canonical deterministic gzip archive entries", () => {
    const bytes = Buffer.from("round7 baseline\n", "utf8");
    const entry = {
      path: "package.json",
      source: "governance_transition",
      sha256: contract.sha256BytesV7(bytes),
      encoding: "gzip-base64-v1",
      bytes: gzipSync(bytes, { level: 9 } as any).toString("base64"),
    };
    const archive = contract.withCanonicalDigestV7({
      schemaVersion: 7,
      kind: contract.CONTINUATION_V7_BASELINE_ARCHIVE_KIND,
      algorithm: contract.CONTINUATION_V7_ALGORITHM,
      programId: "conversation-progressive-disclosure-v3.9.2-2026-08",
      ...
      round: contract.CONTINUATION_V7_ROUND,
      entries: [entry],
      entrySetDigest: contract.hashCanonicalV7([entry]),
    });
    expect(contract.validateBaselineArchiveV7(archive)).toEqual([]);
    const mutant = structuredClone(archive);
    mutant.entries[0].bytes = gzipSync(bytes, { level: 1 } as any)
      .toString("base64");
    expect(contract.validateBaselineArchiveV7(rehash(mutant)).join("\n"))
