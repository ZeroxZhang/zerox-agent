...
describe("README", () => {
  it("documents how to verify, start, validate, and find local data", () => {
    ...
    expect(readme).toContain("Zerox.Agent-2.9.2-arm64.dmg");
  ...
  it("states the desktop control-plane positioning and links the decision matrix", () => {
    ...
    expect(readme).toContain("current release is **v2.9.2**");
    expect(readme).toContain("当前版本是 **v2.9.2**");
  });
  ...
  it("documents Goal Mode architecture, eval coverage, and feature-list status", () => {
    ...
    expect(readme).toContain("1235 tests");
    expect(readme).toContain("1235 个测试");
    expect(readme).toContain("Current version: v2.9.2.");
    expect(readme).toContain("当前版本：v2.9.2。");
