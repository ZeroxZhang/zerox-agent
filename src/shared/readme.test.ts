...
describe("README", () => {
  it("documents how to verify, start, validate, and find local data", () => {
    ...
    expect(readme).toContain("Zerox Agent-2.3.6-arm64.dmg");
  ...
  it("states the desktop control-plane positioning and links the decision matrix", () => {
    ...
    expect(readme).toContain("current release is **v2.3.6**");
    expect(readme).toContain("当前版本是 **v2.3.6**");
  });
  ...
  it("documents Goal Mode architecture, eval coverage, and feature-list status", () => {
    ...
    expect(readme).toContain("767 tests");
    expect(readme).toContain("767 个测试");
    expect(readme).toContain("Current version: v2.3.6.");
    expect(readme).toContain("当前版本：v2.3.6。");
    expect(readme).toContain(
      "v2.3.6 ships deterministic local artifact goals",
    );
    expect(readme).toContain(
      "independent packaged-app computer-use acceptance gate that passed",
    );
    expect(readme).toContain("v2.3.6 P11.6 passed the command-line verification gate");
    expect(readme).toContain("v2.3.6 P11.6 已通过命令行验证 gate");
    ...
    expect(readme).toContain("release metadata now matches the v2.3.6 distribution");
    expect(readme).toContain("release metadata 现在已经匹配 v2.3.6 分发版本");
