...
const governance = await import(
  ...
  "../../scripts/conversation-disclosure-program-governance-v6.mjs"
);
...
describe("conversation disclosure program governance v6", () => {
  it("accepts the closed-world P107A admission lifecycle and reports every rule", () => {
    ...
    expect(result.kind).toBe(governance.GOVERNANCE_V6_RESULT_KIND);
    expect(result.errors).toEqual([]);
    expect(result.ruleResults.map((rule: RuleResult) => rule.id)).toEqual(
      governance.PROGRAM_GOVERNANCE_V6_RULE_IDS,
    );
  ...
  it("accepts only the exact all-from or all-to governance target state", () => {
    ...
    expect(contract.validateGovernanceTransitionStateV6(
      transitions,
  ...
  it("keeps the Round6 harness diagnostic-only by default and identity claims not-signed", () => {
    ...
    expect(contract.CONTINUATION_V6_CALLER_DISPATCH_ASSURANCE).toBe(
      "caller-attested-not-signed",
    );
    expect(contract.CONTINUATION_V6_IDENTITY_ASSURANCE).toBe("not-signed");
    expect(contract.CONTINUATION_V6_INDEPENDENCE_CLAIM).toBe(
      "caller-attested-distinct-review-contexts",
    );
  });

  it("changes the Round5 package target only for the V6 test orchestrator", () => {
  ...
  it("keeps pre-freeze target tests on the published Round5 baseline archive", () => {
    const target = readFileSync(path.join(
      process.cwd(),
      ".zerox/verification/conversation-disclosure/CD03A-round6-program-test.target.ts",
    ), "utf8");
    expect(target).toContain("CD03A-round5-baseline-archive.json");
    expect(target).not.toContain("CD03A-round6-baseline-archive.json");
  });
  ...
  it("rejects an unknown required rule id", () => {
    ...
    expect(ruleResult(validate(input), "CDG4-001-input-contract").message).toContain(
      "closedWorld.ruleIds must exactly match the known v6 ledger",
    );
  });
...
function validate(input: GovernanceInput) {
  return governance.validateConversationDisclosureProgramGovernanceV6(input);
}
...
function createInput() {
  ...
  const workstreams = [
    ...
    createWorkstream("CD08", "P112-v6.9.2-disclosure-hardening", "planned", ["CD07"], ["D1"]),
    createWorkstream("CD09", "P113-v6.9.2-disclosure-adversarial-acceptance", "planned", ["CD08"], ["D1"]),
  ];
  ...
  const program = {
      schemaVersion: 1,
      programId: "conversation-progressive-disclosure-v6.9.2-2026-08",
      status: "active",
      maxActiveFeatures: 1,
  ...
  const programRootBinding = governance.createProgramRootBindingV6(program);
  return {
    program,
    ...
    closedWorld: {
      schemaVersion: 6,
      ...programRootBinding,
      ruleIds: [...governance.PROGRAM_GOVERNANCE_V6_RULE_IDS],
...
function createTransitions() {
  return Object.entries(
    contract.CONTINUATION_V6_GOVERNANCE_TRANSITIONS as Record<string, string>,
  )
    ...
    .map(([livePath, kind], index) => ({
      ...
      kind,
      stagedTargetPath:
        `.zerox/verification/conversation-disclosure/CD03A-round6-target-${index}`,
...
type GovernanceResult = ReturnType<typeof governance.validateConversationDisclosureProgramGovernanceV6>;
