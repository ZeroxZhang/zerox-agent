...
const toolOptions: Array<{ name: AgentToolName; label: string }> = [
  ...
  { name: "skill_resource_list", label: "列出技能资源 (skill_resource_list)" },
  { name: "skill_load", label: "加载技能内容 (skill_load)" },
  { name: "history_search", label: "检索原始历史 (history_search)" },
  { name: "history_around", label: "读取历史上下文 (history_around)" },
...
function defaultArgsJson(toolName: AgentToolName): string {
  const defaults: Record<AgentToolName, Record<string, unknown>> = {
    ...
    file_rollback_moves: {
      transaction: { id: "transaction_id", root: "~/Downloads", moves: [] },
    },
    ...
    chrome_bookmarks_read: { profile: "Default", maxBookmarks: 5000 },
    ...
    history_search: { query: "skill_load 工具输出", limit: 5 },
    history_around: { entryId: "history_entry_id", before: 2, after: 2 },
    skill_resource_list: { skillName: "research-writing" },
    skill_load: { skillName: "research-writing" },
