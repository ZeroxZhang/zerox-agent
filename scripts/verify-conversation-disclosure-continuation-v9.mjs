...
// CD03A external closure runner v9. This file is intentionally self-contained:
...
const ROUND8_REVIEW_REJECTION_PATH =
...
const ROUND8_POLICY_ROOT = Object.freeze({
  path:
...
const ROUND8_SNAPSHOT_ROOT = Object.freeze({
  path:
...
const ROUND8_RECEIPT_ROOTS = Object.freeze([
  Object.freeze({
    lane: "governance",
    path:
      ".zerox/verification/conversation-disclosure/CD03A-round8-governance-review.json",
    byteSha256:
      "sha256:7c1defbcc636745e3ae6d1d9bdeabcb9b1f8244281e8826e0eee29d699c0b0e8",
    canonicalDigest:
      "sha256:4c59584c3d639abf2df38d69a3efe52e92e31566e0778656757e7a5174f06994",
    challenge:
      "sha256:ba1c4e2085b09aab90331e11ba33025a2c561e4d47072d65af9c34959f26122d",
    verdict: "passed",
  ...
  Object.freeze({
    ...
    byteSha256:
      "sha256:08a676414fb8a8655534f373b86071cff9fa8b5b1848afff25f3cb5f0fa94b42",
    canonicalDigest:
      "sha256:c77fb3bb0598f2d08f86bd2a9407492c349b479ff06c03648d52dad007eef32e",
    challenge:
      "sha256:ea7e859ad169f65179cd903864ae90469be6079d31d03b5670c30a7509d51929",
    verdict: "failed",
    findingCounts: Object.freeze({ critical: 0, major: 5, minor: 0 }),
  }),
]);
const ROUND8_FINDING_IDS = Object.freeze([
  "R8-RUNTIME-001",
  "R8-RUNTIME-002",
  "R8-RUNTIME-003",
  "R8-RUNTIME-004",
...
const ROUND8_FINDING_SET_DIGEST =
  "sha256:18b85d54d7e8709b6105b9b40827a5e90cd02315136cca9b3e7ee8c4ccd056f7";
const ROUND8_FINDING_COUNTS = Object.freeze({
  critical: 0,
  major: 5,
  minor: 0,
});
const ROUND8_FORBIDDEN_OUTPUTS = Object.freeze([
...
const JOURNAL_KIND =
  "conversation-disclosure-continuation-publication-transaction-v9";
...
const POLICY_KEYS = Object.freeze([
  ...
  "round1Rejection", "round2PrefreezeRejection", "round8ReviewRejection",
...
const SNAPSHOT_KEYS = Object.freeze([
  ...
  "round8ReviewRejectionDigest", "schemaVersion",
...
const MANIFEST_KEYS = Object.freeze([
  ...
  "policy", "programId", "reviewReceipts", "round", "round8ReviewRejection",
...
const CONTROL_SET_KIND = "conversation-disclosure-continuation-control-set-v9";
...
const RECEIPT_KEYS = Object.freeze([
  ...
  "round8ReviewRejectionDigest", "schemaVersion", "snapshotDigest",
...
const ATTESTATION_KEYS = Object.freeze([
  ...
  "repositoryRealpath", "reviewAssurance", "round8ReviewRejectionDigest",
...
if (options.journalOutput !== `${options.externalAnchorOutput}.closure-v9.journal.json`) {
...
async function buildFreshTransaction() {
  ...
  await captureAndValidateRound8ReviewRejection(policy);
  ...
  if (!exactKeys(snapshot, SNAPSHOT_KEYS)
    ...
    || snapshot.round8ReviewRejectionDigest
      !== policy.round8ReviewRejection?.digest
  ...
  if (transitions.length !== 4) {
    fail("runner v9 requires exactly four governance transitions");
  }
...
async function captureAndValidateRound8ReviewRejection(policy) {
  const embedded = policy.round8ReviewRejection;
  const capture = await captureRepositoryFile(
    ROUND8_REVIEW_REJECTION_PATH,
    "Round8 review-rejection witness",
    { requirePrivate: true },
  );
  const rejection = parseJson(capture.bytes, "Round8 review-rejection witness");
  validateCanonicalObject(rejection, "Round8 review-rejection witness");
  if (canonicalJson(rejection) !== canonicalJson(embedded)) {
    fail("continuation policy Round8 review-rejection binding is stale");
  }
  await validateRound8ReviewRejection(rejection, policy);
}

async function validateRound8ReviewRejection(rejection, policy) {
  const expectedKeys = [
  ...
  if (!exactKeys(rejection, expectedKeys)
    ...
    || rejection.recoveryRound !== 9
    || rejection.rejectedRound !== 8
    || rejection.status !== "rejected_after_review"
    || canonicalJson(rejection.sourcePolicy) !== canonicalJson(ROUND8_POLICY_ROOT)
    || canonicalJson(rejection.sourceSnapshot)
      !== canonicalJson({
        ...ROUND8_SNAPSHOT_ROOT,
        frozenFileCount: 223,
        transitionPayloadFileCount: 4,
        baselineFileCount: 12,
      })
    || canonicalJson(rejection.findingIds) !== canonicalJson(ROUND8_FINDING_IDS)
    || rejection.findingSetDigest !== ROUND8_FINDING_SET_DIGEST
    || canonicalJson(rejection.aggregateFindingCounts)
      !== canonicalJson(ROUND8_FINDING_COUNTS)
    || canonicalJson(rejection.repositoryForbiddenOutputs)
      !== canonicalJson(ROUND8_FORBIDDEN_OUTPUTS)
    || !Array.isArray(rejection.completedReceipts)
    || rejection.completedReceipts.length !== ROUND8_RECEIPT_ROOTS.length) {
    fail("Round8 review-rejection schema or lifecycle binding is stale");
  }
  for (const expected of [
    ROUND8_POLICY_ROOT,
    ROUND8_SNAPSHOT_ROOT,
    ...ROUND8_RECEIPT_ROOTS,
  ]) {
    const current = await captureRepositoryFile(
      expected.path,
      `Round8 rejection source ${expected.path}`,
      { requirePrivate: expected === ROUND8_POLICY_ROOT
        || expected === ROUND8_SNAPSHOT_ROOT
        || ROUND8_RECEIPT_ROOTS.includes(expected) },
    );
    if (current.digest !== expected.byteSha256) {
      fail(`Round8 rejection source bytes drifted: ${expected.path}`);
    }
    const value = parseJson(current.bytes, `Round8 rejection source ${expected.path}`);
    const canonicalDigest = value.digest ?? hashCanonical(value);
    if (canonicalDigest !== expected.canonicalDigest) {
      fail(`Round8 rejection source canonical digest drifted: ${expected.path}`);
    }
  }
  for (let index = 0; index < ROUND8_RECEIPT_ROOTS.length; index += 1) {
    const actual = rejection.completedReceipts[index];
    const expected = ROUND8_RECEIPT_ROOTS[index];
    if (actual?.lane !== expected.lane
      ...
      fail(`Round8 rejection receipt root is stale: ${expected.lane}`);
    }
  }
  for (const relativePath of rejection.repositoryForbiddenOutputs ?? []) {
    if (await repositoryEntryExists(repositoryRealpath, relativePath)) {
      fail(`Round8 forbidden repository output exists: ${relativePath}`);
    }
  }
}
...
async function convergePreparedTransaction(journal, { skipFaults }) {
  ...
  for (const [index, transition] of journal.governanceTransitions.entries()) {
    const requestedFault = skipFaults
      ? ""
      : process.env.ZEROX_CD03A_RUNNER_V9_TEST_FAULT ?? "";
    ...
    if (fault !== "none") fail(`injected runner v9 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
  ...
  for (const publication of journal.publications) {
    ...
    if (bridgeFault !== "none") fail(`injected runner v9 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
...
async function executeCandidatesInStage({ stagedTree, policy, baseAnchor, snapshot }) {
  const stageRoot = await realpath(
    await mkdtemp(path.join(os.tmpdir(), "zerox-cd03a-v9-stage-")),
  );
  ...
  try {
    ...
    for (const [kind, relativePath] of [["checker", checkerPath], ["harness", harnessPath]]) {
      ...
      const receiptKind = kind === "checker"
        ? "cd03a-continuation-checker-v9-receipt"
        : "cd03a-continuation-harness-v9-receipt";
...
async function removeValidatedStageRoot(stageRoot, controlRoot) {
  ...
  if (path.dirname(stageRealpath) !== temporaryRoot
    || !path.basename(stageRealpath).startsWith("zerox-cd03a-v9-stage-")
...
function validateJournal(journal, caller, repoRealpath, runnerDigest) {
  ...
  if (!exactKeys(journal, JOURNAL_KEYS)
    || journal.schemaVersion !== 9 || journal.kind !== JOURNAL_KIND
  ...
  if (!exactKeys(embeddedSnapshot, SNAPSHOT_KEYS)
    || embeddedSnapshot.schemaVersion !== 9
  ...
  if (!exactKeys(attestation, ATTESTATION_KEYS)
    ...
    || attestation?.round8ReviewRejectionDigest
      !== embeddedPendingManifest.round8ReviewRejection?.canonicalDigest
  ...
  if (!exactKeys(anchor, ANCHOR_KEYS)
    ...
    || anchor?.round8ReviewRejectionDigest
      !== embeddedPendingManifest.round8ReviewRejection?.canonicalDigest
...
function validateCandidateResultSet(
  results,
  checkerPath = "scripts/check-conversation-disclosure-continuation-v9.mjs",
) {
...
function parseOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    fail(`unknown runner v9 option: ${argument}`);
  }
  for (const key of [
    ...
    if (!values[key]) fail(`missing required runner v9 option: ${key}`);
  }
...
function injectFault(point) {
  if (process.env.ZEROX_CD03A_RUNNER_V9_TEST_FAULT === point) {
    fail(`injected runner v9 fault: ${point}`);
  }
}
...
function printPublicationReceipt(journal, recovered) {
  ...
  const receiptWithoutDigest = {
    kind: "cd03a-external-publication-v9-receipt",
    status: "passed",
    recovered,
...
function fail(message) {
  console.error("CD03A external continuation runner v9 failed:");
