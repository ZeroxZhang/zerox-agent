...
// CD03A external closure runner v8. This file is intentionally self-contained:
...
const ROUND7_REVIEW_REJECTION_PATH =
...
const ROUND7_POLICY_ROOT = Object.freeze({
  path:
...
const ROUND7_SNAPSHOT_ROOT = Object.freeze({
  path:
...
const ROUND7_RECEIPT_ROOTS = Object.freeze([
  Object.freeze({
    lane: "governance",
    path:
      ".zerox/verification/conversation-disclosure/CD03A-round7-governance-review.json",
    byteSha256:
      "sha256:a9de52c5b036e7cbaf6c1b5d9e9ddf9889c041b46b1268281f1ddde3d1507eca",
    canonicalDigest:
      "sha256:28b1a83480cf362c37a13e6bb96b4c436ff26c8ad129e87267d38eeb1578e935",
    challenge:
      "sha256:04de1c67c7bf0a574e88c168e5aa3909261dea41bc9edd43d953b037dee03376",
    verdict: "passed",
  ...
  Object.freeze({
    ...
    byteSha256:
      "sha256:24280f496ad9434156859a053d8ec284880ffe6c209d344601a503c5f64a3def",
    canonicalDigest:
      "sha256:4838df8dff6a3eaaa7c83bf75cacaa1487fcd05cfe58a7bc1c9fdfdf4acd7bf2",
    challenge:
      "sha256:b3f659fb51f93d1e1bf9954663a877cacb32f741c97fcfc44bfe2ac0d0939254",
    verdict: "failed",
    findingCounts: Object.freeze({ critical: 0, major: 4, minor: 0 }),
  }),
]);
const ROUND7_FINDING_IDS = Object.freeze([
  "R7-RUNTIME-001",
  "R7-RUNTIME-002",
  "R7-RUNTIME-003",
  "R7-RUNTIME-004",
]);
const ROUND7_FINDING_SET_DIGEST =
  "sha256:a56468a403fd1e9a9ac0ec99a2c5473e5fc043b7cce9ea580c23f85a1c36ecdf";
const ROUND7_FINDING_COUNTS = Object.freeze({
  critical: 0,
  major: 4,
  minor: 0,
});
const ROUND7_FORBIDDEN_OUTPUTS = Object.freeze([
...
const JOURNAL_KIND =
  "conversation-disclosure-continuation-publication-transaction-v8";
...
const POLICY_KEYS = Object.freeze([
  ...
  "round1Rejection", "round2PrefreezeRejection", "round7ReviewRejection",
...
const SNAPSHOT_KEYS = Object.freeze([
  ...
  "round7ReviewRejectionDigest", "schemaVersion",
...
const MANIFEST_KEYS = Object.freeze([
  ...
  "policy", "programId", "reviewReceipts", "round", "round7ReviewRejection",
...
const CONTROL_SET_KIND = "conversation-disclosure-continuation-control-set-v8";
...
const RECEIPT_KEYS = Object.freeze([
  ...
  "round7ReviewRejectionDigest", "schemaVersion", "snapshotDigest",
...
const ATTESTATION_KEYS = Object.freeze([
  ...
  "repositoryRealpath", "reviewAssurance", "round7ReviewRejectionDigest",
...
if (options.journalOutput !== `${options.externalAnchorOutput}.closure-v8.journal.json`) {
...
async function buildFreshTransaction() {
  ...
  await captureAndValidateRound7ReviewRejection(policy);
  ...
  if (!exactKeys(snapshot, SNAPSHOT_KEYS)
    ...
    || snapshot.round7ReviewRejectionDigest
      !== policy.round7ReviewRejection?.digest
  ...
  if (transitions.length !== 4) {
    fail("runner v8 requires exactly four governance transitions");
  }
...
async function captureAndValidateRound7ReviewRejection(policy) {
  const embedded = policy.round7ReviewRejection;
  const capture = await captureRepositoryFile(
    ROUND7_REVIEW_REJECTION_PATH,
    "Round7 review-rejection witness",
    { requirePrivate: true },
  );
  const rejection = parseJson(capture.bytes, "Round7 review-rejection witness");
  validateCanonicalObject(rejection, "Round7 review-rejection witness");
  if (canonicalJson(rejection) !== canonicalJson(embedded)) {
    fail("continuation policy Round7 review-rejection binding is stale");
  }
  await validateRound7ReviewRejection(rejection, policy);
}

async function validateRound7ReviewRejection(rejection, policy) {
  const expectedKeys = [
  ...
  if (!exactKeys(rejection, expectedKeys)
    ...
    || rejection.recoveryRound !== 8
    || rejection.rejectedRound !== 7
    || rejection.status !== "rejected_after_review"
    || canonicalJson(rejection.sourcePolicy) !== canonicalJson(ROUND7_POLICY_ROOT)
    || canonicalJson(rejection.sourceSnapshot)
      !== canonicalJson({
        ...ROUND7_SNAPSHOT_ROOT,
        frozenFileCount: 188,
        transitionPayloadFileCount: 4,
        baselineFileCount: 12,
      })
    || canonicalJson(rejection.findingIds) !== canonicalJson(ROUND7_FINDING_IDS)
    || rejection.findingSetDigest !== ROUND7_FINDING_SET_DIGEST
    || canonicalJson(rejection.aggregateFindingCounts)
      !== canonicalJson(ROUND7_FINDING_COUNTS)
    || canonicalJson(rejection.repositoryForbiddenOutputs)
      !== canonicalJson(ROUND7_FORBIDDEN_OUTPUTS)
    || !Array.isArray(rejection.completedReceipts)
    || rejection.completedReceipts.length !== ROUND7_RECEIPT_ROOTS.length) {
    fail("Round7 review-rejection schema or lifecycle binding is stale");
  }
  for (const expected of [
    ROUND7_POLICY_ROOT,
    ROUND7_SNAPSHOT_ROOT,
    ...ROUND7_RECEIPT_ROOTS,
  ]) {
    const current = await captureRepositoryFile(
      expected.path,
      `Round7 rejection source ${expected.path}`,
      { requirePrivate: expected === ROUND7_POLICY_ROOT
        || expected === ROUND7_SNAPSHOT_ROOT
        || ROUND7_RECEIPT_ROOTS.includes(expected) },
    );
    if (current.digest !== expected.byteSha256) {
      fail(`Round7 rejection source bytes drifted: ${expected.path}`);
    }
    const value = parseJson(current.bytes, `Round7 rejection source ${expected.path}`);
    const canonicalDigest = value.digest ?? hashCanonical(value);
    if (canonicalDigest !== expected.canonicalDigest) {
      fail(`Round7 rejection source canonical digest drifted: ${expected.path}`);
    }
  }
  for (let index = 0; index < ROUND7_RECEIPT_ROOTS.length; index += 1) {
    const actual = rejection.completedReceipts[index];
    const expected = ROUND7_RECEIPT_ROOTS[index];
    if (actual?.lane !== expected.lane
      ...
      fail(`Round7 rejection receipt root is stale: ${expected.lane}`);
    }
  }
  for (const relativePath of rejection.repositoryForbiddenOutputs ?? []) {
    if (await repositoryEntryExists(repositoryRealpath, relativePath)) {
      fail(`Round7 forbidden repository output exists: ${relativePath}`);
    }
  }
}
...
async function convergePreparedTransaction(journal, { skipFaults }) {
  ...
  for (const [index, transition] of journal.governanceTransitions.entries()) {
    const requestedFault = skipFaults
      ? ""
      : process.env.ZEROX_CD03A_RUNNER_V8_TEST_FAULT ?? "";
    ...
    if (fault !== "none") fail(`injected runner v8 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
  ...
  for (const publication of journal.publications) {
    ...
    if (bridgeFault !== "none") fail(`injected runner v8 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
...
async function executeCandidatesInStage({ stagedTree, policy, baseAnchor, snapshot }) {
  const stageRoot = await realpath(
    await mkdtemp(path.join(os.tmpdir(), "zerox-cd03a-v8-stage-")),
  );
  ...
  try {
    ...
    for (const [kind, relativePath] of [["checker", checkerPath], ["harness", harnessPath]]) {
      ...
      const receiptKind = kind === "checker"
        ? "cd03a-continuation-checker-v8-receipt"
        : "cd03a-continuation-harness-v8-receipt";
...
async function removeValidatedStageRoot(stageRoot, controlRoot) {
  ...
  if (path.dirname(stageRealpath) !== temporaryRoot
    || !path.basename(stageRealpath).startsWith("zerox-cd03a-v8-stage-")
...
function validateJournal(journal, caller, repoRealpath, runnerDigest) {
  ...
  if (!exactKeys(journal, JOURNAL_KEYS)
    || journal.schemaVersion !== 8 || journal.kind !== JOURNAL_KIND
  ...
  if (!exactKeys(embeddedSnapshot, SNAPSHOT_KEYS)
    || embeddedSnapshot.schemaVersion !== 8
  ...
  if (!exactKeys(attestation, ATTESTATION_KEYS)
    ...
    || attestation?.round7ReviewRejectionDigest
      !== embeddedPendingManifest.round7ReviewRejection?.canonicalDigest
  ...
  if (!exactKeys(anchor, ANCHOR_KEYS)
    ...
    || anchor?.round7ReviewRejectionDigest
      !== embeddedPendingManifest.round7ReviewRejection?.canonicalDigest
...
function validateCandidateResultSet(
  results,
  checkerPath = "scripts/check-conversation-disclosure-continuation-v8.mjs",
) {
...
function parseOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    fail(`unknown runner v8 option: ${argument}`);
  }
  for (const key of [
    ...
    if (!values[key]) fail(`missing required runner v8 option: ${key}`);
  }
...
function injectFault(point) {
  if (process.env.ZEROX_CD03A_RUNNER_V8_TEST_FAULT === point) {
    fail(`injected runner v8 fault: ${point}`);
  }
}
...
function printPublicationReceipt(journal, recovered) {
  ...
  const receiptWithoutDigest = {
    kind: "cd03a-external-publication-v8-receipt",
    status: "passed",
    recovered,
...
function fail(message) {
  console.error("CD03A external continuation runner v8 failed:");
