...
import {
  ...
  CONTINUATION_V5_LIFECYCLE_PROFILE_PHASES,
...
import {
  PROGRAM_GOVERNANCE_V5_RULE_IDS,
  validateConversationDisclosureProgramGovernanceV5,
} from "./conversation-disclosure-program-governance-v5.mjs";

const PROGRAM_PATH = ".zerox/conversation-disclosure-program.json";
const FEATURE_LIST_PATH = ".zerox/feature_list.json";
const CHECKER_RECEIPT_KIND = "cd03a-continuation-checker-v5-receipt";
...
export async function runConversationDisclosureContinuationCheckerV5(
  argv = process.argv.slice(2),
) {
  const options = parseOptionsV5(argv);
  ...
  const [policy, archive, rejection, snapshot, program, featureList] =
    await Promise.all([
      readControlJson(CONTINUATION_V5_POLICY_PATH, "Round5 policy", {
  ...
  for (const entry of snapshot.frozenFiles) {
    const capture = await captureControl(
      entry.path,
      `Round5 frozen subject ${entry.path}`,
      {
        privateEvidence: [
          CONTINUATION_V5_POLICY_PATH,
          CONTINUATION_V5_ROUND4_REVIEW_REJECTION_PATH,
    ...
    if (capture.digest !== entry.sha256) {
      throw new Error(`Round5 frozen subject drifted: ${entry.path}`);
    }
  }
  for (const entry of snapshot.transitionPayloadFiles) {
    const capture = await captureControl(
      entry.path,
      `Round5 transition payload ${entry.path}`,
    );
    if (capture.digest !== entry.sha256) {
      throw new Error(`Round5 transition payload drifted: ${entry.path}`);
    }
  }
  ...
  for (const entry of snapshot.baselineFiles) {
    if (POST_TRANSITION_PHASES.has(options.mode)
      ...
      await captureControl(entry.path, `Round5 mutable baseline ${entry.path}`);
      continue;
    }
    const capture = await captureControl(
      entry.path,
      `Round5 baseline ${entry.path}`,
    );
    if (capture.digest !== entry.sha256) {
      throw new Error(`Round5 baseline drifted: ${entry.path}`);
    }
  }
  ...
  for (const transition of policy.governanceTransitions) {
    const [live, staged] = await Promise.all([
      captureControl(transition.path, `Round5 transition live ${transition.path}`),
      captureControl(
        transition.stagedTargetPath,
        `Round5 transition staged ${transition.stagedTargetPath}`,
      ),
    ]);
  ...
  for (const relativePath of snapshot.absentPaths) {
    ...
    await captureRequiredAbsentV5(
      ...
      `Round5 required absence ${relativePath}`,
  ...
  if (options.mode === "review_pre_transition") {
    for (const relativePath of snapshot.reviewOutputAbsentPaths) {
      if (relativePath === CONTINUATION_V5_REVIEW_SNAPSHOT_PATH) continue;
      await captureRequiredAbsentV5(
        path.join(controlRoot, relativePath),
        `Round5 pre-review output ${relativePath}`,
        { expectedRoot: controlRoot, ledger },
      );
    }
  } else {
  ...
  const lifecycleProfile = selectLifecycleProfileV5(policy, options.mode);
  if (!lifecycleProfile) {
    throw new Error(`Round5 lifecycle profile is missing: ${options.mode}`);
  }
  assertNoErrors(validateLifecycleStateV5({
    phase: options.mode,
    activeFeatureId: program.activeFeatureId,
    nextFeatureId: program.nextFeatureId,
    workstreams: program.workstreams,
    features: featureList.features,
  }, policy), "Round5 lifecycle state");
  const parentEvidence = await materializeParentEvidence(
    policy,
    baseAnchor,
    readControlJson,
  );
  const governance = validateConversationDisclosureProgramGovernanceV5({
    program,
    featureList,
    closedWorld: {
      schemaVersion: 5,
      ...
      ruleIds: [...PROGRAM_GOVERNANCE_V5_RULE_IDS],
  ...
  assertNoErrors(governance.errors, "Round5 program governance");
  ...
  return {
    ...
    digest: hashCanonicalV5(receiptWithoutDigest),
  };
}
...
async function validateContinuationEvidence({
  ...
  const [policyCapture, snapshotCapture, rejectionCapture, validatorCapture,
    ...
    captureControl(CONTINUATION_V5_POLICY_PATH, "Round5 manifest policy", {
      privateEvidence: true,
    }),
    captureControl(
      CONTINUATION_V5_REVIEW_SNAPSHOT_PATH,
      "Round5 manifest snapshot",
      { privateEvidence: true },
    ),
    captureControl(
      CONTINUATION_V5_ROUND4_REVIEW_REJECTION_PATH,
      "Round4 manifest rejection",
      { privateEvidence: true },
    ),
    captureControl(manifest.validator?.path, "Round5 manifest validator"),
    captureControl(manifest.externalRunner?.path, "Round5 manifest runner"),
  ]);
  const reviewReferences = receipts.map((receipt) => ({
    lane: receipt.lane,
    path:
      `.zerox/verification/conversation-disclosure/CD03A-round5-${receipt.lane}-review.json`,
  ...
  const manifestBindings = {
    ...
    round4ReviewRejectionReference: {
  ...
  if (options.mode === "review_post_transition") {
    assertNoErrors(validateContinuationClosureManifestV5(
      manifest,
      manifestBindings,
    ), "Round5 pending closure manifest");
    ...
    await captureRequiredAbsentV5(
      path.join(controlRoot, CONTINUATION_V5_EXTERNAL_ATTESTATION_PATH),
      "Round5 pre-transaction external attestation",
      { expectedRoot: controlRoot, ledger },
    );
  ...
  const attestation = await readControlJson(
    CONTINUATION_V5_EXTERNAL_ATTESTATION_PATH,
    "Round5 external attestation",
    { privateEvidence: true },
  );
  const pendingManifest = reconstructPendingManifestV5(manifest);
  assertNoErrors(validateContinuationExternalAttestationV5(attestation, {
    policy,
    snapshot,
  ...
  if (ORDINARY_PHASES.has(options.mode)) {
    ...
    if (canonicalAnchor !== options.continuationAnchorPath
      ...
      throw new Error("Round5 continuation anchor must be canonical and external");
    }
    anchor = parseJson(
      (await capturePrivateEvidenceV5(
        canonicalAnchor,
        "Round5 continuation anchor",
        { ledger },
      )).bytes,
      "Round5 continuation anchor",
    );
    assertNoErrors(validateContinuationExternalAnchorV5(anchor, {
    ...
    if (anchor.repositoryRealpath !== subjectRepositoryRealpath
      ...
      throw new Error("Round5 continuation anchor bindings are stale");
    }
  }
  return { receipts, manifest, attestation, anchor };
}

function reconstructPendingManifestV5(finalManifest) {
  ...
  return withCanonicalDigestV5(projected);
}

function parseOptionsV5(argv) {
  const options = {
    errors: [],
  };
  const map = new Map([
  ...
  if (!CONTINUATION_V5_LIFECYCLE_PROFILE_PHASES.includes(options.mode)) {
    options.errors.push("checker V5 --mode is invalid");
  }
  return options;
}
...
if (process.argv[1]
  ...
  runConversationDisclosureContinuationCheckerV5()
    .then((receipt) => {
      process.stdout.write("Conversation disclosure continuation v5 check passed.\n");
      process.stdout.write(`${JSON.stringify(receipt)}\n`);
    })
    .catch((error) => {
      process.stderr.write("Conversation disclosure continuation v5 check failed:\n");
