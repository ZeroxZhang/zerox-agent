...
export const PRIVATE_MODE_V11 = 0o600;

const LEDGER_KIND = "conversation-disclosure-runtime-capture-ledger-v11";

export class RuntimeIoV11Error extends Error {
  constructor(code, message) {
    super(message);
    this.name = "RuntimeIoV11Error";
    this.code = code;
  }
}

export function createCaptureLedgerV11() {
  return {
    kind: LEDGER_KIND,
    entries: [],
...
export function sha256BytesV11(bytes) {
...
export async function captureStableFileV11(
  absolutePath,
  label = absolutePath,
  {
    expectedRoot,
  ...
  await assertExpectedRootV11(absolutePath, expectedRoot, label, true);
  const parents = await captureParentIdentitiesV11(absolutePath, label);
  let before;
  try {
  ...
  const requiredMode = requirePrivate ? PRIVATE_MODE_V11 : expectedMode;
  ...
  try {
    ...
    await assertParentIdentitiesV11(parents, label);
    await assertExpectedRootV11(absolutePath, expectedRoot, label, true);
    const capture = {
      kind: "present",
      absolutePath,
      ...
      bytes,
      digest: sha256BytesV11(bytes),
      mode: Number(opened.mode & 0o777n),
      uid: Number(opened.uid),
    };
    recordCaptureV11(ledger, capture);
    return capture;
  } finally {
    await handle.close();
  }
...
export function capturePrivateEvidenceV11(
  absolutePath,
  label = absolutePath,
  options = {},
) {
  return captureStableFileV11(absolutePath, label, {
    ...options,
    requirePrivate: true,
  });
}

export async function captureRequiredAbsentV11(
  absolutePath,
  label = absolutePath,
  { expectedRoot, ledger } = {},
) {
  requireAbsolutePath(absolutePath, label);
  await assertExpectedRootV11(absolutePath, expectedRoot, label, false);
  const parents = await captureParentIdentitiesV11(absolutePath, label);
  try {
...
export async function postflightCaptureLedgerV11(ledger) {
  requireLedgerV11(ledger);
  for (const capture of ledger.entries) {
    await assertParentIdentitiesV11(capture.parents, capture.label);
    await assertExpectedRootV11(
    ...
    const current = await captureStableFileV11(
      capture.absolutePath,
      capture.label,
      {
...
export async function publishPrivateExactV11(
  absolutePath,
  bytes,
  ...
  try {
    ...
    try {
      ...
      if (existingEntry.nlink === 1n) {
        const existing = await capturePrivateEvidenceV11(
          absolutePath,
          label,
          { expectedRoot },
        );
    ...
    const status = await runPrivatePublicationBridgeV11({
      directoryHandle,
      expectedParent,
      basename,
      bytes,
    ...
    await assertOpenedDirectoryV11(directoryHandle, expectedParent, label);
    await assertParentIdentitiesV11(parents, label);
    const published = await capturePrivateEvidenceV11(
      absolutePath,
      label,
      { expectedRoot },
    );
...
async function runPrivatePublicationBridgeV11({
  directoryHandle,
  expectedParent,
  basename,
  bytes,
...
function recordCaptureV11(ledger, capture) {
...
function requireLedgerV11(ledger) {
...
async function captureParentIdentitiesV11(absolutePath, label) {
...
async function assertParentIdentitiesV11(identities, label) {
...
async function assertOpenedDirectoryV11(handle, expected, label) {
...
async function assertExpectedRootV11(absolutePath, expectedRoot, label, leafExists) {
...
function failure(code, message) {
  return new RuntimeIoV11Error(`RUNTIME_IO_V11_${code}`, message);
}
