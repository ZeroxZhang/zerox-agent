...
} from "./check-conversation-disclosure-continuation-v12.mjs";

export async function checkConversationDisclosureProgramV12({
  ...
  const successorFeature = featureList.features?.find(
    (entry) => entry?.id === CONTINUATION_V12_SUCCESSOR_FEATURE_ID,
  );
  const errors = [];

  if (canonicalJsonV12(workstream?.completionArtifacts)
    !== canonicalJsonV12(P107A_V12_COMPLETION_ARTIFACTS)) {
    errors.push("P107A completionArtifacts are not the exact Round12 closure set");
  }
  const rejected = new Set(CONTINUATION_V12_REJECTED_OUTPUT_ABSENT_PATHS);
  ...
  for (const relativePath of CONTINUATION_V12_REJECTED_OUTPUT_ABSENT_PATHS) {
  ...
  const reviewState = workstream?.state === "in_progress"
    ...
    && program.activeFeatureId === CONTINUATION_V12_FEATURE_ID
    && program.nextFeatureId === CONTINUATION_V12_FEATURE_ID;
  const anchoredPlannedState = workstream?.state === "completed"
    && feature?.status === "done"
    && successorWorkstream?.state === "planned"
    && successorFeature === undefined
    && program.activeFeatureId === null
    && program.nextFeatureId === CONTINUATION_V12_SUCCESSOR_FEATURE_ID;
  const authorizedActiveState = workstream?.state === "completed"
    ...
    && program.activeFeatureId === CONTINUATION_V12_SUCCESSOR_FEATURE_ID
    && program.nextFeatureId === CONTINUATION_V12_SUCCESSOR_FEATURE_ID;
  if (!reviewState && !anchoredPlannedState && !authorizedActiveState) {
    errors.push("P107A/P108 lifecycle is not an admitted V12 state");
  }
  if (successorFeature?.status === "done") {
    errors.push("P108 completion requires a separately reviewed delta head");
  }
  if (reviewState) {
    for (const transition of CONTINUATION_V12_GOVERNANCE_TRANSITION_TRUST_ROOTS) {
      try {
        const live = await readFile(path.join(root, transition.path));
        const staged = await readFile(
          path.join(root, transition.stagedTargetPath),
        );
        if (sha256BytesV12(live) !== transition.fromSha256
          || sha256BytesV12(staged) !== transition.toSha256) {
  ...
  if (workstream?.state === "completed") {
    for (const relativePath of P107A_V12_COMPLETION_ARTIFACTS) {
      try {
    ...
    if (!path.isAbsolute(baseAnchor ?? "")
      ...
      const [policy, snapshot] = await Promise.all([
        readJson(root, CONTINUATION_V12_POLICY_PATH, "Round12 policy"),
        readJson(root, CONTINUATION_V12_REVIEW_SNAPSHOT_PATH, "Round12 snapshot"),
      ]);
      try {
        await runConversationDisclosureContinuationCheckerV12([
          "--mode",
  ...
  return {
    ...
    phase: reviewState
      ...
      : anchoredPlannedState
        ? "anchored_planned"
        : "authorized_active",
    completionArtifactCount: P107A_V12_COMPLETION_ARTIFACTS.length,
    rejectedOutputCount: CONTINUATION_V12_REJECTED_OUTPUT_ABSENT_PATHS.length,
  };
}
...
if (process.argv[1]
  ...
  checkConversationDisclosureProgramV12().then((result) => {
    process.stdout.write(
      `Conversation disclosure program V12 check passed (${result.phase}, ${result.completionArtifactCount} completion artifacts, ${result.rejectedOutputCount} rejected outputs).\n`,
    );
  }).catch((error) => {
    process.stderr.write("Conversation disclosure program V12 check failed:\n");
