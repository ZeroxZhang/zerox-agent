...
export const GOVERNANCE_V7_INPUT_KIND =
  "conversation-disclosure-program-governance-v7-input";
export const GOVERNANCE_V7_RESULT_KIND =
  "conversation-disclosure-program-governance-v7-result";
...
const P107A_WORKSTREAM_ID = CONTINUATION_V7_WORKSTREAM_ID;
const P107A_FEATURE_ID = CONTINUATION_V7_FEATURE_ID;
const P108_WORKSTREAM_ID = CONTINUATION_V7_SUCCESSOR_WORKSTREAM_ID;
const P108_FEATURE_ID = CONTINUATION_V7_SUCCESSOR_FEATURE_ID;
export const P107A_V7_COMPLETION_ARTIFACTS = Object.freeze([
...
export const PROGRAM_GOVERNANCE_V7_RULE_LEDGER = Object.freeze([
...
export const PROGRAM_GOVERNANCE_V7_RULE_IDS = Object.freeze(
  PROGRAM_GOVERNANCE_V7_RULE_LEDGER.map((rule) => rule.id),
);

export function validateConversationDisclosureProgramGovernanceV7(input) {
  const issues = new Map(PROGRAM_GOVERNANCE_V7_RULE_IDS.map((id) => [id, []]));
  ...
  run("CDG4-001-input-contract", () => {
    ...
    if (!sameOrderedStrings(Object.keys(value).sort(), expectedKeys)) {
      add("CDG4-001-input-contract", "input keys must exactly match the v7 contract");
    }
    if (!sameOrderedStrings(Object.keys(roster).sort(), [
      ...
      "schemaVersion",
      "workstreamIds",
    ]) || roster.schemaVersion !== 7) {
      add("CDG4-001-input-contract", "unknown closed-world governance schema");
    }
    if (!sameOrderedStrings(roster.ruleIds, PROGRAM_GOVERNANCE_V7_RULE_IDS)) {
      add("CDG4-001-input-contract", "closedWorld.ruleIds must exactly match the known v7 ledger");
    }
  ...
  run("CDG4-011-program-root-contract", () => {
    if (!plainObject(roster.programRootDefinition)
      ...
      || roster.programRootDefinitionDigest
        !== hashCanonicalV7(roster.programRootDefinition)) {
      add("CDG4-011-program-root-contract", "frozen Program root binding is invalid");
      return;
    }
    const liveDefinition = stableProgramRootDefinitionV7(program);
    if (canonicalJsonV7(liveDefinition)
        !== canonicalJsonV7(roster.programRootDefinition)
      || hashCanonicalV7(liveDefinition) !== roster.programRootDefinitionDigest) {
      add(
  ...
  run("CDG4-054-completion-artifacts", () => {
    ...
    if (!sameOrderedStrings(
      ...
      add(
        ...
        "P107A completionArtifacts must match the exact satisfiable Round7 closure set",
      );
    }
    const forbidden = new Set(CONTINUATION_V7_REJECTED_OUTPUT_ABSENT_PATHS);
  ...
  const ruleResults = PROGRAM_GOVERNANCE_V7_RULE_LEDGER.map((rule) => {
  ...
  return Object.freeze({
    schemaVersion: 7,
    kind: GOVERNANCE_V7_RESULT_KIND,
...
export function createProgramRootBindingV7(program) {
  const programRootDefinition = stableProgramRootDefinitionV7(program);
  return {
    programRootDefinition,
    programRootDefinitionDigest: hashCanonicalV7(programRootDefinition),
  };
}
