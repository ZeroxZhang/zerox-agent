...
// CD03A external closure runner v3. This file is intentionally self-contained:
...
const ROUND2_PREFREEZE_REJECTION_PATH =
...
const JOURNAL_KIND =
  "conversation-disclosure-continuation-publication-transaction-v3";
...
const POLICY_KEYS = Object.freeze([
  "admission", "admissionCoverage", "algorithm", "baselineArchive", "closedWorld",
  "continuationExecutables", "digest", "featureId", "governanceTransitions",
  "kind", "parentEvidence", "pathAuthorities", "policyId", "programId",
  "reviewSnapshot", "round", "round1Rejection", "round2PrefreezeRejection",
  "schemaVersion", "status", "successor",
  "timePolicy", "trustRoots", "workstreamId",
]);
const SNAPSHOT_KEYS = Object.freeze([
  "absentPaths", "admissionFeatureDefinitionDigest", "admissionFeatureFileSetDigest",
  "algorithm", "baselineArchive", "baselineFiles", "closedWorldDigest", "digest",
  "featureId", "frozenAt", "frozenFiles", "governanceTransitions", "kind",
  "parentEvidenceBundleDigest", "pathAuthorityDigest", "policyDigest", "programId",
  "reviewOutputAbsentPaths", "round", "schemaVersion", "successorFeatureDefinitionDigest",
  "successorWorkstreamDefinitionDigest", "transitionPayloadFiles", "workstreamId",
]);
const MANIFEST_KEYS = Object.freeze([
  "digest", "externalAttestation", "externalRunner", "featureId", "kind",
  "parentEvidenceBundleDigest", "policy", "programId", "reviewReceipts", "round",
...
const CONTROL_SET_KIND = "conversation-disclosure-continuation-control-set-v3";
...
if (options.journalOutput !== `${options.externalAnchorOutput}.closure-v3.journal.json`) {
...
async function buildFreshTransaction() {
  ...
  const policyCapture = await captureRepositoryFile(POLICY_PATH, "continuation policy");
  ...
  await captureAndValidateRound2PrefreezeRejection(policy);
  ...
  if (transitions.length !== 4) {
    fail("runner v3 requires exactly four governance transitions");
  }
  ...
  const attestation = withDigest({
    ...
    reviewReceiptBindings: receiptReferences,
    candidateResults,
  });
...
async function captureAndValidateBaselineArchive(policy) {
  ...
  const capture = await captureRepositoryFile(reference.path, "baseline archive");
  ...
  for (const [index, entry] of archive.entries.entries()) {
    if (!exactKeys(entry, ARCHIVE_ENTRY_KEYS)
      ...
      || (index > 0 && entry.path <= previousPath)) {
...
async function captureAndValidateRound2PrefreezeRejection(policy) {
  const embedded = policy.round2PrefreezeRejection;
  const capture = await captureRepositoryFile(
    ROUND2_PREFREEZE_REJECTION_PATH,
    "Round2 pre-freeze rejection",
    { requirePrivate: true },
  );
  const rejection = parseJson(capture.bytes, "Round2 pre-freeze rejection");
  validateCanonicalObject(rejection, "Round2 pre-freeze rejection");
  if (canonicalJson(rejection) !== canonicalJson(embedded)) {
    fail("continuation policy Round2 pre-freeze rejection binding is stale");
  }
  validateRound2PrefreezeRejection(rejection, policy);
}

function validateRound2PrefreezeRejection(rejection, policy) {
  const expectedKeys = [
    "algorithm", "baselineArchive", "contradiction", "continuationExecutables",
    "digest", "featureId", "governanceTransitions", "kind", "liveTransitionFiles",
    "programId", "recoveryRound", "rejectedRound", "schemaVersion", "sourcePolicy",
    "status", "transitionPayloadFiles", "verifiedAbsentPaths", "workstreamId",
  ];
  if (!exactKeys(rejection, expectedKeys)
    || rejection.schemaVersion !== 3
    || rejection.kind !== "conversation-disclosure-continuation-prefreeze-rejection"
    ...
    || rejection.recoveryRound !== 3
    || rejection.rejectedRound !== 2
    || rejection.status !== "rejected_pre_freeze"
    || !Array.isArray(rejection.liveTransitionFiles)
    || !Array.isArray(rejection.transitionPayloadFiles)
    || !Array.isArray(rejection.verifiedAbsentPaths)) {
    fail("Round2 pre-freeze rejection schema or lifecycle binding is stale");
  }
}
...
async function validateSnapshotTransitionPayloadFiles(snapshot, policy) {
  ...
  for (const [index, entry] of actual.entries()) {
    if (!exactKeys(entry, ["path", "sha256"])
      || !repositoryPath(entry.path)
      || !sha256Digest(entry.sha256)
      || (index > 0 && entry.path <= actual[index - 1].path)
...
function validateSnapshotReviewOutputPathArray(snapshot) {
  ...
  if (!Array.isArray(reviewOutputs)
    ...
    || reviewOutputs.some((entry, index) => index > 0 && entry <= reviewOutputs[index - 1])) {
...
async function convergePreparedTransaction(journal, { skipFaults }) {
  ...
  for (const [index, transition] of journal.governanceTransitions.entries()) {
    const requestedFault = skipFaults
      ? ""
      : process.env.ZEROX_CD03A_RUNNER_V3_TEST_FAULT ?? "";
    ...
    if (fault !== "none") fail(`injected runner v3 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
  ...
  for (const publication of journal.publications) {
    ...
    if (bridgeFault !== "none") fail(`injected runner v3 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
...
async function executeCandidatesInStage({ stagedTree, policy, baseAnchor, snapshot }) {
  const stageRoot = await realpath(
    await mkdtemp(path.join(os.tmpdir(), "zerox-cd03a-v3-stage-")),
  );
  ...
  try {
    ...
    for (const [kind, relativePath] of [["checker", checkerPath], ["harness", harnessPath]]) {
      ...
      const receiptKind = kind === "checker"
        ? "cd03a-continuation-checker-v3-receipt"
        : "cd03a-continuation-harness-v3-receipt";
...
async function removeValidatedStageRoot(stageRoot, controlRoot) {
  ...
  if (path.dirname(stageRealpath) !== temporaryRoot
    || !path.basename(stageRealpath).startsWith("zerox-cd03a-v3-stage-")
...
async function captureRepositoryTree(root) {
  ...
  const excluded = new Set([".git", "node_modules", "dist", "dist-electron"]);
...
function validateJournal(journal, caller, repoRealpath, runnerDigest) {
  ...
  if (!exactKeys(journal, JOURNAL_KEYS)
    || journal.schemaVersion !== 3 || journal.kind !== JOURNAL_KIND
  ...
  if (!exactKeys(embeddedSnapshot, SNAPSHOT_KEYS)
    || embeddedSnapshot.schemaVersion !== 3
  ...
  if (!exactKeys(embeddedPendingManifest, MANIFEST_KEYS)
    ...
    || embeddedPendingManifest.status !== "review_passed_pending_external_anchor") {
  ...
  if (!exactKeys(manifest, MANIFEST_KEYS)
    ...
    || manifest.policy?.digest !== caller.expectedPolicyDigest
    || manifest.snapshot?.digest !== caller.expectedSnapshotDigest
  ...
  if (attestation?.schemaVersion !== 3
    ...
    || attestation?.pendingManifestDigest !== caller.expectedPendingManifestDigest
...
async function revalidatePreparedJournalAgainstSources(journal) {
  ...
  const snapshotCapture = await captureRepositoryFile(SNAPSHOT_PATH, "review snapshot");
  ...
  if (!exactKeys(pendingManifest, MANIFEST_KEYS)
    ...
    || pendingManifest.policy?.digest !== policy.digest
    || pendingManifest.snapshot?.digest !== snapshot.digest
...
function validateReviewSet(receipts) {
  if (receipts.length !== 3
    ...
    || new Set(receipts.map((receipt) => receipt.reviewTaskPath)).size !== 3
    || new Set(receipts.map((receipt) => receipt.reviewAgentId)).size !== 3) {
    fail("review set must contain three independent lane/task/agent/challenge identities");
  }
  for (const lane of REQUIRED_LANES) {
...
function validateGeneratedEvidence({ attestation, finalManifest, anchor, manifest, policy, snapshot }) {
  ...
  if (attestation.pendingManifestDigest !== manifest.digest
...
function parseOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    fail(`unknown runner v3 option: ${argument}`);
  }
  for (const key of [
    ...
    if (!values[key]) fail(`missing required runner v3 option: ${key}`);
  }
...
function injectFault(point) {
  if (process.env.ZEROX_CD03A_RUNNER_V3_TEST_FAULT === point) {
    fail(`injected runner v3 fault: ${point}`);
  }
}
...
function printPublicationReceipt(journal, recovered) {
  ...
  const receiptWithoutDigest = {
    kind: "cd03a-external-publication-v3-receipt",
    status: "passed",
    recovered,
...
function fail(message) {
  console.error("CD03A external continuation runner v3 failed:");
