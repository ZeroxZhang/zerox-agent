...
import {
  ...
  CONTINUATION_V7_CALLER_DISPATCH_ASSURANCE,
...
const LANE_SCOPE = Object.freeze({
  contract:
    "Audit exact V7 schemas and semantic bindings, including candidate results, trusted time, final-manifest projection, complete roots, the exact successor-admission head, and rejection of P108 done.",
  runtime:
...
export function buildReviewDispatchArtifactsV7({
  policy,
  snapshot,
  challenges,
}) {
  if (!sha256DigestV7(policy?.digest)
    || !sha256DigestV7(snapshot?.digest)
    || snapshot.policyDigest !== policy.digest
    || !sha256DigestV7(policy?.round6ReviewRejection?.digest)) {
    throw new Error("dispatch builder requires one bound V7 policy and snapshot");
  }
  const challengeMap = new Map(challenges ?? []);
  if (challengeMap.size !== CONTINUATION_V7_REVIEW_LANES.length
    || CONTINUATION_V7_REVIEW_LANES.some(
      (lane) => !sha256DigestV7(challengeMap.get(lane)),
    )
  ...
  const instructions = [];
  const entries = CONTINUATION_V7_REVIEW_LANES.map((lane) => {
    const challenge = challengeMap.get(lane);
    const reviewContextId = `round7-${lane}-${challenge.slice(7, 23)}`;
    const taskPath = `round7/${lane}`;
    const agentLabel = `round7-${lane}-reviewer`;
    const text = [
      "READ-ONLY independent Round7 review.",
      "Do not edit, publish, chmod, rename, or delete any file.",
      `Policy: ${policy.digest}`,
      `Snapshot: ${snapshot.digest}`,
      `Rejected parent witness: ${policy.round6ReviewRejection.digest}`,
    ...
    const instructionDigest = sha256BytesV7(bytes);
    instructions.push({
      lane,
    ...
    return {
      lane,
      assurance: CONTINUATION_V7_CALLER_DISPATCH_ASSURANCE,
      challenge,
      instructionDigest,
      reviewContextId,
      taskPath,
  ...
  return {
    entries,
    dispatchSetDigest: hashCanonicalV7(entries),
    instructions,
  };
}

export async function buildConversationDisclosureReviewDispatchV7({
  ...
  const readPrivateJson = async (relativePath, label) =>
    JSON.parse((await capturePrivateEvidenceV7(
      path.join(root, relativePath),
      label,
      { expectedRoot: root, ledger },
    )).bytes.toString("utf8"));
  const [policy, archive, rejection, snapshot] = await Promise.all([
    readPrivateJson(CONTINUATION_V7_POLICY_PATH, "Round7 policy"),
    readPrivateJson(CONTINUATION_V7_BASELINE_ARCHIVE_PATH, "Round7 archive"),
    readPrivateJson(
      CONTINUATION_V7_ROUND6_REVIEW_REJECTION_PATH,
      "Round6 review rejection",
    ),
    readPrivateJson(CONTINUATION_V7_REVIEW_SNAPSHOT_PATH, "Round7 snapshot"),
  ]);
  ...
  assertNoErrors(validateBaselineArchiveV7(archive, policy), "Round7 archive");
  ...
  const artifacts = buildReviewDispatchArtifactsV7({
    policy,
    snapshot,
    challenges,
  });
  ...
  for (const instruction of artifacts.instructions) {
    publications.push(await publishPrivateExactV7(
      path.join(outputRoot, instruction.filename),
      instruction.bytes,
      { label: `Round7 ${instruction.lane} review instruction` },
    ));
  }
  ...
  publications.push(await publishPrivateExactV7(
    ...
    { label: "Round7 caller dispatch set" },
  ));
  return {
    ...artifacts,
...
if (process.argv[1]
  ...
  buildConversationDisclosureReviewDispatchV7(
