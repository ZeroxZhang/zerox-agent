...
export async function freezeConversationDisclosureContinuationV5({
  repositoryRoot = process.cwd(),
  expectedPolicyDigest,
  frozenAt,
  outputPath = CONTINUATION_V5_REVIEW_SNAPSHOT_PATH,
} = {}) {
  ...
  const [policy, rejection, archive] = await Promise.all([
    readJson(CONTINUATION_V5_POLICY_PATH, "Round5 policy", {
    ...
    readJson(CONTINUATION_V5_BASELINE_ARCHIVE_PATH, "Round5 baseline archive", {
      privateEvidence: true,
    }),
  ]);
  ...
  const byClass = new Map(
    CONTINUATION_V5_ADMISSION_CLASSES.map((entry) => [entry, []]),
  );
  ...
  for (const relativePath of byClass.get("frozen_file")) {
    ...
    const capture = await captureRepository(
      relativePath,
      `Round5 frozen subject ${relativePath}`,
      { privateEvidence },
    );
  ...
  for (const transition of policy.governanceTransitions) {
    const [live, payload] = await Promise.all([
      captureRepository(
        transition.path,
        `Round5 transition live ${transition.path}`,
      ),
      captureRepository(
        transition.stagedTargetPath,
        `Round5 transition payload ${transition.stagedTargetPath}`,
      ),
    ]);
  ...
  for (const authority of policy.pathAuthorities) {
    if (authority.class === "modify"
      ...
      const capture = await captureRepository(
        authority.path,
        `Round5 authority baseline ${authority.path}`,
      );
      if (capture.digest !== authority.baseline.sha256) {
        throw new Error(`Round5 authority baseline drifted: ${authority.path}`);
      }
      ...
      if (authority.baseline.presence === "present") {
        const capture = await captureRepository(
          authority.path,
          `Round5 bookkeeping baseline ${authority.path}`,
        );
        if (capture.digest !== authority.baseline.sha256) {
          throw new Error(`Round5 bookkeeping baseline drifted: ${authority.path}`);
        }
        baselineFiles.push({ path: authority.path, sha256: capture.digest });
      } else {
        absentPaths.add(authority.path);
  ...
  for (const relativePath of reviewOutputAbsentPaths) {
    if (relativePath === CONTINUATION_V5_REVIEW_SNAPSHOT_PATH
      && existingSnapshot) {
      continue;
    }
    await captureRequiredAbsentV5(
      path.join(root, relativePath),
      `Round5 review output absence ${relativePath}`,
      { expectedRoot: root, ledger },
    );
  }

  const snapshot = withCanonicalDigestV5({
    schemaVersion: 5,
    kind: CONTINUATION_V5_SNAPSHOT_KIND,
    ...
    pathAuthorityDigest: hashCanonicalV5(policy.pathAuthorities),
  ...
  assertNoErrors(validateContinuationReviewSnapshotV5(
    snapshot,
    policy,
    { verifierNow: Date.parse(frozenAt) },
  ), "generated Round5 review snapshot");
  const bytes = Buffer.from(`${JSON.stringify(snapshot, null, 2)}\n`, "utf8");
  if (existingSnapshot
    && !Buffer.from(`${JSON.stringify(existingSnapshot, null, 2)}\n`, "utf8")
      .equals(bytes)) {
    throw new Error("existing Round5 snapshot differs from deterministic bytes");
  }
  await postflightCaptureLedgerV5(ledger);
  const publication = await publishPrivateExactV5(outputAbsolute, bytes, {
    expectedRoot: root,
    label: "Round5 review snapshot",
  });
  return {
    snapshot,
    bytes,
...
function resolveExactOutput(root, outputPath) {
  ...
  const expected = path.join(root, CONTINUATION_V5_REVIEW_SNAPSHOT_PATH);
  if (resolved !== expected) {
    throw new Error("--output must be the exact Round5 review snapshot path");
  }
  return resolved;
}
...
if (process.argv[1]
  ...
  freezeConversationDisclosureContinuationV5(
