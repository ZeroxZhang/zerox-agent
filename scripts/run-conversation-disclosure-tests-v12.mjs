...
export const ROUND12_TRANSITION_MAPPINGS = Object.freeze([
  Object.freeze({
    source:
...
async function main() {
  ...
  try {
    const transitionState = await detectRound12TransitionState(root);
    let currentRoot = root;
    if (transitionState === "target") {
      currentRoot = await createFixture("zerox-r12-current-source-");
      await restoreCurrentRound12Sources(currentRoot);
    }
    ...
    const round11Root = await createFixture("zerox-r12-v11-history-");
    await restoreRound11Semantics(round11Root);
    await runVitest(round11Root, ["--run", ...historicalV11Tests]);

    const targetRoot = await createFixture("zerox-r12-target-tests-");
    await applyRound12Targets(targetRoot);
...
async function restoreRound11Semantics(fixture) {
  const policyPath = path.join(
    fixture,
...
export async function detectRound12TransitionState(repositoryRoot) {
  const states = [];
  for (const transition of ROUND12_TRANSITION_MAPPINGS) {
    const targetBytes = await readFile(path.join(repositoryRoot, transition.source));
    if (sha256(targetBytes) !== transition.toSha256) {
      throw new Error(
        `Round12 transition payload is stale: ${transition.source}`,
      );
    }
    ...
    if (liveDigest === transition.fromSha256) {
      ...
      throw new Error(
        `Round12 transition live bytes are a third state: ${transition.destination}`,
      );
    }
  }
  const uniqueStates = new Set(states);
  if (uniqueStates.size !== 1) {
    throw new Error("Round12 transition live bytes are mixed");
  }
  return states[0];
}

export async function restoreCurrentRound12Sources(fixture) {
  const archivePath =
    ".zerox/verification/conversation-disclosure/CD03A-round11-baseline-archive.json";
  ...
  for (const transition of ROUND12_TRANSITION_MAPPINGS) {
    ...
    if (!entry || entry.sha256 !== transition.fromSha256
      || entry.encoding !== "gzip-base64-v1") {
      throw new Error(
        `Round11 archive misses Round12 source: ${transition.destination}`,
      );
    }
    const bytes = gunzipSync(Buffer.from(entry.bytes, "base64"));
    if (sha256(bytes) !== transition.fromSha256) {
      throw new Error(
        `Round11 archive source bytes are stale: ${transition.destination}`,
      );
    }
    await writeFile(path.join(fixture, transition.destination), bytes);
  }
  if (await detectRound12TransitionState(fixture) !== "source") {
    throw new Error("Round12 source reconstruction did not converge");
  }
}

async function applyRound12Targets(fixture) {
  for (const { source, destination } of ROUND12_TRANSITION_MAPPINGS) {
    await writeFile(
