...
// CD03A external closure runner v6. This file is intentionally self-contained:
...
const ROUND5_REVIEW_REJECTION_PATH =
...
const ROUND5_POLICY_ROOT = Object.freeze({
  path:
...
const ROUND5_SNAPSHOT_ROOT = Object.freeze({
  path:
...
const ROUND5_RECEIPT_ROOTS = Object.freeze([
  Object.freeze({
    lane: "governance",
    path:
...
const ROUND5_FINDING_IDS = Object.freeze([
  "R5-GOV-001",
  "R5-GOV-002",
  "R5-GOV-003",
]);
const ROUND5_FINDING_SET_DIGEST =
  "sha256:8375d6cfc0e37d16dba46e6e26684a4c80564f4068dad34c4e777ed387d25169";
const ROUND5_FINDING_COUNTS = Object.freeze({
  critical: 1,
  major: 2,
  minor: 0,
});
const ROUND5_FORBIDDEN_OUTPUTS = Object.freeze([
...
const JOURNAL_KIND =
  "conversation-disclosure-continuation-publication-transaction-v6";
...
const POLICY_KEYS = Object.freeze([
  ...
  "round1Rejection", "round2PrefreezeRejection", "round5ReviewRejection",
...
const SNAPSHOT_KEYS = Object.freeze([
  ...
  "round5ReviewRejectionDigest", "schemaVersion",
...
const MANIFEST_KEYS = Object.freeze([
  ...
  "policy", "programId", "reviewReceipts", "round", "round5ReviewRejection",
...
const CONTROL_SET_KIND = "conversation-disclosure-continuation-control-set-v6";
...
const RECEIPT_KEYS = Object.freeze([
  ...
  "round5ReviewRejectionDigest", "schemaVersion", "snapshotDigest",
...
const ATTESTATION_KEYS = Object.freeze([
  ...
  "repositoryRealpath", "reviewAssurance", "round5ReviewRejectionDigest",
...
if (options.journalOutput !== `${options.externalAnchorOutput}.closure-v6.journal.json`) {
...
async function buildFreshTransaction() {
  ...
  await captureAndValidateRound5ReviewRejection(policy);
  ...
  if (!exactKeys(snapshot, SNAPSHOT_KEYS)
    ...
    || snapshot.round5ReviewRejectionDigest
      !== policy.round5ReviewRejection?.digest
  ...
  if (transitions.length !== 4) {
    fail("runner v6 requires exactly four governance transitions");
  }
...
async function captureAndValidateRound5ReviewRejection(policy) {
  const embedded = policy.round5ReviewRejection;
  const capture = await captureRepositoryFile(
    ROUND5_REVIEW_REJECTION_PATH,
    "Round5 review-rejection witness",
    { requirePrivate: true },
  );
  const rejection = parseJson(capture.bytes, "Round5 review-rejection witness");
  validateCanonicalObject(rejection, "Round5 review-rejection witness");
  if (canonicalJson(rejection) !== canonicalJson(embedded)) {
    fail("continuation policy Round5 review-rejection binding is stale");
  }
  await validateRound5ReviewRejection(rejection, policy);
}

async function validateRound5ReviewRejection(rejection, policy) {
  const expectedKeys = [
  ...
  if (!exactKeys(rejection, expectedKeys)
    ...
    || canonicalJson(rejection.sourcePolicy) !== canonicalJson(ROUND5_POLICY_ROOT)
    || canonicalJson(rejection.sourceSnapshot)
      !== canonicalJson({
        ...ROUND5_SNAPSHOT_ROOT,
        frozenFileCount: 121,
        transitionPayloadFileCount: 4,
        baselineFileCount: 12,
      })
    || canonicalJson(rejection.findingIds) !== canonicalJson(ROUND5_FINDING_IDS)
    || rejection.findingSetDigest !== ROUND5_FINDING_SET_DIGEST
    || canonicalJson(rejection.aggregateFindingCounts)
      !== canonicalJson(ROUND5_FINDING_COUNTS)
    || canonicalJson(rejection.repositoryForbiddenOutputs)
      !== canonicalJson(ROUND5_FORBIDDEN_OUTPUTS)
    || !Array.isArray(rejection.failedReceipts)
    || rejection.failedReceipts.length !== 1) {
    fail("Round5 review-rejection schema or lifecycle binding is stale");
  }
  for (const expected of [
    ROUND5_POLICY_ROOT,
    ROUND5_SNAPSHOT_ROOT,
    ...ROUND5_RECEIPT_ROOTS,
  ]) {
    const current = await captureRepositoryFile(
      expected.path,
      `Round5 rejection source ${expected.path}`,
      { requirePrivate: expected === ROUND5_POLICY_ROOT
        || expected === ROUND5_SNAPSHOT_ROOT
        || ROUND5_RECEIPT_ROOTS.includes(expected) },
    );
    if (current.digest !== expected.byteSha256) {
      fail(`Round5 rejection source bytes drifted: ${expected.path}`);
    }
    const value = parseJson(current.bytes, `Round5 rejection source ${expected.path}`);
    const canonicalDigest = value.digest ?? hashCanonical(value);
    if (canonicalDigest !== expected.canonicalDigest) {
      fail(`Round5 rejection source canonical digest drifted: ${expected.path}`);
    }
  }
  for (let index = 0; index < ROUND5_RECEIPT_ROOTS.length; index += 1) {
    const actual = rejection.failedReceipts[index];
    const expected = ROUND5_RECEIPT_ROOTS[index];
    if (actual?.lane !== expected.lane
      || actual?.path !== expected.path
      || actual?.byteSha256 !== expected.byteSha256
      || actual?.canonicalDigest !== expected.canonicalDigest) {
      fail(`Round5 rejection receipt root is stale: ${expected.lane}`);
    }
  }
  for (const relativePath of rejection.repositoryForbiddenOutputs ?? []) {
    if (await repositoryEntryExists(repositoryRealpath, relativePath)) {
      fail(`Round5 forbidden repository output exists: ${relativePath}`);
    }
  }
}
...
async function convergePreparedTransaction(journal, { skipFaults }) {
  ...
  for (const [index, transition] of journal.governanceTransitions.entries()) {
    const requestedFault = skipFaults
      ? ""
      : process.env.ZEROX_CD03A_RUNNER_V6_TEST_FAULT ?? "";
    ...
    if (fault !== "none") fail(`injected runner v6 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
  ...
  for (const publication of journal.publications) {
    ...
    if (bridgeFault !== "none") fail(`injected runner v6 fault: ${requestedFault}`);
    await validateRepositoryControlState(journal, {
...
async function executeCandidatesInStage({ stagedTree, policy, baseAnchor, snapshot }) {
  const stageRoot = await realpath(
    await mkdtemp(path.join(os.tmpdir(), "zerox-cd03a-v6-stage-")),
  );
  ...
  try {
    ...
    for (const [kind, relativePath] of [["checker", checkerPath], ["harness", harnessPath]]) {
      ...
      const receiptKind = kind === "checker"
        ? "cd03a-continuation-checker-v6-receipt"
        : "cd03a-continuation-harness-v6-receipt";
...
async function removeValidatedStageRoot(stageRoot, controlRoot) {
  ...
  if (path.dirname(stageRealpath) !== temporaryRoot
    || !path.basename(stageRealpath).startsWith("zerox-cd03a-v6-stage-")
...
function validateJournal(journal, caller, repoRealpath, runnerDigest) {
  ...
  if (!exactKeys(journal, JOURNAL_KEYS)
    || journal.schemaVersion !== 6 || journal.kind !== JOURNAL_KIND
  ...
  if (!exactKeys(embeddedSnapshot, SNAPSHOT_KEYS)
    || embeddedSnapshot.schemaVersion !== 6
  ...
  if (!exactKeys(attestation, ATTESTATION_KEYS)
    ...
    || attestation?.round5ReviewRejectionDigest
      !== embeddedPendingManifest.round5ReviewRejection?.canonicalDigest
  ...
  if (!exactKeys(anchor, ANCHOR_KEYS)
    ...
    || anchor?.round5ReviewRejectionDigest
      !== embeddedPendingManifest.round5ReviewRejection?.canonicalDigest
...
function validateCandidateResultSet(
  results,
  checkerPath = "scripts/check-conversation-disclosure-continuation-v6.mjs",
) {
...
function parseOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    fail(`unknown runner v6 option: ${argument}`);
  }
  for (const key of [
    ...
    if (!values[key]) fail(`missing required runner v6 option: ${key}`);
  }
...
function injectFault(point) {
  if (process.env.ZEROX_CD03A_RUNNER_V6_TEST_FAULT === point) {
    fail(`injected runner v6 fault: ${point}`);
  }
}
...
function printPublicationReceipt(journal, recovered) {
  ...
  const receiptWithoutDigest = {
    kind: "cd03a-external-publication-v6-receipt",
    status: "passed",
    recovered,
...
function fail(message) {
  console.error("CD03A external continuation runner v6 failed:");
