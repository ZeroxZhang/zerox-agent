...
export const GOVERNANCE_V6_INPUT_KIND =
  "conversation-disclosure-program-governance-v6-input";
export const GOVERNANCE_V6_RESULT_KIND =
  "conversation-disclosure-program-governance-v6-result";
...
const P107A_WORKSTREAM_ID = CONTINUATION_V6_WORKSTREAM_ID;
const P107A_FEATURE_ID = CONTINUATION_V6_FEATURE_ID;
const P108_WORKSTREAM_ID = CONTINUATION_V6_SUCCESSOR_WORKSTREAM_ID;
const P108_FEATURE_ID = CONTINUATION_V6_SUCCESSOR_FEATURE_ID;
...
export const PROGRAM_GOVERNANCE_V6_RULE_LEDGER = Object.freeze([
...
export const PROGRAM_GOVERNANCE_V6_RULE_IDS = Object.freeze(
  PROGRAM_GOVERNANCE_V6_RULE_LEDGER.map((rule) => rule.id),
);

export function validateConversationDisclosureProgramGovernanceV6(input) {
  const issues = new Map(PROGRAM_GOVERNANCE_V6_RULE_IDS.map((id) => [id, []]));
  ...
  run("CDG4-001-input-contract", () => {
    ...
    if (!sameOrderedStrings(Object.keys(value).sort(), expectedKeys)) {
      add("CDG4-001-input-contract", "input keys must exactly match the v6 contract");
    }
    if (!sameOrderedStrings(Object.keys(roster).sort(), [
      ...
      "schemaVersion",
      "workstreamIds",
    ]) || roster.schemaVersion !== 6) {
      add("CDG4-001-input-contract", "unknown closed-world governance schema");
    }
    if (!sameOrderedStrings(roster.ruleIds, PROGRAM_GOVERNANCE_V6_RULE_IDS)) {
      add("CDG4-001-input-contract", "closedWorld.ruleIds must exactly match the known v6 ledger");
    }
  ...
  run("CDG4-011-program-root-contract", () => {
    if (!plainObject(roster.programRootDefinition)
      ...
      || roster.programRootDefinitionDigest
        !== hashCanonicalV6(roster.programRootDefinition)) {
      add("CDG4-011-program-root-contract", "frozen Program root binding is invalid");
      return;
    }
    const liveDefinition = stableProgramRootDefinitionV6(program);
    if (canonicalJsonV6(liveDefinition)
        !== canonicalJsonV6(roster.programRootDefinition)
      || hashCanonicalV6(liveDefinition) !== roster.programRootDefinitionDigest) {
      add(
  ...
  const ruleResults = PROGRAM_GOVERNANCE_V6_RULE_LEDGER.map((rule) => {
  ...
  return Object.freeze({
    schemaVersion: 6,
    kind: GOVERNANCE_V6_RESULT_KIND,
...
export function createProgramRootBindingV6(program) {
  const programRootDefinition = stableProgramRootDefinitionV6(program);
  return {
    programRootDefinition,
    programRootDefinitionDigest: hashCanonicalV6(programRootDefinition),
  };
}
