...
export const GOVERNANCE_V5_INPUT_KIND =
  "conversation-disclosure-program-governance-v5-input";
export const GOVERNANCE_V5_RESULT_KIND =
  "conversation-disclosure-program-governance-v5-result";
...
const P107A_WORKSTREAM_ID = CONTINUATION_V5_WORKSTREAM_ID;
const P107A_FEATURE_ID = CONTINUATION_V5_FEATURE_ID;
const P108_WORKSTREAM_ID = CONTINUATION_V5_SUCCESSOR_WORKSTREAM_ID;
const P108_FEATURE_ID = CONTINUATION_V5_SUCCESSOR_FEATURE_ID;
...
export const PROGRAM_GOVERNANCE_V5_RULE_LEDGER = Object.freeze([
...
export const PROGRAM_GOVERNANCE_V5_RULE_IDS = Object.freeze(
  PROGRAM_GOVERNANCE_V5_RULE_LEDGER.map((rule) => rule.id),
);

export function validateConversationDisclosureProgramGovernanceV5(input) {
  const issues = new Map(PROGRAM_GOVERNANCE_V5_RULE_IDS.map((id) => [id, []]));
  ...
  run("CDG4-001-input-contract", () => {
    ...
    if (!sameOrderedStrings(Object.keys(value).sort(), expectedKeys)) {
      add("CDG4-001-input-contract", "input keys must exactly match the v5 contract");
    }
    if (!sameOrderedStrings(Object.keys(roster).sort(), [
      ...
      "schemaVersion",
      "workstreamIds",
    ]) || roster.schemaVersion !== 5) {
      add("CDG4-001-input-contract", "unknown closed-world governance schema");
    }
    if (!sameOrderedStrings(roster.ruleIds, PROGRAM_GOVERNANCE_V5_RULE_IDS)) {
      add("CDG4-001-input-contract", "closedWorld.ruleIds must exactly match the known v5 ledger");
    }
  ...
  run("CDG4-011-program-root-contract", () => {
    if (!plainObject(roster.programRootDefinition)
      ...
      || roster.programRootDefinitionDigest
        !== hashCanonicalV5(roster.programRootDefinition)) {
      add("CDG4-011-program-root-contract", "frozen Program root binding is invalid");
      return;
    }
    const liveDefinition = stableProgramRootDefinitionV5(program);
    if (canonicalJsonV5(liveDefinition)
        !== canonicalJsonV5(roster.programRootDefinition)
      || hashCanonicalV5(liveDefinition) !== roster.programRootDefinitionDigest) {
      add(
  ...
  const ruleResults = PROGRAM_GOVERNANCE_V5_RULE_LEDGER.map((rule) => {
  ...
  return Object.freeze({
    schemaVersion: 5,
    kind: GOVERNANCE_V5_RESULT_KIND,
...
export function createProgramRootBindingV5(program) {
  const programRootDefinition = stableProgramRootDefinitionV5(program);
  return {
    programRootDefinition,
    programRootDefinitionDigest: hashCanonicalV5(programRootDefinition),
  };
}
