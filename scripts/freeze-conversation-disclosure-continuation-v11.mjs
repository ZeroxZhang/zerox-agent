...
import {
  ...
  CONTINUATION_V11_SNAPSHOT_KIND,
...
export async function freezeConversationDisclosureContinuationV11({
  repositoryRoot = process.cwd(),
  expectedPolicyDigest,
  frozenAt,
  outputPath = CONTINUATION_V11_REVIEW_SNAPSHOT_PATH,
} = {}) {
  ...
  const [policy, rejection, archive] = await Promise.all([
    readJson(CONTINUATION_V11_POLICY_PATH, "Round11 policy", {
    ...
    readJson(CONTINUATION_V11_BASELINE_ARCHIVE_PATH, "Round11 baseline archive", {
      privateEvidence: true,
    }),
  ]);
  ...
  const byClass = new Map(
    CONTINUATION_V11_ADMISSION_CLASSES.map((entry) => [entry, []]),
  );
  ...
  for (const relativePath of byClass.get("frozen_file")) {
    ...
    const capture = await captureRepository(
      relativePath,
      `Round11 frozen subject ${relativePath}`,
      { privateEvidence },
    );
  ...
  for (const transition of policy.governanceTransitions) {
    const [live, payload] = await Promise.all([
      captureRepository(
        transition.path,
        `Round11 transition live ${transition.path}`,
      ),
      captureRepository(
        transition.stagedTargetPath,
        `Round11 transition payload ${transition.stagedTargetPath}`,
      ),
    ]);
  ...
  for (const authority of policy.pathAuthorities) {
    if (authority.class === "modify"
      ...
      const capture = await captureRepository(
        authority.path,
        `Round11 authority baseline ${authority.path}`,
      );
      if (capture.digest !== authority.baseline.sha256) {
        throw new Error(`Round11 authority baseline drifted: ${authority.path}`);
      }
      ...
      if (authority.baseline.presence === "present") {
        const capture = await captureRepository(
          authority.path,
          `Round11 bookkeeping baseline ${authority.path}`,
        );
        if (capture.digest !== authority.baseline.sha256) {
          throw new Error(`Round11 bookkeeping baseline drifted: ${authority.path}`);
        }
        baselineFiles.push({ path: authority.path, sha256: capture.digest });
      } else {
        absentPaths.add(authority.path);
  ...
  for (const relativePath of reviewOutputAbsentPaths) {
    if (relativePath === CONTINUATION_V11_REVIEW_SNAPSHOT_PATH
      && existingSnapshot) {
      continue;
    }
    await captureRequiredAbsentV11(
      path.join(root, relativePath),
      `Round11 review output absence ${relativePath}`,
      { expectedRoot: root, ledger },
    );
  }

  const snapshot = withCanonicalDigestV11({
    schemaVersion: 11,
    kind: CONTINUATION_V11_SNAPSHOT_KIND,
    ...
    pathAuthorityDigest: hashCanonicalV11(policy.pathAuthorities),
  ...
  assertNoErrors(validateContinuationReviewSnapshotV11(
    snapshot,
    policy,
    { verifierNow: Date.parse(frozenAt) },
  ), "generated Round11 review snapshot");
  const bytes = Buffer.from(`${JSON.stringify(snapshot, null, 2)}\n`, "utf8");
  if (existingSnapshot
    && !Buffer.from(`${JSON.stringify(existingSnapshot, null, 2)}\n`, "utf8")
      .equals(bytes)) {
    throw new Error("existing Round11 snapshot differs from deterministic bytes");
  }
  await postflightCaptureLedgerV11(ledger);
  const publication = await publishPrivateExactV11(outputAbsolute, bytes, {
    expectedRoot: root,
    label: "Round11 review snapshot",
  });
  return {
    snapshot,
    bytes,
...
function resolveExactOutput(root, outputPath) {
  ...
  const expected = path.join(root, CONTINUATION_V11_REVIEW_SNAPSHOT_PATH);
  if (resolved !== expected) {
    throw new Error("--output must be the exact Round11 review snapshot path");
  }
  return resolved;
}
...
if (process.argv[1]
  ...
  freezeConversationDisclosureContinuationV11(
