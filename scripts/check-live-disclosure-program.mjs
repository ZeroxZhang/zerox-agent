#!/usr/bin/env node
// Live Process Disclosure program contract check.
//
// Mirrors the runtime-convergence program check: one active workstream at a
// time, dependencies completed before promotion, feature_list agreement, and
// full coverage of the declared findings. Findings are declared by the manifest
// itself (`requiredFindings`) because this program is not derived from a
// numbered external review.

import { access, readFile } from "node:fs/promises";
import path from "node:path";

const root = process.cwd();
const manifestPath = path.join(root, ".zerox", "live-disclosure-program.json");
...
if (
  !Array.isArray(manifest.requiredFindings)
  || manifest.requiredFindings.length === 0
) {
  errors.push("program must declare requiredFindings");
}
...
for (const [index, workstream] of (manifest.workstreams ?? []).entries()) {
  ...
  if (
    workstream.architectureDecisionRequired === true
    && !nonEmpty(workstream.architectureDecision)
  ) {
    errors.push(
      `${subject}.architectureDecision is required when architectureDecisionRequired is true`,
    );
  }
}
...
for (const workstream of workstreams.values()) {
  ...
  if (workstream.state === "planned" && feature?.status === "done") {
    errors.push(
      `planned workstream ${workstream.id} cannot reference a done feature`,
    );
  }
}
...
for (const finding of manifest.requiredFindings ?? []) {
  if (!coveredFindings.has(finding)) {
    errors.push(`program does not cover finding ${finding}`);
  }
}
const declaredFindings = new Set(manifest.requiredFindings ?? []);
for (const finding of coveredFindings) {
  if (!declaredFindings.has(finding)) {
    errors.push(
      `workstream references undeclared finding ${JSON.stringify(finding)}`,
    );
  }
}

if (errors.length > 0) {
  console.error("Live process disclosure program check failed:");
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log(
  `Live process disclosure program check passed (${workstreams.size} workstreams, `
  + `${inProgress.length} active, ${declaredFindings.size} findings).`,
);
