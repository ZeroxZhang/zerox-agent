...
export const PRIVATE_MODE_V12 = 0o600;

const LEDGER_KIND = "conversation-disclosure-runtime-capture-ledger-v12";

export class RuntimeIoV12Error extends Error {
  constructor(code, message) {
    super(message);
    this.name = "RuntimeIoV12Error";
    this.code = code;
  }
}

export function createCaptureLedgerV12() {
  return {
    kind: LEDGER_KIND,
    entries: [],
...
export function sha256BytesV12(bytes) {
...
export async function captureStableFileV12(
  absolutePath,
  label = absolutePath,
  {
    expectedRoot,
  ...
  await assertExpectedRootV12(absolutePath, expectedRoot, label, true);
  const parents = await captureParentIdentitiesV12(absolutePath, label);
  let before;
  try {
  ...
  const requiredMode = requirePrivate ? PRIVATE_MODE_V12 : expectedMode;
  ...
  try {
    ...
    await assertParentIdentitiesV12(parents, label);
    await assertExpectedRootV12(absolutePath, expectedRoot, label, true);
    const capture = {
      kind: "present",
      absolutePath,
      ...
      bytes,
      digest: sha256BytesV12(bytes),
      mode: Number(opened.mode & 0o777n),
      uid: Number(opened.uid),
    };
    recordCaptureV12(ledger, capture);
    return capture;
  } finally {
    await handle.close();
  }
...
export function capturePrivateEvidenceV12(
  absolutePath,
  label = absolutePath,
  options = {},
) {
  return captureStableFileV12(absolutePath, label, {
    ...options,
    requirePrivate: true,
  });
}

export async function captureRequiredAbsentV12(
  absolutePath,
  label = absolutePath,
  { expectedRoot, ledger } = {},
) {
  requireAbsolutePath(absolutePath, label);
  await assertExpectedRootV12(absolutePath, expectedRoot, label, false);
  const parents = await captureParentIdentitiesV12(absolutePath, label);
  try {
...
export async function postflightCaptureLedgerV12(ledger) {
  requireLedgerV12(ledger);
  for (const capture of ledger.entries) {
    await assertParentIdentitiesV12(capture.parents, capture.label);
    await assertExpectedRootV12(
    ...
    const current = await captureStableFileV12(
      capture.absolutePath,
      capture.label,
      {
...
export async function publishPrivateExactV12(
  absolutePath,
  bytes,
  ...
  try {
    ...
    try {
      ...
      if (existingEntry.nlink === 1n) {
        const existing = await capturePrivateEvidenceV12(
          absolutePath,
          label,
          { expectedRoot },
        );
    ...
    const status = await runPrivatePublicationBridgeV12({
      directoryHandle,
      expectedParent,
      basename,
      bytes,
    ...
    await assertOpenedDirectoryV12(directoryHandle, expectedParent, label);
    await assertParentIdentitiesV12(parents, label);
    const published = await capturePrivateEvidenceV12(
      absolutePath,
      label,
      { expectedRoot },
    );
...
async function runPrivatePublicationBridgeV12({
  directoryHandle,
  expectedParent,
  basename,
  bytes,
...
function recordCaptureV12(ledger, capture) {
...
function requireLedgerV12(ledger) {
...
async function captureParentIdentitiesV12(absolutePath, label) {
...
async function assertParentIdentitiesV12(identities, label) {
...
async function assertOpenedDirectoryV12(handle, expected, label) {
...
async function assertExpectedRootV12(absolutePath, expectedRoot, label, leafExists) {
...
function failure(code, message) {
  return new RuntimeIoV12Error(`RUNTIME_IO_V12_${code}`, message);
}
