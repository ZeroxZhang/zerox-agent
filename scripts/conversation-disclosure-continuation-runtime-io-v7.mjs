...
export const PRIVATE_MODE_V7 = 0o600;

const LEDGER_KIND = "conversation-disclosure-runtime-capture-ledger-v7";

export class RuntimeIoV7Error extends Error {
  constructor(code, message) {
    super(message);
    this.name = "RuntimeIoV7Error";
    this.code = code;
  }
}

export function createCaptureLedgerV7() {
  return {
    kind: LEDGER_KIND,
    entries: [],
...
export function sha256BytesV7(bytes) {
...
export async function captureStableFileV7(
  absolutePath,
  label = absolutePath,
  {
    expectedRoot,
  ...
  await assertExpectedRootV7(absolutePath, expectedRoot, label, true);
  const parents = await captureParentIdentitiesV7(absolutePath, label);
  let before;
  try {
  ...
  const requiredMode = requirePrivate ? PRIVATE_MODE_V7 : expectedMode;
  ...
  try {
    ...
    await assertParentIdentitiesV7(parents, label);
    await assertExpectedRootV7(absolutePath, expectedRoot, label, true);
    const capture = {
      kind: "present",
      absolutePath,
      ...
      bytes,
      digest: sha256BytesV7(bytes),
      mode: Number(opened.mode & 0o777n),
      uid: Number(opened.uid),
    };
    recordCaptureV7(ledger, capture);
    return capture;
  } finally {
    await handle.close();
  }
...
export function capturePrivateEvidenceV7(
  absolutePath,
  label = absolutePath,
  options = {},
) {
  return captureStableFileV7(absolutePath, label, {
    ...options,
    requirePrivate: true,
  });
}

export async function captureRequiredAbsentV7(
  absolutePath,
  label = absolutePath,
  { expectedRoot, ledger } = {},
) {
  requireAbsolutePath(absolutePath, label);
  await assertExpectedRootV7(absolutePath, expectedRoot, label, false);
  const parents = await captureParentIdentitiesV7(absolutePath, label);
  try {
...
export async function postflightCaptureLedgerV7(ledger) {
  requireLedgerV7(ledger);
  for (const capture of ledger.entries) {
    await assertParentIdentitiesV7(capture.parents, capture.label);
    await assertExpectedRootV7(
    ...
    const current = await captureStableFileV7(
      capture.absolutePath,
      capture.label,
      {
...
export async function publishPrivateExactV7(
  absolutePath,
  bytes,
  ...
  try {
    ...
    const existing = await captureOptionalPrivateV7(absolutePath, label, expectedRoot);
    if (existing) {
      if (!existing.bytes.equals(bytes)) {
        throw failure("THIRD_STATE", `${label} contains different existing bytes`);
      }
      await assertOpenedDirectoryV7(directoryHandle, expectedParent, label);
      await assertParentIdentitiesV7(parents, label);
    ...
    try {
      ...
      try {
        await temporaryHandle.chmod(PRIVATE_MODE_V7);
      ...
      try {
        ...
        const raced = await capturePrivateEvidenceV7(absolutePath, label, { expectedRoot });
      ...
      await assertOpenedDirectoryV7(directoryHandle, expectedParent, label);
      await assertParentIdentitiesV7(parents, label);
      const published = await capturePrivateEvidenceV7(absolutePath, label, { expectedRoot });
...
async function captureOptionalPrivateV7(absolutePath, label, expectedRoot) {
  try {
    await lstat(absolutePath, { bigint: true });
  } catch (error) {
  ...
  return capturePrivateEvidenceV7(absolutePath, label, { expectedRoot });
}
...
function recordCaptureV7(ledger, capture) {
...
function requireLedgerV7(ledger) {
...
async function captureParentIdentitiesV7(absolutePath, label) {
...
async function assertParentIdentitiesV7(identities, label) {
...
async function assertOpenedDirectoryV7(handle, expected, label) {
...
async function assertExpectedRootV7(absolutePath, expectedRoot, label, leafExists) {
...
function failure(code, message) {
  return new RuntimeIoV7Error(`RUNTIME_IO_V7_${code}`, message);
}
