...
export const PRIVATE_MODE_V6 = 0o600;

const LEDGER_KIND = "conversation-disclosure-runtime-capture-ledger-v6";

export class RuntimeIoV6Error extends Error {
  constructor(code, message) {
    super(message);
    this.name = "RuntimeIoV6Error";
    this.code = code;
  }
}

export function createCaptureLedgerV6() {
  return {
    kind: LEDGER_KIND,
    entries: [],
...
export function sha256BytesV6(bytes) {
...
export async function captureStableFileV6(
  absolutePath,
  label = absolutePath,
  {
    expectedRoot,
  ...
  await assertExpectedRootV6(absolutePath, expectedRoot, label, true);
  const parents = await captureParentIdentitiesV6(absolutePath, label);
  let before;
  try {
  ...
  const requiredMode = requirePrivate ? PRIVATE_MODE_V6 : expectedMode;
  ...
  try {
    ...
    await assertParentIdentitiesV6(parents, label);
    await assertExpectedRootV6(absolutePath, expectedRoot, label, true);
    const capture = {
      kind: "present",
      absolutePath,
      ...
      bytes,
      digest: sha256BytesV6(bytes),
      mode: Number(opened.mode & 0o777n),
      uid: Number(opened.uid),
    };
    recordCaptureV6(ledger, capture);
    return capture;
  } finally {
    await handle.close();
  }
...
export function capturePrivateEvidenceV6(
  absolutePath,
  label = absolutePath,
  options = {},
) {
  return captureStableFileV6(absolutePath, label, {
    ...options,
    requirePrivate: true,
  });
}

export async function captureRequiredAbsentV6(
  absolutePath,
  label = absolutePath,
  { expectedRoot, ledger } = {},
) {
  requireAbsolutePath(absolutePath, label);
  await assertExpectedRootV6(absolutePath, expectedRoot, label, false);
  const parents = await captureParentIdentitiesV6(absolutePath, label);
  try {
...
export async function postflightCaptureLedgerV6(ledger) {
  requireLedgerV6(ledger);
  for (const capture of ledger.entries) {
    await assertParentIdentitiesV6(capture.parents, capture.label);
    await assertExpectedRootV6(
    ...
    const current = await captureStableFileV6(
      capture.absolutePath,
      capture.label,
      {
...
export async function publishPrivateExactV6(
  absolutePath,
  bytes,
  ...
  try {
    ...
    const existing = await captureOptionalPrivateV6(absolutePath, label, expectedRoot);
    if (existing) {
      if (!existing.bytes.equals(bytes)) {
        throw failure("THIRD_STATE", `${label} contains different existing bytes`);
      }
      await assertOpenedDirectoryV6(directoryHandle, expectedParent, label);
      await assertParentIdentitiesV6(parents, label);
    ...
    try {
      ...
      try {
        await temporaryHandle.chmod(PRIVATE_MODE_V6);
      ...
      try {
        ...
        const raced = await capturePrivateEvidenceV6(absolutePath, label, { expectedRoot });
      ...
      await assertOpenedDirectoryV6(directoryHandle, expectedParent, label);
      await assertParentIdentitiesV6(parents, label);
      const published = await capturePrivateEvidenceV6(absolutePath, label, { expectedRoot });
...
async function captureOptionalPrivateV6(absolutePath, label, expectedRoot) {
  try {
    await lstat(absolutePath, { bigint: true });
  } catch (error) {
  ...
  return capturePrivateEvidenceV6(absolutePath, label, { expectedRoot });
}
...
function recordCaptureV6(ledger, capture) {
...
function requireLedgerV6(ledger) {
...
async function captureParentIdentitiesV6(absolutePath, label) {
...
async function assertParentIdentitiesV6(identities, label) {
...
async function assertOpenedDirectoryV6(handle, expected, label) {
...
async function assertExpectedRootV6(absolutePath, expectedRoot, label, leafExists) {
...
function failure(code, message) {
  return new RuntimeIoV6Error(`RUNTIME_IO_V6_${code}`, message);
}
