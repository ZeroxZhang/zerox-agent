...
**Zerox Agent** is a local-first desktop control plane for personal AI agents. The current release is **v2.0.1**. The name derives from **Zero + X**: starting from a blank slate and turning unknown local workflows into observable, permissioned, workspace-scoped runs.
...
v2.0.1 rebuilds the work surface as a Kimi Work-style desktop task console while preserving the local-first runtime, ToolAuthorizationService checks, workspace sandboxing, Goal Mode, and recoverable run loop. The first screen now uses a Work/Chat segmented switch, Kimi-like work shortcuts, project and conversation groups, a document-style task transcript, progress/context side cards, and a bottom composer with Add content, All allow, context capacity, model, interrupt, and send controls. The change is intentionally UI and interaction focused: core model, tool, permission, memory, and goal execution behavior stays on the existing audited paths.
...
```bash
xattr -dr com.apple.quarantine ~/Downloads/Zerox.Agent-2.0.1-arm64.dmg
```
...
**Zerox Agent** 是一个本地优先的桌面智能体控制台，当前版本是 **v2.0.1**。名字取自 **Zero + X**——从留白开始，把未知的本地工作流转成可观察、受权限管控、可恢复的 Agent 运行。
...
v2.0.1 将工作界面重构为 Kimi Work 风格的桌面任务控制台，同时保留本地优先 runtime、ToolAuthorizationService 检查、workspace sandbox、Goal Mode 和可恢复运行循环。首屏现在包含 Work/Chat 分段切换、Kimi 风格工作入口、项目与对话分组、文档式任务 transcript、进度/上下文侧栏，以及带「添加内容」「全部允许」「上下文容量」「模型」「中断」「发送」控件的底部 composer。本次迭代聚焦界面设计、配色、功能区划分、交互方式和产品动线；模型、工具、权限、记忆和目标执行仍走原有可审计路径。
...
| 测试 | Vitest 4 | 110 个测试文件 / 538 个测试，覆盖共享层、主进程和渲染进程 |
