...
**Zerox Agent** is a local-first desktop control plane for personal AI agents. The current release is **v2.8.1**. The name derives from **Zero + X**: starting from a blank slate and turning unknown local workflows into observable, permissioned, workspace-scoped runs.
...
```bash
xattr -dr com.apple.quarantine ~/Downloads/"Zerox.Agent-2.8.1-arm64.dmg"
```
...
**Zerox Agent** 是一个本地优先的桌面智能体控制台，当前版本是 **v2.8.1**。名字取自 **Zero + X**——从留白开始，把未知的本地工作流转成可观察、受权限管控、可恢复的 Agent 运行。
