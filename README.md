...
**Zerox Agent** is a local-first desktop control plane for personal AI agents. The current release is **v1.8.0**. The name derives from **Zero + X**: starting from a blank slate and turning unknown local workflows into observable, permissioned, workspace-scoped runs.
...
v1.8.0 ships the session-native Goal Command release on top of the Agent Learning Harness Loop and Agent Capability runtime core. Goal Mode now behaves like part of a long conversation: users can type `/目标 ...` or open the composer command menu, choose Goal, and turn the current input into a bounded objective without leaving Chat. The composer command entry uses a Command-style icon, reserves room for future tool and permission commands, and keeps the send/stop/command buttons aligned to the input corner. The goal layer still provides bounded-autonomy goal state, local ledgers, planning, deterministic-first acceptance, review gates, goal-aware compaction, and goal-mode eval coverage. The deterministic agent eval suite now covers 21 runtime, native-tool, recovery, compaction, checkpoint, model-retry, research-writing, eval-candidate, multi-agent lineage, and goal-mode contracts.
...
```bash
xattr -dr com.apple.quarantine ~/Downloads/Zerox.Agent-1.8.0-arm64.dmg
```
...
**Zerox Agent** 是一个本地优先的桌面智能体控制台，当前版本是 **v1.8.0**。名字取自 **Zero + X**——从留白开始，把未知的本地工作流转成可观察、受权限管控、可恢复的 Agent 运行。
...
v1.8.0 在 Agent Learning Harness Loop 和 Agent Capability runtime core 之上发布会话原生的 Goal Command 版本。Goal Mode 现在更像长会话的一部分：用户可以输入 `/目标 ...`，也可以点开输入框里的命令菜单、选择「目标」，把当前输入直接变成有边界的长期目标，而不需要离开 Chat。输入框左侧入口改为 Command 风格图标，命令菜单为后续工具和权限命令预留位置，发送/中断/命令三个图标按钮也统一按输入框右下圆角对齐。目标层仍提供有边界自治目标状态、本地 ledger、规划、确定性优先验收、审核门、goal-aware compaction 和 goal-mode eval 覆盖。确定性 Agent eval suite 现在覆盖 21 个 runtime、原生工具、恢复、压缩、checkpoint、模型重试、研究写作、eval candidate、多 Agent lineage 和 goal-mode 契约。
...
截至 v1.8.0，`npm run verify` 覆盖 Vitest 测试、生产构建、Agent 评测和记忆检索评测；当前包含 105 个 Vitest 文件 / 507 个测试、21 个确定性 Agent eval fixture 和 2 个 memory eval fixture。Agent eval 覆盖原生代码工程、研究写作、测试失败反思、retry budget exhaustion、上下文压缩、tool-call checkpoint、模型重试、episode eval candidate、child handoff review gate 和 goal-mode bounded-autonomy 黄金路径。session-native Goal Mode 架构记录在 `docs/architecture/agent-goal-mode.md`，包含 Chat Session mode 流程、slash goal command、输入框命令菜单和旧 `#goals` 地址回到会话的行为：
